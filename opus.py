...
if sys.version_info < (3, 9):
    raise RuntimeError('Python 3.9+ required: math.nextafter (live order sizing, _float_safe_hundredths) was added in 3.9; this interpreter is %d.%d'
                       % sys.version_info[:2])
if os.name == 'nt':
...
def _check_deps() -> None:
    ...
    for mod, pkg in [('aiohttp', 'aiohttp'), ('websockets', 'websockets'), ('eth_account', 'eth-account'), ('web3', 'web3')]:
        try:
            ...
            missing.append(pkg)
    if missing:
        raise ImportError('Missing packages - run:  pip install ' + ' '.join(missing))
_check_deps()
import aiohttp
import websockets
...
try:
    ...
    # E1: the run's config is graded against whatever file this resolves to, so
    # record it. find_dotenv() here resolves from the same caller frame as
    # load_dotenv()'s internal call, i.e. the same file.
    _DOTENV_PATH = find_dotenv() or ''
...
try:
    ...
    _FAST_JSON = False
_BOT_VERSION = 'v19.5.9j-r46g-oracleage'
# R46c: event-loop starvation watchdog. 0 disables the task entirely.
# Measurement only - it never gates a trade. See _loop_lag_watch for why this
# exists rather than another guess at what starves the WS reader.
_LOOP_LAG_WARN_MS: float = float(os.getenv('LOOP_LAG_WARN_MS', '250') or 0.0)
# R46d: Polymarket closes a shard with `1013 slow consumer: send buffer full`
# when ITS send buffer to us fills. The mechanism was READ, not guessed:
# websockets 15.0.1 connect() defaults max_queue=16, which builds an Assembler
# with high=16, low=4 (websockets/asyncio/messages.py). maybe_pause() fires the
# moment len(frames) > high and calls the transport's pause_reading(), so a
# burst of 17 frames arriving faster than _handle() drains them stops us reading
# the socket at all; TCP backpressure then shuts the receive window and the
# venue closes us. That needs NO CPU saturation and NO event-loop starvation,
# which is exactly what we measure: 0 LOOPLAG breaches on an idle box (23% CPU,
# loadavg 0.30 of 2 cores) while the closes continue, and r46a cutting eval
# volume 4.7x moved the rate by nothing (2g-F: 7.13/hr vs 8.21/hr, p=0.29).
# DEFAULT 16 REPRODUCES THE LIBRARY DEFAULT EXACTLY, so r46d ships behaviour-
# neutral; raising it is a separate, single, pre-registered variable.
# Clamped at 16 because both ends of the range are worse failures than a shard
# close: high=0 pauses on every frame, and unbounded (None) trades the close for
# unbounded memory on a box already at 28% MEM.
_WS_MAX_QUEUE: int = max(16, int(float(os.getenv('WS_MAX_QUEUE', '16') or 16)))
# R46d: reader duty-cycle telemetry (Critic 2g-F ladder step 1). 0 disables.
# Measurement only - it never gates a trade.
_WS_DUTY_LOG_S: float = float(os.getenv('WS_DUTY_LOG_S', '60') or 0.0)
# R50: minimum per-shard inbound msg/s before the health loop flags the feed as
# throughput-stalled. DETECT-ONLY - it logs, it does not reconnect; see the
# health loop for why a reconnect is not the fix. The 90s _last_msgs age check
# below it is a LIVENESS test and a trickling venue socket defeats it: on
# 2026-08-26 04:03:31-07:56:37 BOTH shards ran at 0.1-23.9 msg/s (peak 4.4% of
# normal) for 3.9h while worst_gap peaked at 71.1s - under the 90s bound - so
# neither 'Shard N stale' nor 'ALL shards stale' ever fired. Verified: those two
# log lines occur 0 times in the whole 66.43h run. Measured over 7278 clean 60s
# windows: p0.1=1.4/s, p1=36.7/s, p50=540.5/s, p95=1262.3/s; the 390 stall
# windows sat at p50=2.28/s p90=9.68/s max=18.5/s. 20/s sits above every stall
# window and below only 0.71% of clean ones. 0 disables. sol_ver.md R50.15.
_WS_MIN_MSG_RATE: float = float(os.getenv('WS_MIN_MSG_RATE', '20') or 0.0)
...
# P-1 (Critic 2k-1(3), 2k-2): env knob, default 0.00025 = the previous literal
# exactly, so staging is behaviour-neutral. Critic verdict is "stage the knob, do
# not schedule the variable": at the candidate 2e-05 the MEDIAN displacement row
# already clips (z_med ~1.8-2.6 => Phi 0.97-0.995 => pinned at the 0.85 clip =>
# ceiling ~0.8197), i.e. the gate goes hyper-permissive before calibration can
# improve. Lowering this is a pre-registered single variable, never a default.
GBM_SIGMA_FLOOR_PER_SEC: float = max(0.0, float(os.getenv('GBM_SIGMA_FLOOR_PER_SEC', '0.00025') or 0.00025))
# Critic 2k-1(3): bound |z| before Phi. Read the comment at the clamp site before
# trusting this to hold the 0.85 clip shut -- it does NOT, because Phi(3)=0.9987.
_Z_ABS_CAP: float = max(0.0, float(os.getenv('LATARB_Z_ABS_CAP', '3') or 3.0))
...
# r46-e: venue-clock staleness floor used by OrderBook.is_stale. Default 5000.0 reproduces
# the previous hard-coded value exactly (behaviour-neutral). Lowering it is what makes the
# age gates able to see queue-induced staleness when WS_MAX_QUEUE is raised above 16.
_EXCHANGE_STALE_FLOOR_MS: float = max(0.0, float(os.getenv('EXCHANGE_STALE_FLOOR_MS', '5000') or 5000.0))
# --- ALT-A (Critic 2k-3): frozen 2D empirical q table, q(disp_bucket x ask_band).
# Data, not code. The isotonic (PAVA) + Dirichlet fit lives offline in
# fit_q_table.py and emits this file; the hot path only indexes it. Unset knob or
# any malformed file => _emp_q behaves exactly as r46-e, so staging is provably
# behaviour-neutral and the table can be refit without a code deploy.
_Q_TABLE_PATH: str = os.getenv('LATARB_Q_TABLE', '') or ''
_Q_TABLE: Optional[Dict[str, Any]] = None
_Q_TABLE_MD5: str = ''
_Q_TABLE_TRIED: bool = False


def _load_q_table() -> Optional[Dict[str, Any]]:
    """Parse+validate the frozen q table once. Any defect => None, i.e. OFF.

    Failing closed is deliberate: a half-parsed calibration is worse than none,
    because the fallback ladder would answer from marginals that were never fit.
    _Q_TABLE_TRIED (not `_Q_TABLE is None`) guards the retry, or a missing file
    would be re-opened on every evaluation inside the hot path.
    """
    global _Q_TABLE, _Q_TABLE_MD5, _Q_TABLE_TRIED
    if _Q_TABLE_TRIED or not _Q_TABLE_PATH:
        return _Q_TABLE
    _Q_TABLE_TRIED = True
    try:
        raw = open(_Q_TABLE_PATH, 'rb').read()
        t = _json_loads(raw)
        for k in ('disp_edges', 'ask_edges', 'cells', 'global'):
            if k not in t:
                raise ValueError('q table missing %r' % k)
        if not t['disp_edges'] or not t['ask_edges']:
            raise ValueError('q table has an empty axis')
        _Q_TABLE_MD5 = hashlib.md5(raw).hexdigest()
        _Q_TABLE = t
    except Exception:
        _Q_TABLE = None
    return _Q_TABLE
...
def _q_bucket(edges: List[float], x: float) -> int:
    """Index of the half-open bucket [edges[i], edges[i+1]) holding x."""
    return max(0, min(len(edges) - 1, bisect.bisect_right(edges, x) - 1))


def _q_lookup(t: Dict[str, Any], disp_bps: float, ask: float) -> Optional[float]:
    """ALT-A ladder: cell -> ask marginal -> disp marginal -> global.

    Every rung must carry >= n_floor graded rows or it is skipped, so a sparse
    cell can never outvote a well-populated marginal. None => the caller keeps
    its existing q, which is what makes an incomplete table safe to ship.
    """
    nf = float(t.get('n_floor', 50) or 0)
    i = _q_bucket([float(e) for e in t['disp_edges']], float(disp_bps))
    j = _q_bucket([float(e) for e in t['ask_edges']], float(ask))
    for src in (t.get('cells', {}).get('%d,%d' % (i, j)),
                t.get('ask_marg', {}).get(str(j)),
                t.get('disp_marg', {}).get(str(i)),
                t.get('global')):
        if isinstance(src, dict) and float(src.get('n', 0)) >= nf:
            q = float(src.get('q', 0.0))
            if 0.0 < q < 1.0:
                return q
    return None


# LEAK #5: warn threshold for the 300s-boundary sub-step timer in _fivemin_refresh.
_FIVEMIN_STEP_WARN_MS: float = max(0.0, float(os.getenv('FIVEMIN_STEP_WARN_MS', '100') or 100.0))
# R36 (claim 1): max oracle age accepted as the LatArb PRICE input.
# Before R33 the oracle was only a sign confirmation, so 2.0s was harmless and
# that is what `feed.price(coin, max_age_s=2.0)` still passed. R33 promoted the
# oracle to the price input on BOTH ends of `displacement`, which changes what a
# stale read costs: a 2s-old oracle against a live book is a 2s-old opinion
# about a price the market has already repriced, and any FAK that fills on it is
# adverse selection by construction.
#
# What this bounds, precisely: `price_age_s` is time since the last RTDS PUSH
# (:2746 stamps every accepted message, changed price or not), NOT time since
# the price last moved. So this rejects a feed that has gone QUIET - a dead or
# reconnecting socket - and does not reject a genuinely flat market.
#
# Sizing evidence (_verify_oracle_lag.py, 1,096 operator shadow rows): the
# oracle VALUE changes on a ~1.20-1.30s median interval, measured at a 0.66-1.02s
# row cadence, so true push cadence is at or below that. Every no-change gap
# >30s in the log was verified to be a shadow-logging gap between market
# intervals (0-1 rows inside), not a frozen feed. 1.2s therefore sits at the
# observed change interval: it tolerates normal pushes and trips on silence.
# Tightening further would reject healthy ticks; loosening re-admits the stale
# read that claim 1 is about.
# Phase 1.2: staged from the env at the SAME 1.2s so a re-tune no longer needs a
# code edit. DRYRUN_ONLY.env rev2 records that no knob existed ("it cannot be set
# from any env file"), which is why 1.2 was never tested against an alternative.
LATARB_ORACLE_MAX_AGE_S: float = max(0.0, float(os.getenv('LATARB_ORACLE_MAX_AGE_S', '1.2') or 1.2))
...
# E1 FIX: renamed from LATARB_MIN_LIVE_FILL_RATE Ã¢â‚¬â€ that name collided with the
...
# R40: the CLOB api-key derive call fails in a way that is indistinguishable from
# a bad key. Measured 2026-08-19: 5 consecutive failures, then 6 consecutive
# successes, then 3 more failures, then a sustained outage - all on the same
# unchanged key. The SDK reports it as
# PolyApiException[status_code=None, error_message=Request exception!], which
# discards the cause; unwrapping __context__ shows requests.ReadTimeout.
#
# R41 correction: an earlier version of this comment cited "401 in 33ms" as proof
# the endpoint was healthy. That probe was UNSIGNED, so it only exercised header
# validation. Probing with valid L1 headers shows the real behaviour:
#     GET  /ok                    -> 200 in   36ms
#     GET  /auth/derive-api-key   -> 401 in   43ms   (unsigned: header check only)
#     GET  /auth/derive-api-key   -> 400 in 23.5s    (signed: "Could not derive
#                                                     api key!" - origin failed)
#     POST /auth/api-key          -> 502 in 31.3s    (origin bad gateway)
# The venue ACCEPTS our L1 headers (a different error than the unsigned case) and
# then fails downstream, so signing is fine and the auth backend is degraded.
# Retrying is still right - it absorbs the short flaps - but no boot-time retry
# budget survives a multi-minute outage, hence _CREDS_CACHE below.
_SDK_AUTH_RETRIES: int = 4
_SDK_AUTH_BACKOFF_S: float = 2.0
# R41: api creds are derived deterministically from the private key, so they are
# stable across boots and safe to cache. Without this, a derive outage bricks
# every boot even though the creds the bot needs have not changed. Keyed by
# sha256[:16] of the key so rotating the key invalidates the cache, and by
# sig_type/proxy so a config change does not silently reuse the wrong identity.
_CREDS_CACHE_PATH: str = os.path.expanduser('~/.polymarket_creds.json')


def _creds_cache_id(private_key: str, sig_type: int, proxy: str) -> str:
    h = hashlib.sha256((private_key or '').encode()).hexdigest()[:16]
    return f'{h}:{int(sig_type)}:{(proxy or "").lower()}'


def _load_cached_creds(cache_id: str, log: Any) -> Optional[Any]:
    """Return cached ApiCreds for this identity, or None. Never raises."""
    if ApiCreds is None:
        return None
    try:
        with open(_CREDS_CACHE_PATH, 'r', encoding='utf-8') as fh:
            blob = json.load(fh)
        if not isinstance(blob, dict) or blob.get('cache_id') != cache_id:
            return None
        k, s, p = blob.get('api_key'), blob.get('api_secret'), blob.get('api_passphrase')
        if not (k and s and p):
            return None
        return ApiCreds(api_key=str(k), api_secret=str(s), api_passphrase=str(p))
    except FileNotFoundError:
        return None
    except Exception as e:
        log.warning('creds cache unreadable (ignoring): %s', str(e)[:120])
        return None


def _save_cached_creds(cache_id: str, creds: Any, log: Any) -> None:
    """Persist creds 0600. Best-effort: a write failure must not block boot."""
    tmp = _CREDS_CACHE_PATH + '.tmp'
    try:
        blob = {'cache_id': cache_id, 'api_key': creds.api_key,
                'api_secret': creds.api_secret, 'api_passphrase': creds.api_passphrase}
        ...
        log.info('api creds cached (0600) for reuse across boots: %s', _CREDS_CACHE_PATH)
    except Exception as e:
        log.warning('could not cache api creds (non-fatal): %s', str(e)[:120])
        try:
            os.unlink(tmp)
        except OSError:
            pass
...
# R38: measured lag from interval close to CLOB tokens[].winner being set.
# Polled btc-updown-5m-1787053200 every 20s: winner appeared 429s after its true
# end_time (the slug epoch is interval START, so 729s-300s), with closed flipping
# False->True in the same tick.  Dry settlement fires at end+5s
# (settle_expired_latarb grace), so without this wait the spot ESTIMATE wins the
# race every time - which is how 25/25 settles were booked from spot and 8 named
# the wrong winner.  Below DRY_STALE_SETTLE_S=600 so the flat-close backstop still
# ends any market the chain never resolves.
DRY_ONCHAIN_WAIT_S: float = float(os.environ.get('DRY_ONCHAIN_WAIT_S', '570'))
...
def write_run_config(cfg: 'Config') -> str:
    """E1: dump the RESOLVED config to a per-run JSON and return its path.

    A committed .env is not evidence of what a run loaded: the 2026-09-03 run
    differed from .env.ec2-20260904 on COINS, MIN/MAX_ORDER_USDC, MAX_POSITION,
    WS_SHARD_COUNT, LATENCY_ARB_EDGE, LATARB_ASK_CAP, MAX_SPREAD_PCT and
    MIN_TOP_BOOK_USDC. 47 of the env names this file reads are absent from that
    file entirely, so their values came from source defaults, not from any file
    a reviewer can read. This dump is the only artefact that pins all of them,
    including the module-level constants that are not Config fields at all.
    """
    path = os.path.expanduser(os.environ.get('RUN_CONFIG_PATH', '') or f'~/polybot_runconfig_{_BUILD_FINGERPRINT}_{os.getpid()}.json')
    body = {k: v for k, v in asdict(cfg).items() if k != 'private_key'}
    body['private_key_sha256_8'] = hashlib.sha256(cfg.private_key.encode()).hexdigest()[:8] if cfg.private_key else ''
    # Not Config fields, so no .env diff can recover them.
    body['_module_constants'] = {
        'LATARB_ORACLE_MAX_AGE_S': LATARB_ORACLE_MAX_AGE_S,
        'LATARB_DUAL_BOOK_STALE_MS': LATARB_DUAL_BOOK_STALE_MS,
        'GBM_SIGMA_FLOOR_PER_SEC': GBM_SIGMA_FLOOR_PER_SEC,
        'LATARB_Z_ABS_CAP': _Z_ABS_CAP,
        'WS_MIN_MSG_RATE': _WS_MIN_MSG_RATE,
        'WS_MAX_QUEUE': _WS_MAX_QUEUE,
        'EXCHANGE_STALE_FLOOR_MS': _EXCHANGE_STALE_FLOOR_MS,
        'DRY_ONCHAIN_WAIT_S': DRY_ONCHAIN_WAIT_S,
        'DRY_STALE_SETTLE_S': DRY_STALE_SETTLE_S,
        'LATARB_KILL_SWITCH_MIN_FILL_RATE': LATARB_KILL_SWITCH_MIN_FILL_RATE,
        'LATARB_Q_TABLE': _Q_TABLE_PATH,
        'VENUE_MIN_ORDER_USDC': VENUE_MIN_ORDER_USDC,
    }
    body['_meta'] = {
        'build': _BUILD_FINGERPRINT, 'pid': os.getpid(), 'argv': sys.argv[1:],
        'started_utc': datetime.now(timezone.utc).isoformat(timespec='seconds'),
        'dotenv_loaded': _DOTENV_PATH,
    }
...
class Config:
    ...
    # Live proof (fills + settlement ledger) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â stricter than shadow-only GO.
    ...
    # R36 (claim 3): was 'https://polygon-rpc.com', re-verified DEAD on
    # 2026-08-02 (0/4 JSON-RPC POSTs, HTTP 401, even with a browser UA - so it
    # is not UA-blocking). As `primary` it is EXEMPT from _POLYGON_RPC_DENYLIST
    # by design (:871 keeps an explicitly-configured endpoint), so an operator
    # who never sets POLYGON_RPC_URL paid a guaranteed failed round-trip on
    # every connect. Default is now the fastest verified endpoint (420ms median).
    ...
    # 2026-09-03: the tradeable edge is confined to books whose YES ask barely
    # moves, and the bot had no filter for it - so it also traded the books that
    # move, and the two populations cancelled to ~zero. That is why every prior
    # config search concluded "no edge": it measured the average of a winner and
    # a loser. Measured on 668,832 shadow rows / 3,274 graded markets with a
    # CAUSAL (prefix-only, no look-ahead) label and the bot's own unmodified
    # gate:
    #   quiet (range <= 0.02, >= 3 prior samples): n=124 net +40.75 c/share
    #     CI[+36.46,+45.04], wr 0.9274, Wilson floor 0.8678 vs breakeven 0.5200
    #     -> OOS second half n=62 +35.59 c/share, floor 0.7655 vs be 0.5152
    #   moving (same rule, range > 0.02):          n=1239 net -4.03 c/share
    #     CI[-6.74,-1.33], wr 0.4092
    # Confirmed independently by the bot's own dry-run receipts: 65 quiet
    # settles +250.65 (+$3.86 each) vs 738 moving settles -319.03 (-$0.43 each).
    # Not a feed-stall artifact (clean-feed-only slice still clears at n=73; 50%
    # top-5-hour concentration vs 89% for the known artifact) and not an
    # elapsed-time artifact (quiet with >half the clock left wins 0.8493).
    # CAVEAT: 3.64 days of tape, 76 of 124 entries on tape-day 0. The per-day
    # rate is the least trustworthy number here; the sign is the robust one.
    # The window is fed from _shadow_scan, so LATARB_SHADOW must stay on -
    # Config.validate enforces that rather than letting the filter starve.
    # 0 samples = filter off (restores the pre-2026-09-03 behaviour).
    # E2: this default was 3 while from_env's default is 0, so every run to date
    # ran with the filter OFF (banner: QuietBk OFF min_samples=0) while the
    # dataclass advertised ON. Aligned on 0 = what actually ran; turning the
    # filter on is an explicit LATARB_QUIET_MIN_SAMPLES=3 in the .env.
    latarb_quiet_min_samples: int = 0
    latarb_quiet_ask_range: float = 0.02
    # 2026-08-06 AUDIT: the favored-side ask cap was HARDCODED at 0.65 in _eval_market and in
    # --latarb-analyze, and ask_cap was the #1 latarb gate (204,866 of 712,866 evals in the
    # 2026-08-05/06 run) - but it could not be tuned without editing code.  Now a config knob.
    latarb_ask_cap: float = 0.65
    # R38: floor on the ask. model_prob is ~0.56 for every candidate (measured
    # range 0.559-0.624 over all 20 ttc>=30 candidates) because sigma sits on
    # GBM_SIGMA_FLOOR_PER_SEC and shrinks every z toward 0, so
    # edge = model_prob - ask is almost purely a function of ask: candidates with
    # edge>0.15 have mean ask 0.2411, those with edge<=0.15 have mean ask 0.4682.
    # A cheap book therefore clears the edge gate arithmetically, not because it
    # is mispriced - the top 4 claim 0.40-0.54 edge at asks 0.01/0.08/0.08/0.16.
    # 0.20 is the smallest floor excluding the 0.16 fill that booked the largest
    # apparent win and lost on-chain; it drops 4/20 candidates and 1/29 fills, so
    # it guards the degenerate tail without moving the population that traded.
    latarb_min_ask: float = 0.20
    # R44: verified on 655 venue-graded candidates (Aug 19-22): DN entries
    # underperform their own ask by -8.4c/share (t=-3.02) on every coin and in
    # both tape regimes, while UP is ~fair (+2.6c, t=+1.0). LATARB_SIDE_FILTER
    # removes the measured bleed; it does NOT create significant edge (UP-only
    # EV +1.6c/$ at |disp|>=5bps is t=+0.57 - indistinguishable from zero).
    latarb_side_filter: str = 'BOTH'
    # R44: displacement <5bps at evaluation wins 32% vs ~40% breakeven (n=75),
    # the worst cell in the graded corpus. Units: basis points of price move.
    latarb_min_disp_bps: float = 0.0
    # R45: empirical-calibration win rates (graded, not modeled). When either
    # is >0 the edge input model_prob=Phi(z) - which overpredicts +13.82pp
    # (z~=7) and ranks at chance (AUC 0.5418) on the graded corpus - is
    # REPLACED by these values for the traded side. Graded Aug 19-22:
    # UP 0.5148 (n=357), DN 0.3926 (n=298). Both-or-neither; 0/0 disables.
    latarb_emp_wr_up: float = 0.0
    latarb_emp_wr_dn: float = 0.0
    # R46b (COLLAB_BOARD 2g-C): band-aware q. Flat q makes the edge gate an ASK
    # gate - edge = q - ask - slip - fee >= EDGE solves to ask <= 0.4763 at
    # q=0.5148 - so the only ask band that has ever paid, [0.48,0.60) at
    # +2.07c/$ on 135 CLOB-graded rows, is unreachable by construction. No
    # setting of a single q can reach it: raising q to admit ask>=0.48 also
    # re-admits every cheaper ask, and cheap asks are where the book
    # CONTRADICTS our own oracle trigger (adverse selection: 15.6% win at
    # ask~0.19). A per-band q is the only way to price the two populations
    # differently. 0.0 = OFF and reproduces r45 exactly.
    # HIGH applies when the traded side's ask >= split, LOW below it.
    # Maintenance rule (operational, not enforceable here): do not raise HIGH
    # without >=100 graded in-band rows behind it.
    latarb_q_split_ask: float = 0.0
    latarb_emp_wr_up_low: float = 0.0
    latarb_emp_wr_up_high: float = 0.0
    # R46b (Critic 2d-2): minimum fraction of the interval elapsed before a
    # LatArb entry. ENTRY_START_S/ENTRY_END_S do NOT gate this path - they are
    # directional-strategy knobs (:4941, :5004); the LatArb window is the
    # hard-coded 30 <= ttc <= tf_secs-15 at :7437. Rationale for the gate:
    # late-window displacement has survived most of the Chainlink TWAP60
    # integration window, so less mean-reversion remains. 0.0 = OFF.
    latarb_min_elapsed_frac: float = 0.0
    ...
    # Calibrated from calibration_v19 adverse_bps: toxic-only p50ÃƒÂ¢Ã¢â‚¬Â°Ã‹â€ 250bps, p75ÃƒÂ¢Ã¢â‚¬Â°Ã‹â€ 89
    ...
    # Max LIVE latarb entries sharing one (tf_secs, end_time) interval in the
    # SAME direction, counted at the commit point in LatencyArb._eval_market.
    # 1 = the correlated double-bet of 2026-08-19 (BTC DN + SOL DN, 2.1s
    # apart, one 5m window, -$5.12) cannot recur.  See _patch_corr_cap.py.
    max_latarb_per_interval_dir: int = 1
    ...
    def validate(self) -> List[str]:
        ...
        # model_prob is hard-clamped to [0.3, 0.85] in LatArb ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reject impossible config
        ...
        if self.latarb_quiet_min_samples > 0 and (not self.latarb_shadow):
            errs.append(f'LATARB_QUIET_MIN_SAMPLES={self.latarb_quiet_min_samples} requires LATARB_SHADOW=true - the rolling ask window is fed from _shadow_scan, so with shadow off no market ever reaches the sample floor and every entry is skipped as quiet_unproven')
        if self.latarb_quiet_min_samples > 0 and self.latarb_quiet_ask_range <= 0.0:
            errs.append(f'LATARB_QUIET_ASK_RANGE={self.latarb_quiet_ask_range} <= 0 with the quiet filter on rejects every book (a real range is >= 1 tick = 0.01); set LATARB_QUIET_MIN_SAMPLES=0 to disable the filter instead')
        if self.latarb_min_ask >= self.latarb_ask_cap:
            errs.append(f'LATARB_MIN_ASK={self.latarb_min_ask} >= LATARB_ASK_CAP={self.latarb_ask_cap} leaves no tradable ask band')
        if not 0.0 <= self.latarb_min_ask < 0.5:
            errs.append(f'LATARB_MIN_ASK={self.latarb_min_ask} must be in [0.0, 0.5) - at/above 0.5 it would reject the coin-flip band the strategy actually trades (measured fills 0.33-0.53)')
        if self.latarb_ask_cap > 0.85:
            errs.append(f'LATARB_ASK_CAP={self.latarb_ask_cap} > 0.85 model_prob clamp makes the edge gate unsatisfiable (edge = model_prob - ask - slippage - fee < 0 always)')
        if self.latarb_side_filter not in ('UP', 'DN', 'BOTH'):
            errs.append(f"LATARB_SIDE_FILTER={self.latarb_side_filter!r} must be one of UP, DN, BOTH")
        if not 0.0 <= self.latarb_min_disp_bps <= 100.0:
            errs.append(f'LATARB_MIN_DISP_BPS={self.latarb_min_disp_bps} must be in [0, 100] basis points')
        _emp_set = [v for v in (self.latarb_emp_wr_up, self.latarb_emp_wr_dn) if v > 0.0]
        if len(_emp_set) == 1:
            errs.append('LATARB_EMP_WR_UP / LATARB_EMP_WR_DN must both be set or both be 0 (half-configured empirical mode)')
        for _nm, _v in (('UP', self.latarb_emp_wr_up), ('DN', self.latarb_emp_wr_dn)):
            if _v > 0.0 and not 0.3 <= _v <= 0.9:
                errs.append(f'LATARB_EMP_WR_{_nm}={_v} must be 0 or in [0.3, 0.9]')
        # R46b band-aware q (2g-C). Same both-or-neither discipline as r45: a
        # half-configured split would silently fall back to flat q for one band
        # and look like it was working.
        if not 0.0 <= self.latarb_q_split_ask < 1.0:
            errs.append(f'LATARB_Q_SPLIT_ASK={self.latarb_q_split_ask} must be in [0.0, 1.0) (0 = band-aware q off)')
        _band_set = [v for v in (self.latarb_emp_wr_up_low, self.latarb_emp_wr_up_high) if v > 0.0]
        if self.latarb_q_split_ask > 0.0 and len(_band_set) != 2:
            errs.append(f'LATARB_Q_SPLIT_ASK={self.latarb_q_split_ask} requires BOTH LATARB_EMP_WR_UP_LOW and LATARB_EMP_WR_UP_HIGH to be set')
        if self.latarb_q_split_ask <= 0.0 and _band_set:
            errs.append('LATARB_EMP_WR_UP_LOW/HIGH are set but LATARB_Q_SPLIT_ASK=0, so band-aware q is off and they would be silently ignored')
        for _nm, _v in (('LOW', self.latarb_emp_wr_up_low), ('HIGH', self.latarb_emp_wr_up_high)):
            if _v > 0.0 and not 0.3 <= _v <= 0.9:
                errs.append(f'LATARB_EMP_WR_UP_{_nm}={_v} must be 0 or in [0.3, 0.9]')
        if len(_band_set) == 2 and self.latarb_emp_wr_up_high < self.latarb_emp_wr_up_low:
            errs.append(f'LATARB_EMP_WR_UP_HIGH={self.latarb_emp_wr_up_high} < LATARB_EMP_WR_UP_LOW={self.latarb_emp_wr_up_low}: the expensive-ask band is the one the book CONFIRMS, so its win rate cannot be the lower of the two')
        if not 0.0 <= self.latarb_min_elapsed_frac < 1.0:
            errs.append(f'LATARB_MIN_ELAPSED_FRAC={self.latarb_min_elapsed_frac} must be in [0.0, 1.0) (0 = off)')
        if self.rate_limit <= 0:
            errs.append(f'rate_limit must be positive, got {self.rate_limit}')
        if self.max_order_size < 2 * self.min_order_size:
            log.warning('max_order_size ($%.1f) < 2x min_order_size ($%.1f) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Kelly sizing is effectively disabled; all trades will fire at the venue minimum', self.max_order_size, self.min_order_size)
        ...
        if self.complement_arb_enabled:
            # R36 (claim 5): the implementation was deleted (see :4838), so this
            # flag now has NOTHING to enable. Kept as a hard boot failure rather
            # than silently ignored: an operator who sets it believes they are
            # turning on a hedged arb, and booting quietly would leave them
            # trading a strategy they think is running and is not.
            errs.append('COMPLEMENT_ARB_ENABLED=true is not supported in this build; the non-atomic two-leg arb was REMOVED (a killed FOK leg left a naked position). Set it false. Re-enabling requires depth-walked sizing and an atomic single-fill unwind.')
        if self.entry_start_s >= self.entry_end_s:
            errs.append(f'entry_start_s ({self.entry_start_s}) must be < entry_end_s ({self.entry_end_s}): the entry window would be empty')
        if not self.coins:
            errs.append('coins must not be empty (set COINS, e.g. BTC,ETH,SOL)')
        if self.strategy_interval_s <= 0.0:
            errs.append(f'strategy_interval_s must be > 0, got {self.strategy_interval_s} (a non-positive loop interval busy-spins)')
        if self.discovery_interval_s <= 0.0:
            errs.append(f'discovery_interval_s must be > 0, got {self.discovery_interval_s}')
        if self.eval_debounce_ms < 0.0:
            errs.append(f'eval_debounce_ms must be >= 0 (0 = no debounce), got {self.eval_debounce_ms}')
        if self.ws_shard_count < 1:
            errs.append(f'ws_shard_count must be >= 1, got {self.ws_shard_count}')
        if self.max_open_orders < 1:
            errs.append(f'max_open_orders must be >= 1, got {self.max_open_orders}')
        if self.cycle_s <= 0:
            errs.append(f'cycle_s must be > 0, got {self.cycle_s}: rescale_for_cycle() skips a non-positive cycle, so entry/exit timings would silently keep their 300s defaults')
        if self.max_latarb_per_interval_dir < 1:
            errs.append(f'max_latarb_per_interval_dir must be >= 1, got {self.max_latarb_per_interval_dir}: 0 silently blocks every latarb entry instead of disabling the strategy')
        return errs
...
def _quiet_note(w: Optional[List[float]], ask: Optional[float]) -> Optional[List[float]]:
    """Fold one YES-ask sample into a per-market [min, max, count] window.

    Returns the window (new, or mutated in place), or the input unchanged when
    `ask` is not a usable price - the measured rule ignored rows with
    yes_ask <= 0, so those must not count toward the sample floor either.

    The SAMPLE CADENCE is the caller's and it is load-bearing: the rule was
    measured on shadow rows, i.e. one sample per market per
    LATARB_SHADOW_THROTTLE_MS (3s) from a book fresher than
    LATARB_SHADOW_MAX_AGE_MS (250ms). Sampling faster shrinks the wall-clock
    span the range covers, so a book that is merely briefly still would be
    labelled quiet. Do not call this from the per-tick entry path.

    Once _quiet_reason has frozen a verdict (a 4th slot exists) the window is
    CLOSED and further samples are ignored: the backtest read the prefix up to
    the first candidate row and then stopped, so widening the range afterwards
    would re-decide a market the measured rule had already decided."""
    if ask is None or not ask > 0.0:
        return w
    if w is None:
        return [ask, ask, 1.0]
    if len(w) > 3:
...
# Frozen-verdict codes stored in slot 3 of a _quiet_ask window.
_QUIET_PASS, _QUIET_MOVES, _QUIET_UNPROVEN = 0.0, 1.0, 2.0


def _quiet_reason(w: Optional[List[float]], min_samples: int, max_range: float) -> str:
    """'' when the book qualifies as quiet, else the skip key saying why not.

    DECIDED ONCE PER MARKET, AT FIRST CANDIDACY, THEN FROZEN. This is what the
    backtest did and the distinction is enormous, not cosmetic:

      _prefix.py takes the prefix of rows at-or-before the first row that clears
      (edge, ask band, depth), tests `max-min <= range` over it, and then
      `break`s - one decision per market, at a median of 4 samples.

    An earlier build re-read an ever-widening cumulative min/max on EVERY tick
    for the market's whole life. A whole-history range is monotone
    non-decreasing, so that filter got strictly tighter with market age and was
    NOT the rule that was validated. Replayed over 703,680 tape rows:

        samples in window   frac with range <= 0.02
        <= 3                0.4079     <- what the backtest measured
        <= 10               0.0362
        > 50                0.0001     <- what the cumulative gate converged to

    Live confirmation of the cumulative build: 412 gate arrivals, 412
    not_quiet, 0 entries in 3h10m (measured pass rate 0.0063). Verified
    2026-09-03 on latarb_shadow_v20_strict.csv.

    min_samples <= 0 disables the filter."""
    if min_samples <= 0:
        return ''
    if w is not None and len(w) > 3:
        # Already decided. Never re-evaluate: the measured rule looked once.
        v = w[3]
        return '' if v == _QUIET_PASS else ('not_quiet' if v == _QUIET_MOVES else 'quiet_unproven')
    if w is None:
        # The SAMPLER has not run on this market yet, so there is nothing to
        # decide on and nothing to freeze. No backtest analogue exists either:
        # every candidate row in the tape WAS a shadow row, so a window always
        # existed at candidacy. Wait for one sample, then decide once.
        return 'quiet_unproven'
    if w[2] < min_samples:
        # First candidacy with too little evidence. FROZEN, because the
        # measurement `break`s here and discards the market rather than waiting
        # for more samples - and the cost of getting this wrong is large, not
        # cosmetic. Replaying the tape through these helpers, first-candidacy
        # QUIET reproduces the validated cell exactly (188 of 188 markets, plus 1
        # the measurement dropped only for having no Gamma label). Letting an
        # under-K read retry instead admits 301 further markets; fresh Gamma
        # resolved 301/301 of them and they win 0.5670 against the cell's 0.7962,
        # diluting the rule from +28.64 to +16.53 c/share. Verified 2026-09-03.
        w.append(_QUIET_UNPROVEN)
        return 'quiet_unproven'
    # DELIBERATE exact float comparison - do NOT add a tick epsilon. 0.52-0.50
    # is 0.020000000000000018 in IEEE754, so a true 2-tick range fails `> 0.02`
    # unless the two prices happen to subtract clean. That is the arithmetic the
    # rule was MEASURED with, and it is load-bearing: replaying the tape with
    # `> max_range + 1e-9` admits 9 further markets that between them net
    # -28.96 c/share (wr 0.2222) and drop the cell's Wilson floor from 0.8678 to
    # 0.8135. An epsilon here silently widens the population past what was
    # validated. Verified 2026-09-03 on latarb_shadow_v20_strict.csv.
    if w[1] - w[0] > max_range:
        w.append(_QUIET_MOVES)
        return 'not_quiet'
    w.append(_QUIET_PASS)
    return ''
...
def ioc_buy_request_shares(size_usdc: float, price: float, max_usdc: Optional[float]=None, mkt: Any=None) -> float:
    """Shares an IOC/FAK BUY of `size_usdc` at `price` actually requests.

    E3: this was computed independently in three places - place_order (live),
    place() (dry-run simulator) and the LATARB FOK caller - and they drifted.
    The LATARB caller used the raw size/price, so fill_fraction was measured
    against a share count no order ever asked for: 942 of 1156 filled rows in the
    2026-09-03 run (fingerprint 2c5f7659a774) reported a partial, and 924 of
    those were a 100% fill of the quantized size - only 18 were real partials.
    Returns 0.0 when no venue-legal size fits, exactly like
    quantize_ioc_buy_shares. Checked by test_e3_ioc_request_shares.py.
    """
    shares = round(size_usdc / max(price, 0.001), 6)
    if shares < 1.0:
        shares = 1.0
    return quantize_ioc_buy_shares(shares, price, max_usdc=max_usdc, min_shares=venue_min_shares(mkt))
...
# R36 (claim 3) re-measured 2026-08-02, 4 JSON-RPC POSTs each, browser UA (the
# UA matters: the default urllib UA gets 403 from Cloudflare on live endpoints
# and makes healthy hosts look dead):
#     polygon-bor-rpc.publicnode.com  4/4   420ms median
#     polygon.drpc.org                4/4   508ms median
#     1rpc.io/matic                   4/4  3902ms median  <- 7.7x slower
#     polygon-rpc.com                 0/4   HTTP 401      <- still dead, removed
# polygon-rpc.com dropped from the pool. 1rpc.io/matic KEPT but moved last: it
# answers correctly, and a slow fallback still beats no fallback when the two
# fast endpoints are down - but at 3.9s it must never be tried before them.
_POLYGON_RPC_DEFAULTS: Tuple[str, ...] = ('https://polygon-bor-rpc.publicnode.com', 'https://polygon.drpc.org', 'https://1rpc.io/matic')
...
_POLYGON_RPC_DENYLIST: Tuple[str, ...] = ('https://polygon.llamarpc.com', 'https://rpc.ankr.com/polygon', 'https://polygon-rpc.com')
...
class OrderBook:
    ...
    # Local receipt time (monotonic) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â transport age for LatArb stale-quote gate.
    ...
    def age_ms(self) -> float:
        """Local transport age (receipt ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ now). Primary LatArb book-age metric."""
    ...
    def is_stale(self, max_ms: float) -> bool:
        ...
        if math.isfinite(ea) and ea > max(max_ms, _EXCHANGE_STALE_FLOOR_MS):
            return True
        return False
...
class Market:
    ...
    # Polymarket feeRate coefficient (fee = rate * (p*(1-p))**exponent). None ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ Config.category_fee_rate.
    fee_rate: Optional[float] = None
    fee_exponent: float = 1.0
    # Conditional-token share minimum from CLOB book (not USDC). None ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ unenforced.
...
class PolyClient:
    ...
    async def _build_sdk(self, sig_type: int) -> bool:
        if not _HAS_SDK:
            self.log.error('Polymarket SDK not installed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â run: %s/bin/pip install py-clob-client-v2' % sys.prefix)
            return False
        if not _SDK_IS_V2:
            self.log.critical('Polymarket CLOB V1 SDK (py-clob-client) detected ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â CLOB V1 was retired 2026-04-28 and V1-signed orders are rejected with order_version_mismatch. Install the V2 SDK and restart:\n    %s/bin/pip install py-clob-client-v2' % sys.prefix)
            return False
        ...
        try:
            ...
            if _derive is None:
                raise RuntimeError('SDK exposes neither create_or_derive_api_key nor create_or_derive_api_creds ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â incompatible py-clob-client')
            # R41: try cached creds before the network. Validated with get_api_keys()
            # (a cheap L2 call) so stale creds fall through to derive rather than
            # failing later on the first real order.
            cache_id = _creds_cache_id(self.cfg.private_key, sig_type, self.cfg.proxy_address)
            creds = None
            cached = _load_cached_creds(cache_id, self.log)
            if cached is not None:
                try:
                    sdk.set_api_creds(cached)
                    await loop.run_in_executor(None, sdk.get_api_keys)
                    creds = cached
                    self.log.info('SDK auth from cached creds (derive skipped)')
                except Exception as _ce:
                    # R41b: only an AUTH rejection (401/403) proves the cached creds are
                    # wrong. A 5xx or a timeout is the venue's problem and says nothing
                    # about them - and creds are derived deterministically from the key,
                    # so re-deriving cannot return anything different. Measured
                    # 2026-08-19 10:33:41Z: get_api_keys -> 500 "Could not retrieve API
                    # keys", then the fallthrough derive -> 400, 53ms later. Valid creds
                    # were discarded and the boot died. Verified minutes later that the
                    # SAME cached api_key was what derive returned. So: keep them.
                    _cs = getattr(_ce, 'status_code', None)
                    if _cs in (401, 403):
                        self.log.warning('cached creds rejected by venue (status_code=%s) - deriving fresh: %s', _cs, str(_ce)[:120])
                    else:
                        creds = cached
                        self.log.warning('cached creds unvalidated (status_code=%s is venue-side, not an auth rejection) - proceeding with them: %s', _cs, str(_ce)[:120])
            # R40: retry ONLY the derive round-trip, and only on a transport-level
            # failure (status_code None, or 429/5xx). A real credential rejection
            # is a 4xx and still fails on the first attempt, so a genuinely bad
            # key is not masked behind 4 pointless retries. See _SDK_AUTH_RETRIES.
            if creds is None:
                for _attempt in range(1, _SDK_AUTH_RETRIES + 1):
                    try:
                        creds = await loop.run_in_executor(None, _derive)
                        break
                    except Exception as _de:
                        _sc = getattr(_de, 'status_code', None)
                        # R41b: 400 "Could not derive api key!" is this venue's signature
                        # for its OWN auth backend being down, not for a malformed
                        # request - during the 2026-08-19 outage a freshly generated
                        # throwaway key (zero balance, no history) got the identical 400.
                        # So retry that one message; any other 400 stays fatal.
                        _dmsg = str(getattr(_de, 'error_message', '') or _de).lower()
                        _transient = _sc is None or _sc in (429, 500, 502, 503, 504) or (_sc == 400 and 'could not derive' in _dmsg)
                        if not _transient or _attempt >= _SDK_AUTH_RETRIES:
                            raise
                        _wait = _SDK_AUTH_BACKOFF_S * _attempt
                        self.log.warning('SDK auth derive attempt %d/%d failed (status_code=%s, transient) - retrying in %.1fs: %s', _attempt, _SDK_AUTH_RETRIES, _sc, _wait, str(_de)[:120])
                        await asyncio.sleep(_wait)
                if creds is None:
                    raise RuntimeError('SDK auth derive returned no credentials')
                sdk.set_api_creds(creds)
                _save_cached_creds(cache_id, creds, self.log)
            self.sdk = sdk
    ...
    async def initialize(self, session: aiohttp.ClientSession) -> bool:
        ...
        try:
            ...
            on_chain = None
            self.log.warning('Proxy lookup timed out ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â using .env value')
        if on_chain:
            if not self.cfg.proxy_address:
                self.log.info('Proxy not in .env ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â using on-chain value: %s', on_chain)
                self.cfg.proxy_address = on_chain
            elif self.cfg.proxy_address.lower() != on_chain.lower():
                self.log.warning('PROXY MISMATCH ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â .env=%s  on-chain=%s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â auto-correcting.', self.cfg.proxy_address, on_chain)
                self.cfg.proxy_address = on_chain
            else:
    ...
    async def get_balance(self) -> Optional[float]:
        ...
        if self.sdk:
            try:
                ...
                if bal is not None:
                    return bal
                self.log.warning('SDK balance payload missing/malformed balance field (type=%s keys=%s) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â falling back to REST', type(resp).__name__, list(resp.keys())[:12] if isinstance(resp, dict) else None)
            except Exception as e:
            ...
            try:
                ...
                async with self.session.get(f'{self.cfg.clob_url}{path}', headers=auth_hdrs, timeout=aiohttp.ClientTimeout(total=5)) as r:
                    if r.ok:
                        ...
                        self.log.warning('REST balance HTTP %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no authority', r.status)
            except Exception as e:
    ...
    def _parse_placement_response(resp: dict, side: 'Side') -> PlacementResult:
        ...
        if sm_raw is not None:
            ...
            if shares_micro > 0:
                # R40: these fields arrive as DECIMAL counts, not 6-decimal base
                # units. Measured 2026-08-19 on two real live FAK fills: the
                # venue's own record (data-api /activity size_matched, and CLOB
                # status=MATCHED) was 6.444443 and 5.102039 shares, and this
                # branch booked 6e-06 / 5e-06 - exactly x/1e6 rounded to 6dp. The
                # bot then held 11.5 shares it believed it did not own, which is
                # what POSITION DRIFT (UNDER) caught 2min later: HALT [drift] ->
                # orphan-state boot latch -> 99 supervisor restarts. Cost $4.4181.
                # Do NOT simply delete the divide: if the venue reverts to base
                # units, trusting them raw books 1e6x too much - that is the R35
                # 600-share incident at :2363. Decide by magnitude instead. The
                # 5-share market-buy minimum (_VENUE_MARKET_BUY_MIN_SHARES, :679)
                # makes any genuine base-unit amount >= 5_000_000, while the
                # largest clip this bot can ever request (max_order_size $25 at
                # the $0.20 min ask) is 125 shares - so 10_000 sits 80x above the
                # biggest real decimal and 500x below the smallest real base-unit
                # value. avg_px below is a RATIO of two fields from the same
                # response, so it is already unit-invariant and is left alone.
                matched = float(shares_micro / scale) if shares_micro >= 10000 else float(shares_micro)
    ...
    async def list_open_orders(self) -> List[dict]:
        if not self.sdk:
            return []
        # R42: retry transient venue failures. recover_open_orders (:10316)
        # promotes any exception here to FatalBotError, and on 2026-08-19 a
        # venue-side 500 did that 36 times, reboot-looping the bot. Only
        # transport-level failures retry (status_code None or 429/5xx); a real
        # 4xx rejection still fails on the first attempt. Exhaustion re-raises,
        # so an unreadable order book at live boot remains fatal by design.
        # Same policy and constants as the R40 auth-derive retry (:1592).
        for _attempt in range(1, _SDK_AUTH_RETRIES + 1):
            try:
                loop = asyncio.get_running_loop()
                ...
                _transient = _sc is None or _sc in (429, 500, 502, 503, 504)
                if not _transient or _attempt >= _SDK_AUTH_RETRIES:
                    self.log.warning('list_open_orders failed: %s', e)
                    raise
                _wait = _SDK_AUTH_BACKOFF_S * _attempt
                self.log.warning('list_open_orders attempt %d/%d failed (status_code=%s, transient) - retrying in %.1fs: %s', _attempt, _SDK_AUTH_RETRIES, _sc, _wait, str(e)[:120])
                await asyncio.sleep(_wait)
        return []
    ...
    async def place_order(self, token_id: str, side: str, price: float, size_usdc: float, order_type: str='GTC', neg_risk: bool=False, tick_size: float=0.01, expiration_s: float=0.0, post_only: bool=False, _tick_retry: bool=False) -> Optional[PlacementResult]:
        ...
        if side in ('BUY', Side.BUY) and (order_type or 'GTC').upper() in ('FOK', 'FAK'):
            ...
            legal = ioc_buy_request_shares(size_usdc, price, max_usdc=self.cfg.max_order_size, mkt=_mkt_ref)
            if legal <= 0.0:
        ...
        if otype_u == 'FOK':
            ...
            if ot is None:
                raise RuntimeError('OrderType.FAK unavailable in SDK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â upgrade py-clob-client-v2')
        elif wants_gtd:
            if not hasattr(OrderType, 'GTD'):
                raise RuntimeError('OrderType.GTD unavailable in SDK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refuse to post resting order without venue-backed expiry (no silent GTC downgrade)')
            ...
            if live_maker:
                raise RuntimeError('LIVE ENTRY_MODE=maker refuses bare GTC ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â set MAKER_GTD_TTL_S>=120 for venue-backed expiry')
            ot = OrderType.GTC
        if exp_ts > 0:
            try:
                args = OrderArgs(token_id=token_id, price=price, size=shares, side=sdk_side, expiration=exp_ts)
            except TypeError as e:
                raise RuntimeError('OrderArgs does not accept expiration= ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refuse GTD (upgrade py-clob-client-v2; no GTC fallback)') from e
            got_exp = int(getattr(args, 'expiration', 0) or 0)
            if got_exp != exp_ts:
                raise RuntimeError(f'OrderArgs dropped expiration (wanted {exp_ts}, got {got_exp}) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refuse to sign mismatched GTD')
        else:
            try:
...
class OrderManager:
    ...
    def __init__(self, cfg: Config, client: PolyClient, metrics: Optional[Metrics]=None):
        ...
        self._rejects = 0
        # Phase 0.4: the R46c null-slip probe was written to the log and nowhere
        # else, so the run's look-ahead was unquantified. Aggregated onto STATUS.
        # Pass: median lookahead_bps <= 0 AND frozen_sh==0 on < 5% of sim fills.
        self._dryslip_bps: Deque[float] = deque(maxlen=4000)
        self._dryslip_n: int = 0
        self._dryslip_fills: int = 0
        # r53: resting dry maker orders keyed by oid. See on_dry_trade_print().
        self._dry_resting: Dict[str, Dict[str, Any]] = {}
        self._dry_maker_tasks: Set[Any] = set()
        self._dry_maker_path: str = os.path.expanduser(os.environ.get('DRY_MAKER_LOG', '~/latarb_maker.jsonl'))
        self._dry_maker_stats: Dict[str, int] = {'posted': 0, 'filled_through': 0, 'filled_queue': 0, 'expired': 0, 'prints': 0, 'prints_same_side': 0, 'prints_no_side': 0, 'prints_seen': 0}
        self._dryslip_frozen_zero: int = 0
        ...
        # Same instant, but never advanced: _replay_rest_fill needs "did this fill
        # happen before this process existed?" and the cursor above moves (:2535).
        self._boot_ts: float = self._last_trade_cursor_ts
        self._first_reconcile_done: bool = False
        self._fill_replay_handler: Optional[Callable[[dict], Any]] = None
        self._fill_apply_error: str = ''
        # R35 FIX (OX-6): dry order ids were f'dry-{int(time.time()*1000)}'. The
        # same string is used as BOTH trade_id and order_id (:2204), and _on_fill
        # dedupes on it (:9107), so two paper fills resolving in the same
        # millisecond collided: the second _orders[oid] write overwrote the
        # first and the second position was never credited. Worse, the LatArb
        # caller at :7163 reads matched_sz back off the surviving TrackedOrder
        # and only raises it if the position delta is larger, so the bogus size
        # won and an entry was booked against shares that were never held. A
        # monotonic counter makes the id unique regardless of clock resolution.
        self._dry_oid_seq = itertools.count(1)
        ...
        # A1: WS trades seen with no extractable order id (schema-break alarm).
        self._missing_order_id_count: int = 0
        self._missing_order_id_last: float = 0.0
    ...
    def record_adverse(self, adverse_bps: float, strategy: str='directional') -> float:
        """Fold one adverse-selection observation into the per-strategy EWMA.

        Returns the WINSORISED observation, so callers that also log or persist
        the value record the same number the gate is fed (R35 FIX F3).
        """
        ...
        #
        # R35 NOTE (F4), on what this does and does NOT achieve: the first
        # observation for a key seeds the EWMA at FULL magnitude (no blending
        # toward 0, see the `prev is None` branch below), and validate() at
        # :569-570 REQUIRES adverse_ewma_clamp_bps >= max_adverse_bps. Those two
        # facts together mean a single winsorised print still lands above the cap
        # and still shuts the gate - the clamp bounds how far above, not whether.
        # Recovery comes from R22's half-life decay in adverse_ewma() instead:
        # 120bps decays under a 40bps cap after halflife*log2(120/40) ~ 475s at
        # the default 300s. With LatArb's observed cadence (6 fills in 8.7h) very
        # nearly every observation is a 'first' one, so in practice the gate is
        # driven by single prints on an ~8-minute timer, not by a blended average.
        # Deliberately NOT 'fixed' by seeding at a fraction: that loosens a
        # protective gate, and the measured signal has negative expectancy
        # (-0.54/share over the 2026-08-03 replay), so admitting more fills after
        # a toxic print would manufacture more of exactly the losing trades. Any
        # change here should follow evidence that the signal is profitable.
    ...
    def log_missing_order_id(self, trade_id: str, token_id: str, payload_keys: List[str]) -> None:
        """A1: a WS trade arrived with no extractable order id.

        This is a schema-coupling alarm, not a routine event.  IOC dedup keys off
        order ids (Bot._on_fill), so if the venue renames one of the fields
        _extract_trade_order_ids knows about, this fires instead of silently
        double-crediting fills.  Logged with the payload keys so the new field
        name is visible directly in the log, and counted so a persistent schema
        break shows up in the status line rather than scrolling past.
        Throttled to once per 60s: WS message rate would otherwise flood.
        """
        self._missing_order_id_count += 1
        now = time.monotonic()
        if now - self._missing_order_id_last < 60.0:
            return
        self._missing_order_id_last = now
        self.log.critical('WS_NO_ORDER_ID: trade %s on %s carries no order id (n=%d) - IOC dedup key unavailable, deferring to REST reconciliation. Payload keys: %s. If this persists the venue trade schema changed; update _extract_trade_order_ids.', str(trade_id)[:16], str(token_id)[:12], self._missing_order_id_count, ','.join(payload_keys[:24]))

    @property
    def missing_order_id_count(self) -> int:
        return self._missing_order_id_count
    ...
    def spawn_fill_probe(self, token_id: str, side: Side, fill_price: float, fill_size: float=0.0, trade_id: str='') -> None:
        ...
        async def _probe() -> None:
            try:
                ...
                # R35 FIX (F3): adverse_bps (:2071) is the RAW pre-clamp value.
                # record_adverse only clamped its LOCAL copy (now returns it), but
                # the shadow sink and log line used the raw, so calibration_report
                # averaged +618.6bps and the mean flipped from 0 to 41.42bps,
                # spiking the go/no-go gate's `mean_adverse_bps <= MAX_ADVERSE_BPS`
                # at :10084. Use the returned winsorised value.
                clamped_bps = self.record_adverse(adverse_bps, strategy=strat_key)
                self.log.info('SHADOW_FILL %s %s @ %.4f sz=%.4f tid=%s | entry_mid=%.4f post_mid=%.4f spread=%.4f adverse=%+.1fbps ewma=%+.1fbps strat=%s', side.value, token_id[:12], fill_price, fill_size, (trade_id or '-')[:16], entry_mid, post_mid, entry_spread, clamped_bps, self.adverse_ewma(strat_key) or 0.0, strat_key)
                if self._shadow_sink is not None:
                    try:
                        self._shadow_sink({'market_id': mkt.market_id, 'token_id': token_id, 'side': side.value, 'fill_price': fill_price, 'fill_size': fill_size, 'trade_id': trade_id, 'fill_ts_mono': fill_ts, 'adverse_bps': clamped_bps, 'strategy': strat_key})
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    async def _apply_dry_fill(self, oid: str, token_id: str, side: Side, fill_px: float, shares: float, ioc_aggregate: bool) -> None:
        """One application path for every simulated fill, immediate or deferred.

        r53: extracted so the deferred maker path and the FOK/FAK taker path cannot
        drift apart. Position, ledger and grading all hang off _fill_replay_handler,
        so routing through here is what makes latarb_fills/latarb_settle record the
        maker fills too -- that is where resolution PnL comes from.
        """
        if self._fill_replay_handler is None:
            return
        try:
            payload = {'asset_id': token_id, 'side': side.value, 'price': str(fill_px), 'size': str(shares), 'trade_id': oid, 'order_id': oid, '_ioc_aggregate': ioc_aggregate}
    ...
    def _dry_mid(self, token_id: str) -> Optional[float]:
        mkt = self.client._token_to_market.get(token_id) if self.client is not None else None
        if mkt is None:
            return None
        bk = mkt.book_yes if token_id == mkt.yes_token else mkt.book_no
        return None if bk is None else bk.mid

    def on_dry_trade_print(self, token_id: str, px: float, sz: float, taker_side: str='') -> None:
        """Resolve resting dry maker orders against a real CLOB trade print.

        A resting BUY at post_px fills ONLY when the tape shows the market actually
        reaching it. Two mechanical cases, no RNG:

          through       -- a print strictly BELOW post_px proves the sweep reached
                           our level. r54: it must still clear queue_ahead + want.
                           A 5.05-share print does not fill 10.64 of our shares
                           behind a 40-share queue just because it printed lower.
          queue_cleared -- a print AT post_px only fills us once cumulative printed
                           size at that level exceeds the size already resting there
                           when we posted (queue_ahead). Back of the queue is the
                           conservative assumption: we are last in line.

        r53c: BOTH cases additionally require the print to be on the OPPOSITE taker
        side. last_trade_price.side is the taker's side -- verified live, 68.4% of
        side=BUY prints land at/above the ask and 70.4% of side=SELL prints at/below
        the bid (n=1367). A side=BUY print lifts the ask and cannot touch our resting
        bid, so counting it toward our queue would manufacture fills. A print with no
        side does not fill either; it lands in prints_no_side so the omission is
        visible instead of silently zeroing every fill.

        ponytail: needs the FULL want_shares to clear before booking a fill -- no
        partial-fill plumbing. That under-counts fills, the safe direction for a
        measurement whose entire purpose is to not flatter the strategy. Add
        partials only if fill count, not fill honesty, becomes the binding limit.
        """
        # counted BEFORE the early return: with nothing resting this is the only
        # evidence in the log that the feed reaches the sink at all.
        self._dry_maker_stats['prints_seen'] += 1
        if not self._dry_resting:
            return
        for oid, r in list(self._dry_resting.items()):
            if r['token'] != token_id or r['fill_mono'] is not None:
                continue
            r['prints'] += 1
            self._dry_maker_stats['prints'] += 1
            post_px = r['post_px']
            is_buy = r['side'] == Side.BUY.value
            ts_u = str(taker_side or '').strip().upper()
            if ts_u not in ('BUY', 'SELL'):
                self._dry_maker_stats['prints_no_side'] += 1
                continue
            if (ts_u == 'BUY') == is_buy:
                # taker on OUR side: it consumed the other side of the book, not us.
                self._dry_maker_stats['prints_same_side'] += 1
                continue
            reason = None
            _through = (px < post_px - 1e-09) if is_buy else (px > post_px + 1e-09)
            if _through or abs(px - post_px) <= 1e-09:
                # r54: a print past our level proves the sweep reached us, but it
                # can only consume the size it actually printed. Both cases must
                # clear queue_ahead + want; only the recorded reason differs.
                r['cum_at_level'] += max(0.0, sz)
                if r['cum_at_level'] > r['queue_ahead'] + r['want'] - 1e-09:
                    reason = 'through' if _through else 'queue_cleared'
            r['prints_opp'] = r.get('prints_opp', 0) + 1
            if reason is None:
                continue
            r['fill_mono'] = time.monotonic()
            r['fill_ts'] = time.time()
            r['fill_px'] = post_px
            r['fill_shares'] = r['want']
            r['fill_reason'] = reason
            self._dry_maker_stats['filled_through' if reason == 'through' else 'filled_queue'] += 1
            self.log.info('DRYMAKER FILL %s %s reason=%s taker=%s print=%.4f sz=%.2f post=%.4f sh=%.2f qahead=%.2f cum=%.2f rested=%.1fs', token_id[:12], r['side'], reason, ts_u, px, sz, post_px, r['fill_shares'], r['queue_ahead'], r['cum_at_level'], r['fill_mono'] - r['post_mono'])
            try:
                t = asyncio.get_running_loop().create_task(self._book_dry_maker_fill(oid))
                self._dry_maker_tasks.add(t)
                t.add_done_callback(self._dry_maker_tasks.discard)
            except RuntimeError:
                self.log.warning('DRYMAKER FILL %s: no running loop, fill NOT booked', oid)

    async def _book_dry_maker_fill(self, oid: str) -> None:
        r = self._dry_resting.get(oid)
        if r is None:
            return
        tracked = self._orders.get(oid)
        if tracked is not None:
            tracked.state = OrderState.FILLED
            tracked.filled_size = float(r['fill_shares'])
            tracked.avg_fill_price = float(r['fill_px'])
        await self._apply_dry_fill(oid, r['token'], Side(r['side']), float(r['fill_px']), float(r['fill_shares']), False)

    async def _release_dry_token(self, oid: str, r: Dict[str, Any]) -> None:
        """Hand the (token, side) slot back to DUP_GUARD when an order dies unfilled."""
        tracked = self._orders.get(oid)
        if tracked is not None:
            tracked.state = OrderState.CANCELLED
        async with self._lock:
            k = (r['token'], Side(r['side']))
            if self._by_token.get(k) == oid:
                self._by_token.pop(k, None)

    def _write_dry_maker_row(self, oid: str, r: Dict[str, Any]) -> None:
        """One terminal row per resting order. Resolution PnL is NOT duplicated here:
        join market_id against latarb_settle.jsonl for that."""
        fpx = r['fill_px']
        # sign the markout so NEGATIVE always means adverse selection: a long
        # wants mid to rise after the fill, a short wants it to fall.
        _dir = 1.0 if r['side'] == Side.BUY.value else -1.0
        row = {'oid': oid, 'ts': r['post_ts'], 'coin': r['coin'], 'market_id': r['market_id'], 'token': r['token'], 'side': r['side'], 'strategy': r['strategy'], 'post_px': r['post_px'], 'size_usdc': r['size_usdc'], 'want_shares': r['want'], 'queue_ahead': r['queue_ahead'], 'cum_at_level': r['cum_at_level'], 'prints': r['prints'], 'prints_opposite_side': r.get('prints_opp', 0), 'mid_at_post': r['mid_at_post'], 'best_ask_at_post': r['best_ask_at_post'], 'ttl_s': float(self.cfg.maker_gtd_ttl_s), 'outcome': r['fill_reason'], 'fill_ts': r.get('fill_ts'), 'fill_px': fpx, 'fill_shares': r['fill_shares'], 'rest_s': None if r['fill_mono'] is None else round(r['fill_mono'] - r['post_mono'], 3), 'markout_30s_mid': r['mk30'], 'markout_120s_mid': r['mk120'], 'markout_30s_cents': None if r['mk30'] is None or fpx is None else round((r['mk30'] - fpx) * _dir * 100.0, 4), 'markout_120s_cents': None if r['mk120'] is None or fpx is None else round((r['mk120'] - fpx) * _dir * 100.0, 4)}
        try:
            with open(self._dry_maker_path, 'a', encoding='utf-8') as fh:
                fh.write(json.dumps(row, default=str) + '\n')
        except Exception as e:
            self.log.warning('dry maker row write failed: %s', e)

    async def dry_maker_loop(self) -> None:
        """1 Hz: expire TTL-passed resting orders, sample +30s/+120s markouts.

        Markout is signed against our own fill: mid(t) - fill_px, in cents. Negative
        means the market moved away after we were hit -- adverse selection, the exact
        cost term a coin-flip fill simulator cannot see. A 300s market can resolve
        before +120s; that samples as null and MUST be reported null, not imputed.
        """
        _ticks = 0
        while True:
            try:
                ...
                _ticks += 1
                if _ticks % 60 == 0:
                    st = self._dry_maker_stats
                    self.log.info('DRYMAKER stats resting=%d seen=%d matched=%d same_side=%d no_side=%d posted=%d fill_through=%d fill_queue=%d expired=%d', len(self._dry_resting), st['prints_seen'], st['prints'], st['prints_same_side'], st['prints_no_side'], st['posted'], st['filled_through'], st['filled_queue'], st['expired'])
                for oid, r in list(self._dry_resting.items()):
                    if r['fill_mono'] is None:
                        if now >= r['expiry_mono']:
                            r['fill_reason'] = 'expired_no_print'
                            self._dry_maker_stats['expired'] += 1
                            self.log.info('DRYMAKER EXPIRE %s post=%.4f rested=%.0fs prints=%d - no qualifying print', r['token'][:12], r['post_px'], now - r['post_mono'], r['prints'])
                            self._write_dry_maker_row(oid, r)
                            self._dry_resting.pop(oid, None)
                            await self._release_dry_token(oid, r)
                        continue
                    age = now - r['fill_mono']
                    if r['mk30'] is None and age >= 30.0:
                        r['mk30'] = self._dry_mid(r['token'])
                    if r['mk120'] is None and age >= 120.0:
                        r['mk120'] = self._dry_mid(r['token'])
                    if age >= 120.0:
                        self._write_dry_maker_row(oid, r)
                        self._dry_resting.pop(oid, None)
                ...
                self.log.warning('dry_maker_loop: %s', e)
        ...
        async with self._lock:
            ...
            if existing_oid is not None:
                if not isinstance(existing_oid, str):
                    self.log.warning('DUP_GUARD: refusing %s %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â submission already pending', side.value, token_id[:12])
                    return None
                existing = self._orders.get(existing_oid)
                if existing is not None and existing.state in (OrderState.PENDING, OrderState.OPEN):
                    self.log.warning('DUP_GUARD: refusing %s %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â existing %s still %s', side.value, token_id[:12], existing_oid[:8], existing.state.value)
                    return None
        ...
        try:
            if side == Side.BUY:
                ...
                if mkt_ref is not None:
                    ...
                    if pos.shares > 1e-06:
                        self.log.debug('STP: skipping BUY %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â already hold %.6f shares', token_id[:12], pos.shares)
                        return None
            if self.cfg.dry_run:
                # R46c (Critic 2g-D): freeze the ask ladder BEFORE the simulated
                # latency sleep. The walk further down runs on the LIVE ladder,
                # i.e. the simulator is handed dry_run_latency_ms of hindsight for
                # free. Walking both with the same req_shares measures that
                # look-ahead directly instead of assuming it away. OrderBook.asks
                # rebinds a fresh list of tuples in _ensure_sorted, so list() is a
                # genuine snapshot, not an alias.
                _pre_mkt = self.client._token_to_market.get(token_id)
                _pre_book = None if _pre_mkt is None else (_pre_mkt.book_yes if token_id == _pre_mkt.yes_token else _pre_mkt.book_no)
                frozen_asks = list(_pre_book.asks or []) if _pre_book is not None and hasattr(_pre_book, 'asks') else None
                ...
                if otype_u_dry not in ('FOK', 'FAK') and (str(getattr(self.cfg, 'entry_mode', '')).lower() == 'maker'):
                    # r53: a maker post does NOT fill at submission. Rest it and let
                    # on_dry_trade_print() resolve it against the real tape. The old
                    # else-arm below filled every GTC on a dry_run_fill_prob coin
                    # flip, full size, at our own limit price -- no queue and no
                    # adverse selection, which for a passive strategy is the entire
                    # cost term. DRYSLIP only ever guarded the FOK/FAK branch, so
                    # nothing in the build would have caught it.
                    _rest_oid = f'dry-{int(time.time() * 1000)}-{next(self._dry_oid_seq)}'
                    _want = round(size / max(price, 0.001), 6)
                    _queue_ahead = 0.0
                    _mid_post = None
                    _ask_post = None
                    if book_ref is not None:
                        _mid_post = book_ref.mid
                        _ask_post = book_ref.best_ask
                        # queue ahead of us is the size already resting on OUR side
                        # of the book at OUR price. Back of that queue is the
                        # conservative assumption.
                        for _bpx, _bsz in (book_ref.bids if side == Side.BUY else book_ref.asks) or []:
                            if abs(_bpx - price) <= 1e-09:
                                _queue_ahead = float(_bsz)
                                break
                    async with self._lock:
                        self._orders[_rest_oid] = TrackedOrder(_rest_oid, token_id, side, price, size, strategy, state=OrderState.OPEN)
                        self._by_token[key] = _rest_oid
                        committed = True
                    _now_mono = time.monotonic()
                    self._dry_resting[_rest_oid] = {'token': token_id, 'side': side.value, 'post_px': float(price), 'want': _want, 'queue_ahead': _queue_ahead, 'cum_at_level': 0.0, 'prints': 0, 'post_mono': _now_mono, 'post_ts': time.time(), 'expiry_mono': _now_mono + max(1.0, float(self.cfg.maker_gtd_ttl_s)), 'strategy': strategy.value, 'size_usdc': float(size), 'market_id': None if mkt_ref is None else str(getattr(mkt_ref, 'market_id', '') or ''), 'coin': None if mkt_ref is None else getattr(mkt_ref, 'coin', None), 'mid_at_post': _mid_post, 'best_ask_at_post': _ask_post, 'fill_mono': None, 'fill_ts': None, 'fill_px': None, 'fill_shares': 0.0, 'fill_reason': None, 'mk30': None, 'mk120': None}
                    self._dry_maker_stats['posted'] += 1
                    self.log.info('DRYMAKER POST %s %s %s post=%.4f want=%.2fsh qahead=%.2fsh ttl=%.0fs mid=%s ask=%s', token_id[:12], side.value, strategy.value, price, _want, _queue_ahead, float(self.cfg.maker_gtd_ttl_s), 'n/a' if _mid_post is None else '%.4f' % _mid_post, 'n/a' if _ask_post is None else '%.4f' % _ask_post)
                    return _rest_oid
                if otype_u_dry in ('FOK', 'FAK') and side == Side.BUY:
                    # P1: match live FAK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â request shares = USDC/limit, depth-walk only asks <= limit.
                    ...
                    req_shares = ioc_buy_request_shares(size, price, max_usdc=self.cfg.max_order_size, mkt=mkt_ref)
                    if req_shares <= 0.0:
                    ...
                    if book_ref is not None and hasattr(book_ref, 'asks'):
                        got, cost, live_lv = _walk_asks_limited(list(book_ref.asks or []), price, req_shares)
                        # R46c (2g-D) null-slip probe. frozen_* = the fill this
                        # order could actually have got at submission time; live_*
                        # = what the simulator books. lookahead_bps > 0 means the
                        # sim was handed a cheaper book than the order could hit,
                        # so paper P&L is optimistic by that much. frozen_sh=0
                        # with live_sh>0 is the strong case: no fill was available
                        # at all. Left as a raw 0 rather than an infinity sentinel
                        # so it cannot silently pollute the bps distribution -
                        # score those rows as a separate count.
                        f_got, f_cost, f_lv = _walk_asks_limited(frozen_asks, price, req_shares)
                        _lv_vwap = cost / got if got > 1e-12 else 0.0
                        _fz_vwap = f_cost / f_got if f_got > 1e-12 else 0.0
                        _la_bps = (_fz_vwap - _lv_vwap) / _lv_vwap * 10000.0 if _lv_vwap > 0 and _fz_vwap > 0 else 0.0
                        self.log.info('DRYSLIP %s limit=%.4f live_vwap=%.4f frozen_vwap=%.4f lookahead_bps=%.1f live_lv=%d frozen_lv=%d live_sh=%.4f frozen_sh=%.4f req_sh=%.4f lat_ms=%.0f', token_id[:12], price, _lv_vwap, _fz_vwap, _la_bps, live_lv, f_lv, got, f_got, req_shares, float(self.cfg.dry_run_latency_ms))
                        self._dryslip_n += 1
                        if got > 1e-12:
                            self._dryslip_fills += 1
                            if f_got <= 1e-12:
                                self._dryslip_frozen_zero += 1
                            elif _lv_vwap > 0 and _fz_vwap > 0:
                                self._dryslip_bps.append(_la_bps)
                        if got > 1e-12 and cost > 0:
                            matched_shares = round(got, 6)
                            dry_fill_price = _lv_vwap
                ...
                oid = f'dry-{int(time.time() * 1000)}-{next(self._dry_oid_seq)}' if filled else None
                ...
                async with self._lock:
                    ...
                    committed = True
                await self._apply_dry_fill(oid, token_id, side, dry_fill_price, shares, otype_u_dry in ('FOK', 'FAK'))
                return oid
            otype_u = (otype or 'GTC').upper()
            # LatArb FOKs/FAKs are rare and time-critical ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â skip shared rate limiter.
            if otype_u not in ('FOK', 'FAK'):
                await self._rl.acquire()
            else:
                # N8 FIX: bounded fast path Ã¢â‚¬â€ normal signal cadence (max 5
                ...
                if sum(1 for _t in self._ioc_send_ts if _now_mono - _t <= 1.0) > self._ioc_burst_cap:
                    self.log.warning('IOC burst > %d/s Ã¢â‚¬â€ falling back to shared rate limiter', self._ioc_burst_cap)
                    await self._rl.acquire()
            ...
            if exec_now and matched > 0.0 and self._fill_replay_handler is not None:
                ...
                # R40: the bound was one-sided, so it only ever caught a size too
                # BIG. A matched_size of 5e-06 against an expected 5.10 sailed
                # through and booked a 5e-06 position against a real 5.102039
                # share fill (see :1604). Both directions are the same defect -
                # the venue's number is not in the unit we assumed - and both
                # have the same correct answer: do not credit it, go read the
                # venue's authoritative trade record. A genuine partial fill
                # under 0.1% of the ask is worth fractions of a cent, so
                # deferring it to reconcile costs nothing.
                if shares_fill > max(expected_sh * 2.0, expected_sh + 1.0) or shares_fill < expected_sh * 1e-3:
                    self.log.error('%s matched_size implausible oid=%s matched=%.9f expected~%.6f ? refusing immediate credit; REST reconcile only', otype_u, oid[:12], shares_fill, expected_sh)
                    shares_fill = 0.0
                    # R35 FIX: zeroing only the local left the absurd value in
                    # TrackedOrder.filled_size, which :2280 had already written
                    # from the same unvalidated `matched`. So the guard covered
                    # the fill-replay path and nothing else: :2325 reads
                    # filled_size back as final_matched, place() still returns
                    # oid, and the LatArb caller at :7163 SEEDS matched_sz from
                    # filled_size, overriding it only when the measured position
                    # delta is LARGER. The bogus size therefore won and a
                    # 600-share entry was booked against a zero position. Clear
                    # the tracked order too, and actually perform the REST
                    # reconcile this message promises - the venue's own
                    # authoritative record is what should set the real number.
                    async with self._lock:
                        _t = self._orders.get(oid)
                        if _t is not None:
                            _t.filled_size = 0.0
                            _t.avg_fill_price = 0.0
                    if is_ioc:
                        try:
                            await self.reconcile_fills(wait_if_busy=True)
                        except Exception as e:
                            self.latch_fill_failure(f'absurd matched_size reconciliation failed for {oid}: {e}')
                        async with self._lock:
                            _t = self._orders.get(oid)
                            matched = float(_t.filled_size) if _t is not None else 0.0
                            avg_px = float(_t.avg_fill_price) if _t is not None else 0.0
            ...
            # FAK/FOK: order acceptance ÃƒÂ¢Ã¢â‚¬Â°  fill. Zero matched_size is a miss (Polymarket FAK = fill-and-kill remainder).
            if is_ioc:
                async with self._lock:
                    tracked = self._orders.get(oid)
    ...
    async def reconcile(self) -> int:
        ...
        async with self._lock:
            for oid, tracked in list(self._orders.items()):
                if oid in live_index:
                    ...
                    # R35 FIX (OX-3): pop the (token, side) slot only if it still
                    # points at THIS order. _by_token maps a key to either an oid
                    # or a place() reservation object (:2113), and place() holds
                    # the reservation across its network call - so a key can very
                    # easily belong to a NEWER order by the time reconcile prunes
                    # an older one for the same token+side. Popping it
                    # unconditionally destroys that newer order's DUP_GUARD slot
                    # (:2104-2112): the guard then reads None and admits a second
                    # concurrent order on a token that already has one in flight.
                    # The identity-guarded form is already the established pattern
                    # in this class (:2365 and _clear_if_ours at :2118); these
                    # three sites were the ones that never adopted it. Not popping
                    # leaks nothing - whoever owns the key clears it themselves.
                    if tracked.state == OrderState.FILLED:
                        if self._by_token.get((tracked.token_id, tracked.side)) == oid:
        ...
        async with self._lock:
            if self._rejects > 0:
                self.log.info('Reconcile OK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â clearing stale reject count (%d)', self._rejects)
                self._rejects = 0
        return pruned
    ...
    async def reconcile_fills(self, wait_if_busy: bool = False) -> int:
        ...
        async with self._reconcile_fills_lock:
            ...
            replayed = 0
            max_ts = since_ts
            # M3 FIX: _extract_trade_ts returns 0.0 for an unparseable stamp, and
            # 0.0 sorts FIRST - which would replay an unstamped SELL ahead of the
            # BUY that created its position.  _on_fill would then see pos.shares==0,
            # silently drop the SELL, and mark_trade_seen would make that drop
            # permanent.  Unstamped rows sort LAST instead: applying a fill late is
            # recoverable, applying it out of order against an empty position is not.
            def _sort_key(tr: dict) -> Tuple[float, str]:
                ts = self._extract_trade_ts(tr)
                return (ts if ts > 0 else float('inf'), self._extract_trade_id(tr))
            ordered = sorted((tr for tr in trades if isinstance(tr, dict)), key=_sort_key)
            for tr in ordered:
    ...
    async def _fetch_trades_since(self, since_ts: float) -> List[dict]:
        ...
        def _try() -> List[dict]:
            ...
            if _get is None:
                # C3 FIX (residual): never degrade silently Ã¢â‚¬â€ REST fill
                ...
                if not getattr(self, '_warned_no_get_trades', False):
                    ...
                    self.log.critical('SDK has no get_trades Ã¢â‚¬â€ REST fill reconciliation is DISABLED on this build; WS is the only fill source')
                return []
            try:
    ...
    def _extract_trade_ts(tr: dict) -> float:
        for key in ('match_time', 'timestamp', 'matchTime', 'ts'):
            ...
            if not math.isfinite(f) or f <= 0:
                continue
            # M3 FIX: try the four plausible epoch units explicitly and accept the
            # one that lands in the seconds window (1e9..4e9 == 2001..2096).
            #
            # The old code divided by 1000 in a `while f > 1e11` loop and rejected
            # anything over 1e18.  Two problems: nanosecond stamps were rejected
            # outright (returning 0.0), and a repeated-division loop will walk ANY
            # large number into the plausible window - 1e30 came out as exactly
            # 1e9, i.e. garbage accepted as a valid 2001 timestamp.  Fixed scales
            # cannot do that: a value is either a recognised unit or it is junk.
            #
            # 0.0 is not inert downstream: reconcile_fills sorts on this value, so
            # a stamp that silently became 0.0 used to sort a SELL ahead of the BUY
            # that created its position.
            for scale in (1.0, 1000.0, 1000000.0, 1000000000.0):
                secs = f / scale
                if 1000000000.0 <= secs <= 4000000000.0:
                    return secs
        return 0.0
    ...
    async def cancel(self, oid: str) -> bool:
        ...
        if ok:
            async with self._lock:
                ...
                if tracked:
                    # R35 FIX (OX-3): see reconcile() - only release the slot if it
                    # is still ours. cancel_open_buys (:2616) walks a snapshot list
                    # and awaits a venue round-trip per oid, so a replacement order
                    # for the same token+side can own the key before this returns.
                    if self._by_token.get((tracked.token_id, tracked.side)) == oid:
                        self._by_token.pop((tracked.token_id, tracked.side), None)
        return ok
    ...
    async def cancel_all(self) -> None:
        ...
        try:
            ...
            self.log.error('cancel_all: exchange call failed (%s) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â %d orders may remain open: %s', e, snapshot_count, ', '.join((oid[:8] for oid in snapshot_ids[:5])))
            raise
        async with self._lock:
            self._orders.clear()
    ...
    def find_open(self, token_id: str, side: Side) -> Optional[TrackedOrder]:
        ...
        if not isinstance(oid, str) or not oid:
            return None
        _t = self._orders.get(oid)
        # R35 FIX (OX-7): return only a WORKING order. Every one of the eight
        # callers treats a hit as "an order is still in flight" - :4786 keeps
        # _shares_in_flight reserved, :4800/:4973 cancel it, :4980 and :6944
        # refuse re-entry - so handing back a terminal FILLED order wedges all of
        # them. It reached them two ways. In DRY_RUN a dry order is BORN
        # OrderState.FILLED (:2209) and reconcile() at :2420 is the only pruner
        # of FILLED orders, but the _reconcile_loop gates it behind
        # `if not self.cfg.dry_run` (:9737) - so in paper mode nothing ever
        # pruned it and `if find_open(...BUY): return` at :4980 refused every
        # subsequent entry on that token for the life of the process. Live is
        # exposed too, just briefly: a WS fill marks FILLED (:2287) and the slot
        # stays until the next REST reconcile, up to 30s later (see :9398).
        # Filtering on state fixes both, and is the predicate `count` (:2657)
        # already uses for exactly this question.
        if _t is None or _t.state not in (OrderState.PENDING, OrderState.OPEN):
            return None
        return _t
    ...
    def remove(self, oid: str) -> None:
        tracked = self._orders.pop(oid, None)
        if tracked:
            # R35 FIX (OX-3): see reconcile() - only release the slot if it is
            # still ours, never a successor's.
            if self._by_token.get((tracked.token_id, tracked.side)) == oid:
                self._by_token.pop((tracked.token_id, tracked.side), None)
...
class ChainlinkFeed:
    """Polymarket RTDS Chainlink prices ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â settlement oracle for 5m/15m crypto markets."""
    ...
    def __init__(self, coins: List[str]):
        ...
        self._last_msg: float = 0.0
        self._ws: Optional[Any] = None
        # R43 instrumentation only: TWAP60 mirror of the settlement series.
        # Never read by any gate, sizing or model path - see _note_twap().
        self._twap60: Dict[str, float] = {}
        self._twap_log_ts: Dict[str, float] = {}
    ...
    def _note_twap(self, d: Dict[str, Any], now_mono: float) -> bool:
        """Park a crypto_prices_twap_sixty tick and log its delta vs chainlink.

        Returns True if the message belonged to the TWAP topic (caller skips it).
        Measurement only: self._twap60 is never read by price()/the model.
        The payload field names are INFERRED from the chainlink payload shape,
        not confirmed against a real TWAP message, so every lookup tolerates a
        miss and a malformed tick is dropped silently.
        """
        if str(d.get('topic') or '') != 'crypto_prices_twap_sixty':
            return False
        payload = d.get('payload') or d
        if not isinstance(payload, dict):
            return True
        coin = self._coin_from_symbol(str(payload.get('symbol') or payload.get('Symbol') or ''))
        if not coin:
            return True
        try:
        ...
        if not math.isfinite(p) or p <= 0:
            return True
        self._twap60[coin] = p
        cl = self._prices.get(coin)
        last = self._twap_log_ts.get(coin)
        if cl and (last is None or now_mono - last >= 30.0):
            self._twap_log_ts[coin] = now_mono
            self.log.info('TWAPDELTA coin=%s chainlink=%.6f twap60=%.6f delta=%+.6f bps=%+.4f', coin, cl, p, p - cl, (p - cl) / cl * 10000.0)
        return True

    async def run(self) -> None:
        self._running = True
        sub = {'action': 'subscribe', 'subscriptions': [{'topic': 'crypto_prices_chainlink', 'type': '*', 'filters': ''}, {'topic': 'crypto_prices_twap_sixty', 'type': '*', 'filters': ''}]}
        backoff = 1.0
        while self._running:
            try:
...
def _ws_exchange_ts_ms(msg: Any) -> Optional[int]:
    ...
    for k in ('timestamp', 'timestamp_ms', 'ts', 'time', 'T', 't'):
        ...
        if not math.isfinite(v) or v <= 0:
            continue
        # Seconds epoch ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ ms
        if v < 1e12:
            v *= 1000.0
        return int(v)
    return None
...
class HyperPolyFeed:
    ...
    def __init__(self, shard_count: int=2) -> None:
        ...
        self._rate_baseline: Dict[int, Any] = {}
        # R51: last per-shard msgs/s as computed by the R50 watchdog, published
        # here so the LatArb entry path can read it WITHOUT calling
        # shard_msg_rates(), which resets _rate_baseline and would destroy the
        # watchdog window. Empty until the first sample lands (~60s after boot).
        self.last_shard_rates: Dict[int, float] = {}
        ...
        # r53: last_trade_price fan-out. OrderManager uses it to resolve resting
        # dry maker orders against the real tape instead of a coin flip.
        self._trade_sink: Optional[Callable[[str, float, float, str], Any]] = None
    ...
    def set_trade_sink(self, sink: Callable[[str, float, float, str], Any]) -> None:
        self._trade_sink = sink
    ...
    def shard_msg_rates(self) -> Dict[int, float]:
        """Per-shard inbound msgs/s since the previous call. A shard is omitted
        until it has a >=20s baseline, so the first call after boot returns {}."""
        now = time.monotonic()
        out: Dict[int, float] = {}
        for sid, toks in self._shard_tokens().items():
            if not toks:
                continue
            n = self._msg_counts.get(sid, 0)
            prev = self._rate_baseline.get(sid)
            self._rate_baseline[sid] = (now, n)
            if prev is not None and now - prev[0] >= 20.0:
                out[sid] = (n - prev[1]) / (now - prev[0])
        return out

    def shard_stalled(self, tid: str) -> Optional[float]:
        """R51: published msgs/s for the shard owning `tid`, if it is under the
        R50 floor. None means do not block - either healthy, not yet measured,
        or the floor is disabled.

        Deliberately reads last_shard_rates instead of calling
        shard_msg_rates(), which is destructive (it resets _rate_baseline).
        The published value is at most one watchdog interval old; the stalls it
        exists to catch ran 24 to 248 minutes, so that lag is immaterial.
        """
        if _WS_MIN_MSG_RATE <= 0:
            return None
        rate = self.last_shard_rates.get(self._deterministic_shard(tid))
        if rate is None or rate >= _WS_MIN_MSG_RATE:
            return None
        return rate
    ...
    async def _run_shard(self, shard_id: int, tokens: List[str]) -> None:
        ...
        while self._running:
            ...
            for tid in live_tokens:
                self._snapshot_received.discard(tid)
            try:
                async with websockets.connect(self.WS_URL, ping_interval=10, ping_timeout=5, max_size=16 * 1024 * 1024, compression=None, max_queue=_WS_MAX_QUEUE) as ws:
                    ...
                    self._msg_counts.setdefault(shard_id, 0)
                    self.log.info('Shard %d: subscribed %d tokens', shard_id, len(live_tokens))
                    # R46d (Critic 2g-F step 1): per-shard reader telemetry. The
                    # Assembler is the object that decides to stop reading the
                    # socket, so ask IT rather than inferring from CPU: paused is
                    # True exactly while the library has the transport parked,
                    # which is the definition of us being the slow consumer.
                    # qmax=-1 means the attribute was absent (library version
                    # changed) - a distinct value from 0, so a broken probe can
                    # never be misread as an empty queue.
                    _asm = getattr(ws, 'recv_messages', None)
                    _q = getattr(_asm, 'frames', None)
                    _rl_t0 = _rl_prev = time.monotonic()
                    _rl_busy = _rl_worst = _rl_gap = 0.0
                    _rl_n = _rl_pauses = 0
                    _rl_qmax = -1
                    async for msg in ws:
                        if not self._running:
                            break
                        _rl_recv = time.monotonic()
                        self._last_msgs[shard_id] = _rl_recv
                        self._msg_counts[shard_id] += 1
                        _rl_gap = max(_rl_gap, _rl_recv - _rl_prev)
                        await self._handle(msg)
                        _rl_prev = time.monotonic()
                        _rl_busy += _rl_prev - _rl_recv
                        _rl_worst = max(_rl_worst, _rl_prev - _rl_recv)
                        _rl_n += 1
                        if _q is not None:
                            _rl_qmax = max(_rl_qmax, len(_q))
                            if _asm.paused:
                                _rl_pauses += 1
                        if _WS_DUTY_LOG_S > 0 and _rl_prev - _rl_t0 >= _WS_DUTY_LOG_S:
                            _rl_span = _rl_prev - _rl_t0
                            self.log.info('WSDUTY shard=%d msgs=%d span=%.1fs duty=%.4f mean_handle_ms=%.3f worst_handle_ms=%.0f worst_gap_ms=%.0f qmax=%d qhigh=%s paused_obs=%d', shard_id, _rl_n, _rl_span, _rl_busy / _rl_span, _rl_busy / _rl_n * 1000.0, _rl_worst * 1000.0, _rl_gap * 1000.0, _rl_qmax, getattr(_asm, 'high', None), _rl_pauses)
                            _rl_t0 = _rl_prev
                            _rl_busy = _rl_worst = _rl_gap = 0.0
                            _rl_n = _rl_pauses = 0
                            _rl_qmax = -1
                ...
                if self._running:
                    self.log.warning('Shard %d WS error: %s (retry %.0fs) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â books not ready until resnapshot', shard_id, e, backoff)
                    await asyncio.sleep(backoff * 0.5 + random.uniform(0.0, backoff * 0.5))
                    backoff = min(backoff * 2, 30)
        live_tokens = self._shard_tokens().get(shard_id, tokens)
        self.mark_shard_not_ready(live_tokens)
            ...
            try:
                ...
                if et == 'price_change':
                    ...
                    for c in m.get('price_changes', m.get('changes', [])):
                        ...
                        if bk.is_crossed:
                            ...
                            self.log.debug('crossed book %s after delta ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â awaiting snapshot', c_tid[:10])
                        else:
                    ...
                    for c_tid, c_last in _last_change.items():
                        ...
                        # Withdraw the book until the REST snapshot lands, same as
                        # the phantom path above. Measured on the operator's own
                        # 6,915 L2 DRIFT frames, the disagreement is NOT
                        # symmetric: local ask sits BELOW the venue's on 2,814
                        # frames vs ABOVE on 1,384 - 2:1 toward the side that
                        # makes edge look better than it is, because
                        # _eval_market prices off book.best_ask (:7129).
                        # _phantom_depth only withdraws when the local ask is
                        # under the venue BID, so a merely-too-cheap ask stayed
                        # tradeable through the whole repair window (up to the
                        # 2s resync-loop tick at :9258 plus the HTTP round-trip).
                        # _snapshot_received is re-added by _resync_book (:9223).
                        self._snapshot_received.discard(c_tid)
                        _delta_touched.discard(c_tid)
                        self.request_resync(c_tid)
                ...
                if et == 'book':
                    ...
                    if bk.is_crossed:
                        self._snapshot_received.discard(tid)
                        bk.exchange_ts_ms = None
                        self.log.debug('crossed snapshot %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not trading until clean book', tid[:10])
                    else:
                    ...
                    if math.isfinite(price) and 0 < price < 1:
                        ...
                        if self._trade_sink is not None:
                            try:
                                self._trade_sink(tid, price, size, str(m.get('side') or ''))
                            except Exception as _tsink_err:
                                self.log.debug('trade sink error: %s', _tsink_err)
...
class UserFeed:
    ...
    async def run(self) -> None:
        if not self._client.api_key:
            self.log.warning('No API key ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â user feed disabled')
            return
        self._running = True
        backoff = 1.0
        while self._running:
                    ...
                    async for msg in ws:
                        ...
                        try:
                            ...
                            for m in msgs:
                                ...
                                # A1 FIX: `ws_trade_id` is the VENUE trade id, which
                                # never matches the synthetic 'fak-{oid}'/'{oid}' marks
                                # place() makes when it credits an immediate IOC match.
                                # Dedup against that credit therefore rests ENTIRELY on
                                # order_ids & _applied_ioc_order_ids (Bot._on_fill).  With
                                # no order id in the payload both dedup keys miss and the
                                # fill is applied twice - inflating the position until the
                                # drift halt trips on a corrupted ledger.
                                # Defer to REST reconciliation instead: it re-reads the
                                # trade with its order ids and dedups correctly.  Worst
                                # case is a fill credited one reconcile interval late,
                                # which is recoverable; a double credit is not.
                                if not order_ids:
                                    self._om.log_missing_order_id(ws_trade_id, tid, sorted(m.keys()))
                                    continue
                                try:
    ...
    try:
        ...
        for _fk in ('feeRate', 'takerBaseFee', 'taker_base_fee', 'makerBaseFee'):
            ...
            if _rv <= 0:
                ...
                # R38: integer-scaled fee fields are NOT the charged coefficient.
                # These markets report 1000 while the venue's own entryFeesUsdc over
                # 302 settled positions implies exactly 0.07 (1000/10000 = 0.10
                # overstates charged fees by 43.95%). No divisor maps 1000 -> 0.07,
                # so ignore the field and let CATEGORY_FEE_RATE - which is calibrated
                # to fees actually charged - stand. Verified 2026-06-01..2026-08-17:
                # median 0.07000, p05 0.06990, p95 0.07000, 97.7% within 1%.
                fee_rate = None
            fees_en = fees_en or bool(fee_rate)
            break
...
class PriceTracker:
    ...
    def __init__(self, feed: Any, prob_shrink: float=1.0, min_order_size_usdc: float=0.0, momentum_weight: float=0.0):
        ...
        # M1: per-SECOND drift EWMA, fed ret/dt (NOT ret/sqrt(dt)).
        # _ewma_mean is in vol space (ret/sqrt(dt)) because it is the centring term
        # of the variance recursion; reading it as a drift overstates mu by
        # sqrt(dt).  Kept in a separate store so the variance maths is untouched.
        self._ewma_drift: Dict[str, float] = {}
        # Lead feed (Binance) EWMA ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â higher tick rate; used for LatArb sigma only.
    ...
    async def ingest_lead_tick(self, coin: str, price: float) -> None:
        """High-frequency lead (Binance) tick ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ vol EWMA only. Does not rewrite settlement history."""
    ...
    async def _on_price(self, coin: str, price: float) -> None:
        ...
        if new_bucket and prev_1hz and (prev_1hz > 0) and (prev_ts is not None):
            ...
            if ret is not None and math.isfinite(ret):
                ...
                # M1: drift is a per-unit-time rate, so it needs ret/dt.
                drift = ret_raw / max(dt, 0.001)
                if math.isfinite(drift):
                    a = self._EWMA_ALPHA
                    self._ewma_drift[coin] = a * drift + (1.0 - a) * self._ewma_drift.get(coin, 0.0)
    ...
    def twap(self, coin: str, t0: float, t1: float) -> Optional[float]:
        """Mean of every retained tick in [t0, t1].  None when that window is empty.

        _ts_index / _px_by_ts are the same parallel sorted lists get_price_at
        bisects, so this needs no new bookkeeping.
        """
        ts_list = self._ts_index.get(coin)
        px_list = self._px_by_ts.get(coin)
        if not ts_list or not px_list or t1 < t0:
            return None
        lo = bisect.bisect_left(ts_list, t0)
        hi = bisect.bisect_right(ts_list, t1)
        if hi <= lo:
            return None
        return sum(px_list[lo:hi]) / float(hi - lo)

    def history_starts_after(self, coin: str, target_ts: float) -> bool:
        """True when get_price_at_or_before(coin, target_ts) can NEVER succeed.

        _on_price appends with ts=now and prunes from the front, so the earliest
        retained tick only moves forward. Once it is later than target_ts there
        is no tick at-or-before target_ts and there never will be again - the
        market's open anchor is permanently unreachable from this feed.
        Empty history returns False: the feed may simply not have connected yet.
        """
        ts_list = self._ts_index.get(coin)
        if not ts_list:
            return False
        return ts_list[0] > float(target_ts)
    ...
    def volatility(self, coin: str) -> float:
        # Prefer lead-feed (Binance) EWMA when present ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Chainlink RTDS is too smooth
    ...
    def prob_up(self, coin: str, current_price: float, open_price: float, tau_s: float, yes_book: Optional[OrderBook]=None, yes_trade_ewma: Optional[float]=None, btc_displacement: Optional[float]=None) -> float:
        ...
        if mw > 0.0:
            # M1: _ewma_drift is per-second (ret/dt); _ewma_mean is vol-scaled
            # (ret/sqrt(dt)) and reading it here overstated mu by sqrt(dt).
            raw_mu = self._ewma_drift.get(coin, 0.0)
...
def _walk_asks_limited(levels: Optional[List[Tuple[float, float]]], limit: float, req_shares: float) -> Tuple[float, float, int]:
    """Walk an ask ladder taking only levels priced <= limit.

    Returns (shares_got, cost, levels_consumed).

    R46c (Critic 2g-D): extracted verbatim from the dry-run FAK simulator so the
    SAME walk can be applied to a ladder frozen at submission time and to the
    live ladder at fill time. Two copies of this arithmetic would let the
    look-ahead measurement disagree with the fill it is measuring, which is the
    whole failure mode 2g-D exists to rule out.
    """
    rem = float(req_shares)
    cost = 0.0
    got = 0.0
    used = 0
    for ap, az in (levels or []):
        if ap is None or az is None or ap <= 0 or az <= 0:
            continue
        if float(ap) > float(limit) + 1e-12:
            break
...
class FiveMinStrategy:
    ...
    def __init__(self, cfg: Config, om: OrderManager, risk: 'Risk', tracker: PriceTracker, metrics: Optional[Metrics]=None):
        ...
        # R36 (claim 2): which SOURCE produced each _open_prices entry, and so
        # which CURRENCY it is quoted in. 'chainlink' = USD (settles the market),
        # 'kline' = Binance {coin}USDT. Without this the two are
        # indistinguishable once cached and LatArb cannot tell whether its
        # displacement denominator is USD or USDT. Keyed by market_id, cleared
        # wherever _open_prices is cleared.
        self._open_price_src: Dict[str, str] = {}
        ...
        # R38: market_id -> winning token_id, from CLOB /markets/{cond} tokens[].winner.
        # The 20.7h run booked all 25 settlements as source=dry-oracle-close with zero
        # market_resolved pushes, so _official_from_winner never had an input and the
        # spot estimate graded every trade - 8/25 named the wrong winner (32%).
        self._onchain_winner: Dict[str, str] = {}
        # R38: settle-source census. Any PnL whose settles are mostly
        # dry-spot-ESTIMATE is not a measurement - the estimate was wrong on 32%
        # of the 25 graded settles, turning 88.0% wr / +$58.68 into 56.0% / +$10.68.
        self._settle_src_census: Counter = Counter()
        # Phase 0.5: the census counted settles per grader but the P&L headline
        # still mixed them, so the reported number was part estimate. Split the
        # realized total and the win count the same way; only the
        # dry-onchain-winner slice is quotable.
        self._settle_pnl_by_src: Counter = Counter()
        self._settle_wins_by_src: Counter = Counter()
    ...
    def apply_authoritative_balance(self, new_bal: float, *, expected_gen: Optional[int]=None) -> Tuple[bool, str]:
        if expected_gen is not None and self._balance_gen != expected_gen:
            ...
            self.log.info('Balance snapshot discarded (stale gen: got %d, live %d) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â keeping local free-cash $%.2f', expected_gen, self._balance_gen, self._balance_cache)
        ...
        self.log.critical('CAPITAL SHOCK: free cash $%.2f -> $%.2f (drop $%.2f >= thresh $%.2f) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â external debit (withdraw/transfer). Resting BUY risk cancelled; SELL exits preserved; sizing rebased to residual free cash.', old, new_bal, drop, thresh)
        return (True, 'applied')
    ...
    async def resolve_open_price(self, mkt: 'Market', interval_start: float, now: float) -> Optional[float]:
        ...
        R33/R36 MIXED-UNIT RISK - RESOLVED, READ BEFORE CHANGING THE FALLBACK.
        The two sources are quoted in DIFFERENT CURRENCIES: Chainlink in USD
        (and it is what settles these markets), the kline fallback below in
        USDT (`{coin}USDT`). Left unmarked, LatArb displacement would become
        (Chainlink_USD_now - Binance_USDT_open) / Binance_USDT_open -
        reintroducing exactly the ~10bp USDT/USD basis that the R33 entry-path
        fix removed, and in the opposite sign direction (a USDT open is HIGHER,
        so displacement reads falsely DOWN). Measured basis: median +10.01bp,
        98.9% positive, vs a median real oracle move of 2.95bp - i.e. the
        artifact would be 3.39x the signal.
        R36 fix: every write to _open_prices now stamps _open_price_src in the
        same breath, and LatArb (:6722) whitelists 'chainlink' - it refuses any
        anchor not positively known to be USD. The directional path is
        unaffected and still uses the fallback, because it compares Binance to
        Binance and so carries no basis exposure.
        This path was UNEXERCISED in the evidence window (all 1,096 shadow rows
        had Chainlink history, so the fallback never fired) but is reachable.
        A USD-quoted fallback source would let LatArb use it too; a conversion
        guess would not, and is not worth the model risk."""
        ...
        # R36 (claim 2): record the currency provenance alongside the value.
        _src = 'chainlink' if open_price is not None else ''
                                ...
                                async with _s.get(_url, timeout=aiohttp.ClientTimeout(total=3)) as _r:
                                    if _r.status == 200:
                                        ...
                                        if _data and len(_data) > 0:
                                            ...
                                            # R36 (claim 2): USDT-quoted. LatArb
                                            # must refuse this anchor; the
                                            # directional path may still use it.
                                            _src = 'kline'
                                            self.log.info('OPEN-PRICE REST fallback: %s @ %.4f (%s) [USDT-quoted - LatArb will skip this market]', mkt.coin, open_price, _host)
                                            break
                            except Exception:
                                continue
                except Exception:
        ...
        # R36 (claim 2): stamp provenance in the same breath as the value so the
        # two cannot drift apart.
        self._open_price_src[mkt.market_id] = _src or 'chainlink'
    ...
    async def _evaluate_deprecated(self, mkt: Market) -> None:
        ...
        if periodic:
            ...
            # DIAG is log-only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never return early (that skipped hold-to-expiry / redeem).
            try:
    ...
    async def _evaluate_core(self, mkt: Market) -> None:
        ...
        if elapsed < 0:
            return
        if ttc < 0:
            self.log.debug('SKIP expired market %s (ttc=%.0fs, end_time in the past) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not evaluated', mkt.coin, ttc)
            return
        ...
        if prev_interval is not None and prev_interval != interval_epoch:
            ...
            self._open_price_src.pop(mkt.market_id, None)  # R36 (claim 2): provenance follows the value
        ...
        # Positions known immediately ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â entry-window guards must NOT block hold/redeem.
        ...
        if not has_pos_early:
            ...
            if ttc < buffer_s:
                return
        # P0: LatArb hold-to-settlement ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â register redeem meta before entry-buffer / stale-book gates
        ...
        if has_pos_early and ttc < self.cfg.forced_exit_ttc_s:
            ...
            if only_latarb:
                return
        # R36 (claim 2): this is the DIRECTIONAL path - it compares feed.price()
        # (Chainlink, see PriceTracker.feed) to _open_prices, so it is Chainlink
        # to Chainlink and has NO basis exposure. It may use the kline fallback
        # anchor too (:4353 tags it 'kline'); comparing Binance to Binance is
        # likewise unit-consistent. The LatArb path does its own gate (:6722)
        # and refuses anything not 'chainlink'. 2.0s here is a directional
        # freshness tolerance, not a LatArb price input.
        binance_age = self.tracker.feed.price_age_s(mkt.coin)
        ...
        btc_disp = None
        # R36 (claim 2): BTC-relative displacement for the directional path;
        # both sides are Chainlink (tracker.feed), so no mixed units. The
        # LatArb path does not use btc_disp - it gates on the raw oracle
        # displacement and rejects non-chainlink anchors (:6722).
        if mkt.coin != 'BTC':
        ...
        if ttc < self.cfg.forced_exit_ttc_s and (has_yes or has_no):
            # F4: LatArb edge is hold-to-settlement ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never force-sell via directional p_up.
        ...
        if yes_ask is None or no_ask is None:
            return
        # R36 (claim 5): the COMPLEMENT_ARB two-leg block that stood here was
        # REMOVED, not repaired. It placed YES and NO as two SERIAL awaits:
        #     oid_y = await self.om.place(mkt.yes_token, ...)   # FOK
        #     oid_n = await self.om.place(mkt.no_token, ...)    # FOK
        # FOK means each leg independently fills-or-kills, so a YES fill with a
        # killed NO leaves a NAKED directional position at ~0.5 - the exact
        # opposite of the market-neutral trade it believed it had opened. Its
        # only failure handling unwound _pending_entry BOOKKEEPING; it never
        # cancelled or sold the leg that actually filled, so the naked position
        # was simply held to expiry as a coin flip.
        # It was already unreachable: validate() (:623) makes
        # COMPLEMENT_ARB_ENABLED=true a hard rc=1 boot failure. Deleted rather
        # than left dead, because dead-unsafe-code is one guard-removal away
        # from live-unsafe-code, and a future maintainer deleting a guard they
        # do not understand is a likelier failure than one writing a new arb.
        # A correct version needs depth-walked sizing and an atomic unwind of a
        # single-sided fill; that is a new feature, not a bug fix, so it is not
        # being invented here. The fee math it fed (fee_y/fee_n/arb_cost/
        # arb_edge) died with it - nothing else read those.
    ...
    async def _place_sliced(self, mkt: Market, token_id: str, ask_price: float, label: str, prob: float, edge: float, entry_slip: float=0.0, exit_slip: float=0.0, req_edge: float=0.0) -> None:
        if self.measure_only and (not self.cfg.dry_run):
            self.log.debug('MEASURE-ONLY (LIVE): suppressed %s entry %s (p=%.3f edge=%.3f ask=%.3f) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no proven edge, no order placed.', label, mkt.coin, prob, edge, ask_price)
            return
        if self.measure_only and self.cfg.dry_run:
            self.log.debug('MEASURE-ONLY (DRY): allowing paper trade %s %s (p=%.3f edge=%.3f ask=%.3f) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â collecting outcome data for go/no-go gate.', label, mkt.coin, prob, edge, ask_price)
        if req_edge <= 0.0:
        ...
        if kelly_sz <= 0.0:
            self.log.info('SKIP ENTRY %s %s: min clip exceeds %.0f%% bankroll cap ($%.2f free cash) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â account too small to size safely', label, mkt.coin, self.cfg.max_bankroll_fraction * 100.0, self.free_cash() if self._balance_ts > 0 else self.cfg.max_order_size * 2)
            return
        ...
        if top_edge is not None and top_edge >= req_edge:
            ...
            for _ in range(12):
                ...
                if em is not None and em >= req_edge:
                    ...
                    hi = mid
            sz = best
            self.log.info('SIZE-DOWN %s %s: Kelly $%.1f ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ $%.1f to hold edge ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ %.4f', label, mkt.coin, kelly_sz, sz, req_edge)
        ...
        if self._realized_loss.get(mkt.market_id, 0.0) <= -self.cfg.max_position:
            self.log.warning('SKIP ENTRY %s %s: realized loss $%.2f on this market has reached the max_position cap $%.2f ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refusing new entries (likely overlapping-interval accumulation; investigate).', label, mkt.coin, self._realized_loss.get(mkt.market_id, 0.0), self.cfg.max_position)
            return
    ...
    def _mark_for_redemption(self, mkt: 'Market', token_id: str, pos: 'Position', expected_payout: float) -> None:
        ...
        if rkey in self._pending_redemptions:
            # Already registered on fill ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refresh sizes from live position; do not double soft PnL.
    ...
    def record_outcome(self, win: bool, market_id: Optional[str]=None, net_pnl: Optional[float]=None, coin: Optional[str]=None, side: str='', strategy: str='directional', outcome_kind: str='final', log_calibration: bool=True, grade_source: str='', ref: Optional[Dict[str, Any]]=None) -> None:
        win_b = bool(win)
        ...
        if str(strategy or '').lower() == 'latarb' and str(outcome_kind or '') == 'final' and net_pnl is not None:
            _row = {'ts': time.time(), 'market_id': market_id or '', 'coin': coin or '', 'side': side or '', 'win': win_b, 'net_pnl': float(net_pnl), 'dry_run': bool(self.cfg.dry_run), 'source': 'settle', 'grade_source': grade_source}
            if ref:
                _row.update({f'ref_{k}': v for k, v in ref.items()})
            _append_latarb_settle(_row, self.cfg)
    ...
    def _log_shadow(self, info: Dict[str, Any]) -> None:
        # R35 FIX (F3): strategy was hardcoded 'probe', so every shadow row was
        # attributed to a strategy that does not exist and LatArb's own adverse
        # prints could not be separated from directional ones. The probe now
        # supplies the real entry strategy (:2085); fall back to 'probe' only if
        # it is absent. calibration_report's adverse mean (:4278) filters on
        # row_type=='shadow' alone and never reads this column, so the recorded
        # value changes attribution only - no existing aggregate shifts.
        self._calibration_log('shadow', market_id=str(info.get('market_id') or ''), side=str(info.get('side') or ''), adverse_bps=info.get('adverse_bps'), strategy=str(info.get('strategy') or 'probe'), outcome_kind='')
            ...
            if f is None:
                ...
                for cand in candidates:
                    try:
                        ...
                        # R36 FIX: buffering=1 (line-buffered). The flush-every-
                        # _CALIB_FLUSH_EVERY below only runs on a later write, so
                        # a kill -9 / restart lost every row still in the 8192B
                        # text buffer: the 08-07..08-11 run wrote 99 settles but
                        # the CSV ends at 175 rows == exactly 7 * 25.
                        f = open(cand, 'a', buffering=1, encoding='utf-8')
                    except OSError:
                        continue
                    if cand != primary:
    ...
    async def _execute_exit(self, mkt: Market, token_id: str, bid_price: float, label: str, shares: float) -> None:
        ...
        if sweep_price <= 0:
            self.log.warning('EXIT %s skipped ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â insufficient bid depth', label)
            _rollback_inflight()
            return
        if sweep_price < 0.01:
            sweep_price = 0.01
    ...
    def cleanup_expired(self, markets: List[Market]) -> None:
        ...
        all_tracked = set()
        for container in (self._open_prices, self._open_price_src, self._open_intervals, self._sustain_counts, self._entry_times, self._eval_debounce):
            all_tracked.update(container.keys())
        for mid in all_tracked:
            if mid not in active_ids:
                self._open_prices.pop(mid, None)
                self._open_price_src.pop(mid, None)  # R36 (claim 2): provenance follows the value
    ...
    def _settle_resolved_market(self, market_id: str, winning_asset_id: Optional[str]=None) -> None:
        ...
        def _official_from_winner(win_tid: str) -> Optional[Tuple[bool, float, bool]]:
            if not win_tid:
                win_tid = str(self._onchain_winner.get(market_id) or '')
            if not win_tid:
                return None
        ...
        _spot_ref: Dict[str, Any] = {}

        def _spot_reference() -> Dict[str, Any]:
            """Chainlink open / interval-close / TWAP60 for this market, resolved once.

            Phase 2.1 asks whether a spot reference reproduces the on-chain winner.
            The settle ledger recorded only the graded side, so those inputs lived
            nowhere durable and the comparison could not be run from any log. Both
            _official_from_spot and the ledger row read this one dict, so the
            estimator and the record of it can never disagree.
            """
            if _spot_ref:
                return _spot_ref
            ...
            if end_ts <= 0.0:
                try:
                    ...
                    end_ts = 0.0
            usable = bool(self.tracker) and end_ts > 0.0
            _spot_ref.update({'open': open_spot if open_spot > 0 else None, 'close': self.tracker.get_price_at(coin, end_ts, max_gap_s=10.0) if usable else None, 'twap60': self.tracker.twap(coin, end_ts - 60.0, end_ts) if usable else None, 'end_ts': end_ts})
            return _spot_ref

        def _official_from_spot() -> Optional[Tuple[bool, float, bool]]:
            # Paper settlement must use the Chainlink price AT interval close, never live spot hours later.
            ref = _spot_reference()
            open_spot, final_spot = (ref.get('open'), ref.get('close'))
            if not final_spot or not open_spot:
                return None
        ...
        def _apply_settlement(proceeds: float, redeemed_amounts: List[float], source: str) -> None:
            ...
            self._open_price_src.pop(market_id, None)  # R36 (claim 2): provenance follows the value
            ...
            self._notify_state()
            # R52: `source` here is the dry-onchain-winner / dry-spot-ESTIMATE
            # label from :6599. The estimate is a 900s fallback that named the
            # wrong winner on 32% of settles in an earlier run, so the ledger
            # must say which grader decided each row - state is pruned and the
            # log is rotated, so this file is the only durable record.
            # Phase 2.1: carry the graded direction and the spot reference into the
            # ledger row so `sign(close - open)` and TWAP60 can be scored against the
            # on-chain winner from the file alone, with no venue call.
            _ref = dict(_spot_reference())
            _ref['up_won'] = official[2] if official else None
            self.record_outcome(total_realized > 0.0, market_id=market_id, net_pnl=total_realized, coin=coin, side=held_side, strategy=strategy, outcome_kind='final', grade_source=source, ref=_ref)
            self._settle_src_census[source] += 1
            self._settle_pnl_by_src[source] += float(total_realized)
            if total_realized > 0.0:
                self._settle_wins_by_src[source] += 1
            _cen = ' '.join(f'{k}={v}' for k, v in sorted(self._settle_src_census.items()))
            _est = self._settle_src_census.get('dry-spot-ESTIMATE', 0)
            _tot = sum(self._settle_src_census.values()) or 1
            self.log.info('SETTLED %s | source=%s payout=$%.6f cost=$%.6f partial=$%+.6f realized=$%+.6f correction=$%+.6f | settle_sources: %s (%.0f%% estimated, NOT evidence)', coin, source, proceeds, local_cost, partial_pnl, total_realized, correction, _cen, 100.0 * _est / _tot)
        ...
        _from_chain = official is not None
        ...
        if self.cfg.dry_run:
            if official is None:
                ...
                self._notify_state()
                return
            # R38: an on-chain winner arrives ~429s after close but dry settlement is
            # due at end+5s.  Booking the spot estimate first is what made 32% of
            # settles name the wrong winner, so retain until the chain has had its
            # window; DRY_STALE_SETTLE_S still flat-closes anything never resolved.
            if not _from_chain:
                _wait_end = 0.0
                _wm = self._market_lookup(market_id) if self._market_lookup is not None else None
                if _wm is not None and getattr(_wm, 'end_time', None):
                    _wait_end = float(_wm.end_time or 0.0)
                if _wait_end <= 0.0:
                    try:
                        _wait_end = float(meta0.get('end_time') or 0.0)
                    except (TypeError, ValueError):
                        _wait_end = 0.0
                if _wait_end > 0.0 and time.time() - _wait_end < DRY_ONCHAIN_WAIT_S:
                    self._release_settling_exposure(market_id, leg_keys)
                    self._log_retained_settlement(market_id, 'RESOLVED %s | awaiting on-chain winner (%.0fs of %.0fs); spot estimate withheld', coin, time.time() - _wait_end, DRY_ONCHAIN_WAIT_S)
                    self._notify_state()
                    return
            _, _, up_won = official
            paper_payout = shares_yes if up_won else shares_no
            # R38: name the source by what actually decided it. 'dry-oracle-close' read
            # as authoritative in every report; the spot compare is an ESTIMATE and was
            # wrong on 32% of settles, so PnL carrying this tag is not evidence.
            _src = 'dry-onchain-winner' if _from_chain else 'dry-spot-ESTIMATE'
            _apply_settlement(float(paper_payout), [shares_yes, shares_no], _src)
            return
...
def _append_latarb_settle(row: dict, cfg: Optional['Config']=None) -> None:
    ...
    A silent failure here is invisible loss of the ONLY settlement-true evidence
    the live-proof gate reads, so a write failure is logged rather than swallowed.
    """
    path = ''
    try:
        ...
        get_logger('SettleLedger').warning('settle ledger write FAILED (%s): %s - live-proof evidence lost for market %s', path or '<unresolved>', e, row.get('market_id', '?'))
...
def _write_latarb_state_unlocked(state: dict, path: str) -> None:
    ...
    try:
        with open(tmp, 'w', encoding='utf-8') as fh:
            ...
            os.fsync(fh.fileno())
        os.replace(tmp, path)
        # R35 FIX: fsync the CONTAINING DIRECTORY, not just the temp file. The
        # fsync above makes the new bytes durable; it says nothing about the
        # rename, whose metadata can still sit in the directory's dirty page
        # cache. On a power loss the file could revert to the previous
        # snapshot even though _write_latarb_state_durable had already returned
        # - which is exactly the acknowledgement contract :8118-8127 promises
        # the fill path (a fill is not acknowledged until its state is durable,
        # and :9309 escalates a failed checkpoint to the fill-durability latch).
        # Best-effort: directory fsync is not supported on every platform
        # (Windows raises), and failing the whole write over it would be worse
        # than the gap it closes.
        try:
            _dfd = os.open(os.path.dirname(path) or '.', os.O_RDONLY)
            try:
                os.fsync(_dfd)
            finally:
                os.close(_dfd)
...
# A3: bounded retry for the durable state write.  Deliberately small - this runs
# in the fill acknowledgement path, so the total stall on a sick disk is capped
# at roughly _STATE_WRITE_RETRIES * _STATE_WRITE_BACKOFF_S (~0.35s) before the
# fault is escalated to the fill-durability latch.
_STATE_WRITE_RETRIES: int = 3
_STATE_WRITE_BACKOFF_S: float = 0.05


class StateWriteTransientError(OSError):
    """Durable state write failed for an environmental reason (disk full, EIO,
    transient NFS fault) rather than a data-integrity one.  Retryable."""


def _write_latarb_state_durable(state: dict, path: str) -> None:
    """Write runtime state, retrying transient OS faults before giving up.

    A3 FIX: previously ONE os.fsync OSError propagated straight through
    _on_fill -> latch_fill_failure and permanently blocked every subsequent
    order while positions stayed open and unmanaged - a transient disk hiccup
    became a silent trading halt with live exposure.

    The distinction that matters: an OSError is environmental and usually clears
    on retry; a ValueError/TypeError from json.dump (NaN, unserialisable object)
    is a data-integrity fault that WILL recur and must latch immediately.  Only
    the former is retried, and only a bounded number of times - an unbounded
    retry inside the fill path would hang the loop instead of halting it.
    """
    last: Optional[BaseException] = None
    for attempt in range(_STATE_WRITE_RETRIES):
        try:
            with _LATARB_STATE_LOCK:
                _write_latarb_state_unlocked(state, path)
            return
        except OSError as e:
            # Environmental: retry with a short backoff.
            last = e
            if attempt + 1 < _STATE_WRITE_RETRIES:
                time.sleep(_STATE_WRITE_BACKOFF_S * (attempt + 1))
        except Exception:
            # Integrity fault (unserialisable state) - retrying cannot help.
            raise
    raise StateWriteTransientError(f'durable state write to {path} failed after {_STATE_WRITE_RETRIES} attempts: {last}') from last
...
def _snapshot_runtime_state(markets: List['Market'], strategy: Optional['FiveMinStrategy'], risk: Optional['Risk'], redeemer: Optional['RedemptionEngine'], dry_run: bool, identity: dict, path: str, trade_pnl_in_flight: Optional[Dict[Tuple[str, str], float]]=None, om: Optional['OrderManager']=None, applied_trade_ids: Optional[Deque[str]]=None, applied_ioc_order_ids: Optional[Deque[str]]=None, defer_write: bool=False) -> dict:
    """Build (and by default write) the durable runtime-state snapshot.

    A7 FIX: `defer_write=True` returns the validated snapshot WITHOUT touching
    disk, so a caller on the asyncio loop can hand the blocking
    json.dump/fsync/os.replace to a worker thread (Bot._save_runtime_state_async).
    The BUILD must stay on the loop thread either way: it reads live Market /
    Risk / Redemption objects, and building it off-thread while the loop mutates
    them would persist a torn snapshot - strictly worse than a latency spike."""
    with _LATARB_STATE_LOCK:
        ...
        risk_row: dict = {}
        if risk is not None:
            risk_row = {'pnl': float(risk._pnl), 'pnl_peak': float(risk._pnl_peak), 'day_start': float(risk._day_start), 'day_start_bankroll': float(getattr(risk, '_day_start_bankroll', 0.0)), 'day_age_s': max(0.0, time.monotonic() - risk._day_reset), 'day_anchor': str(getattr(risk, '_day_anchor', '') or ''), 'month_start': float(risk._month_start), 'month_age_s': max(0.0, time.monotonic() - risk._month_reset), 'consecutive_losses': int(risk._consecutive_losses), 'halted': bool(risk._halted), 'halt_type': str(risk._halt_type), 'reason': str(risk._reason)}
...
def _restore_runtime_state(markets: List['Market'], strategy: 'FiveMinStrategy', risk: 'Risk', redeemer: 'RedemptionEngine', om: 'OrderManager', dry_run: bool, state: Optional[dict]=None, trade_pnl_in_flight: Optional[Dict[Tuple[str, str], float]]=None, applied_trade_ids: Optional[Deque[str]]=None, applied_ioc_order_ids: Optional[Deque[str]]=None) -> Tuple[int, Set[str], List[str]]:
    ...
    for row in state.get('redeem_meta', []):
        ...
        if clean.get('open_price'):
            ...
            # R36 (claim 2): redeem_meta persists the VALUE but not its currency,
            # so a restored anchor could be either feed. Mark it unknown rather
            # than assuming; LatArb whitelists 'chainlink' and will re-resolve
            # this market with real provenance at the next interval roll (:4453).
            strategy._open_price_src[mid] = 'restored'
    ...
    _now_wall = time.time()
    # R35 FIX: a saved_at in the FUTURE (host clock stepped backwards, or an NTP
    # correction after a forward jump) drives the max() below to 0.0, so no
    # downtime is credited and day_age stops advancing across restarts. Every
    # halt that expires on the day timer (:6414) then stalls for exactly as long
    # as the clock stays behind saved_at, and _validate_runtime_state does not
    # catch it - :6043 calls _finite_nonnegative with no `upper`, so it only
    # rejects saved_at <= 0. Worse, the condition is silent: nothing logs it, so
    # the operator sees a halt that will not clear and no reason why. Warn and
    # treat the write as if it happened now, which bounds the stall to this boot
    # instead of letting it repeat on every subsequent one.
    if saved_at > _now_wall + 1.0:
        logging.getLogger('Bot').warning('runtime state saved_at is %.0fs in the FUTURE (host clock moved backwards); treating it as now. Day/month age is not credited for that gap.', saved_at - _now_wall)
        saved_at = _now_wall
    downtime = max(0.0, _now_wall - saved_at)
    ...
    if rr:
        ...
        # R34 FIX (the 2-day restart storm in dryrun-analysis.md): _pnl_peak has
        # to expire with the day here, exactly as Risk.ok() already rebases it at
        # :7668, because the cap it feeds (MAX_DRAWDOWN_FROM_PEAK) is a per-day
        # allowance. It did not, and that made the drawdown halt UNCLEARABLE:
        # persisted pnl=-$22.62 with peak=$4.52 gives dd=$27.14 > $15 cap. Once a
        # day had passed, the keep_halt test below correctly cleared the restored
        # halt - and then the FIRST Risk.ok() call re-halted on the stale peak and
        # re-persisted it. ok()'s own rebase could not save it either, because
        # _day_reset is set to `now` just below, so its 86400s test needed another
        # full day. Every boot refused with rc=3: 41,714 boot->refuse->exit cycles
        # over 2 days (11,106 drawdown + 9,751 daily_loss) and zero trades, on an
        # account whose only real problem was a $22 loss. Rebasing here breaks the
        # cycle at the source; a fresh drawdown inside the new day still halts.
        risk._pnl_peak = risk._pnl if day_age >= 86400.0 else max(risk._pnl, float(rr.get('pnl_peak', risk._pnl)))
        ...
        # AUDIT FIX (calendar-day anchor): restore the persisted UTC day anchor
        # so variance halts clear at the UTC midnight boundary; state files
        # written by older builds (no key) keep the rolling-24h fallback.
        risk._day_anchor = str(rr.get('day_anchor') or '') or None
        ...
        # R35 FIX: 'monthly_loss' (:8002) is a variance throttle on a 30-day
        # timer, exactly like daily_loss on a 24h one, but it was in no clear
        # set anywhere. ok()'s month rebase at :7997-7999 cannot clear it either
        # - `if self._halted: return False` at :7958 returns first - so the only
        # way out was deleting the state file by hand. Expire it with the month
        # here, where _month_start/_month_reset (:6422-6423) already rebase.
        _anchor_rolled = risk._day_anchor is not None and risk._day_anchor < datetime.now(timezone.utc).date().isoformat()
        keep_halt = bool(rr.get('halted')) and not ((day_age >= 86400.0 or _anchor_rolled) and halt_type in ('daily_loss', 'consec_losses', 'drawdown')) and (not (month_age >= 30 * 86400.0 and halt_type == 'monthly_loss'))
        # R35 FIX: a 'rejects' halt (:7975, 5 consecutive venue rejects) is the
        # one halt whose own evidence cannot survive a restart. The counter
        # lives on OrderManager (:1907), is reset by any success (:2281) or
        # reconcile (:2387), and is NOT in risk_row - so om.rejects is 0 in
        # every fresh process and the restored halt is stale by construction.
        # Risk.ok() already encodes that intent at :7923 (`_halt_type ==
        # 'rejects' and om.rejects < 5` -> clear), but the boot gate at :8668
        # refuses before ok() is ever reached, and no revalidation block covers
        # it (:8572 drift/rpc, :8619 capital_shock, :8654 redemption). The halt
        # therefore had NO clear path: deleting the state file by hand was the
        # only way to re-arm. Dropping it here is self-limiting - if the venue
        # is still rejecting, 5 fresh rejects re-halt within seconds - and
        # carries no position risk, because a rejected order never filled.
        if keep_halt and halt_type == 'rejects':
            logging.getLogger('Bot').warning('Clearing restored HALT [rejects]: the reject counter does not persist, so this halt cannot be revalidated. 5 fresh rejects will re-halt.')
            keep_halt = False
...
class LatencyArb:
    ...
    def __init__(self, cfg: Config, om: OrderManager, tracker: PriceTracker, polyfeed: HyperPolyFeed, by_coin: Dict[str, List[Market]]):
        ...
        # market_id -> [min_yes_ask, max_yes_ask, samples] while undecided, then
        # [..., verdict] once _quiet_reason has frozen it at first candidacy.
        # Fed by _shadow_scan on the shadow throttle's cadence.
        # See _quiet_note / _quiet_reason / Config.latarb_quiet_*.
        self._quiet_ask: Dict[str, List[float]] = {}
        self._last_latarb_log: Dict[str, float] = {}
        self._shadow_fh = None
        self._shadow_init_done = False
        self._shadow_writes = 0
        # R45: per-market throttle for PAIREVAL instrumentation lines.
        self._pair_log_ts: Dict[str, float] = {}
        ...
        self._slip_n: int = 0
        self._expected_fill_px: Dict[str, float] = {}  # token ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ submission-time expected VWAP
        ...
        # Coins with a lead-tick-driven eval fan-out already running. See the
        # backpressure note in on_binance_tick.
        self._lead_eval_inflight: Set[str] = set()
        # R46a: last lead-driven fan-out per coin (monotonic). Bounds RATE, which
        # _lead_eval_inflight does not - see on_binance_tick. 3 keys, no cleanup.
        self._lead_eval_ts: Dict[str, float] = {}
        self._place_lock = asyncio.Lock()
        self._skip_log_last: Dict[str, float] = {}
        self._skip_counts = Counter()
        # Phase 1.2: the oracle_stale skip log is throttled (706 lines for 27,966
        # skips in the 2026-09-03 run), so the ages the tolerance should be tuned
        # from were never on record. Keep them.
        self._oracle_age_s: Deque[float] = deque(maxlen=4000)
    ...
    def _emp_q(self, up: bool, ask: Optional[float],
               disp_bps: Optional[float]=None) -> float:
        """Effective empirical win rate for the traded side. 0.0 = EMP mode off.

        R46b (2g-C): single source of truth for the substituted q, so the
        PAIREVAL instrumentation line and the edge gate can never disagree
        about which q was actually applied - the r45 line logged the flat
        config value, which under band-aware q would have been a different
        number from the one that priced the trade.

        DN keeps the flat rate: LATARB_SIDE_FILTER=UP means the DN band has no
        graded in-band sample to split on. Splitting it would be inventing a
        threshold from zero rows.
        """
        if not up:
            return float(self.cfg.latarb_emp_wr_dn)
        # ALT-A (Critic 2k-3, promoted to head of queue): the 2D table is
        # strictly more general than the 1D band split below -- it CONTAINS it --
        # so it is consulted first and the band split stays the fallback. Needs
        # both axes; a caller with no displacement gets the r46-e path unchanged.
        if disp_bps is not None and ask is not None:
            _t = _load_q_table()
            if _t is not None:
                _q2 = _q_lookup(_t, float(disp_bps), float(ask))
                if _q2 is not None:
                    return _q2
        split = float(self.cfg.latarb_q_split_ask)
        if split > 0.0 and ask is not None and self.cfg.latarb_emp_wr_up_high > 0.0:
            return float(self.cfg.latarb_emp_wr_up_high if float(ask) >= split
                         else self.cfg.latarb_emp_wr_up_low)
        return float(self.cfg.latarb_emp_wr_up)

    def skip_summary(self, limit: int=0) -> str:
        """Skip reasons, most frequent first. limit <= 0 means no truncation.

        R35 FIX: this defaulted to 8 and truncated with most_common(8). A reason
        that kept firing but fell out of the top 8 simply vanished from the
        status line at whatever count it held when it was displaced, and its
        true total became unrecoverable from the log. On 2026-08-03
        'anchor_currency' was last shown at 7,940 and then kept firing for a
        further 3h18m (18 of its 39 WARNINGs land after that point) with no
        trace of the final value anywhere. The reason set is a small fixed
        vocabulary of string literals, so printing all of them costs one
        bounded line per 60s status tick.
        """
        if not self._skip_counts:
            return ''
        _items = self._skip_counts.most_common(int(limit)) if int(limit) > 0 else self._skip_counts.most_common()
        return ', '.join(f'{k}={v}' for k, v in _items)
    ...
    def _taker_delay_seconds(self) -> float:
        """SignalÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢fill horizon for delay buffer: measured order p95 when available, else 250ms."""
    ...
    def maybe_update_kill_switch(self) -> None:
        ...
        if n >= LATARB_MIN_LIVE_ATTEMPTS_FOR_KILL:
            ...
            if rw_fr < LATARB_KILL_SWITCH_MIN_FILL_RATE:
                ...
                self.log.critical('LatArb measure-only: rolling fill rate %.0f%% over last %d FAK attempts < %.0f%% ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â pause %.0fs then re-check', 100.0 * rw_fr, n, 100.0 * LATARB_KILL_SWITCH_MIN_FILL_RATE, LATARB_KILL_COOLDOWN_S)
    ...
    def _append_fill_log(self, row: dict) -> None:
        # Silent failure here starves the live-proof fill-rate gate, so log it.
        path = ''
        try:
            ...
            self.log.warning('fill ledger write FAILED (%s): %s - live-proof evidence lost', path or '<unresolved>', e)

    async def on_binance_tick(self, coin: str, price: float, *, is_lead: bool=True) -> None:
        # R33: Binance (is_lead=True) is now a WAKE-UP and a volatility source
        # only - it no longer sets direction, so no USDT-quoted price leaves this
        # method. Direction and magnitude both come from Chainlink, which is what
        # settles these markets. Oracle wake (is_lead=False) carries the oracle
        # tick itself in `price`, so it becomes `oracle_px` directly below.
        ...
        if is_lead and self.tracker is not None:
            try:
                ...
                pass
        # R33: an is_lead branch here used to reuse the last Binance tick as
        # `lead_px` and hand it to the entry path as the direction input. Deleted
        # with the mixed-feed defect - the oracle tick in `raw_px` is now the
        # signal, and the guard that followed could never fire (both branches
        # assigned a value already proven > 0 above).
        oracle_px = None
        if self.tracker is not None and self.tracker.feed is not None:
            try:
                # R36 (claim 1): was max_age_s=2.0, a pre-R33 tolerance from when
                # the oracle only confirmed the sign. It is the price input now,
                # so a stale read is adverse selection - see the constant. (The
                # max_age_s=2.0 reads at :4523/:4543 are the DIRECTIONAL path
                # and are intentionally left alone.)
                oracle_px = self.tracker.feed.price(coin, max_age_s=LATARB_ORACLE_MAX_AGE_S)
            except Exception:
                oracle_px = None
        if not is_lead and raw_px > 0:
            oracle_px = raw_px
        if self.cfg.latarb_shadow:
            # R33: shadow records the SETTLING feed, same as live fire. It used
            # to record the Binance lead against a Chainlink open, which is the
            # mixed-feed defect itself, so `--latarb-analyze` was scoring the
            # USDT/USD basis and calling it a latency edge.
            self._shadow_scan(coin, oracle_px=oracle_px)
        if not self.cfg.latency_arb_enabled:
            return
        # r52 ROOT CAUSE of the oracle_stale skip flood. A lead (Binance) tick
        # fans out an eval that reads the CACHED Chainlink price; measured on the
        # 2026-09-04 13.96h run the oracle age is p50=1.39s p90=2.54s against the
        # 1.2s tolerance, so the MEDIAN lead tick is ALREADY past tolerance before
        # the fan-out starts and every one of those evals walks every market only
        # to die at oracle_stale (:8164). The age deque saturates at maxlen=4000,
        # so >=4000 skips, not the 1,308 throttled log LINES.
        # The tolerance is NOT the bug and must not be raised: R50/R51 measured
        # stalled-feed entries settling +457.31 at 0.8571 win vs clean-feed -95.04
        # at 0.4748 on the SAME fill price - staleness inflates |z| and MANUFACTURES
        # the edge, which is why R51 blocks it. Oracle wakes (is_lead=False) carry a
        # fresh tick (:7888) and never reach this branch.
        # This removes wasted work and log noise. It does NOT add a single trade:
        # the eval it drops could only ever have returned oracle_stale.
        if is_lead and oracle_px is None:
            _sage = float('inf')
            try:
                if self.tracker is not None and self.tracker.feed is not None:
                    _sage = float(self.tracker.feed.price_age_s(coin))
            except Exception:
                pass
            if math.isfinite(_sage):
                self._oracle_age_s.append(_sage)
            return self._skip_latarb('oracle_stale_lead', '%s cached oracle age %.2fs > tol %.2fs - lead tick cannot pass the gate', coin, _sage, LATARB_ORACLE_MAX_AGE_S)
        ...
        if self._cleanup_counter % 100 == 0:
            ...
            # 1h >> the 300/900s market lifetime, so no live market is ever dropped
            # mid-flight; without this both dicts grow by ~2000 keys/day forever.
            self._shadow_last = {k: v for k, v in self._shadow_last.items() if _cleanup_now - v <= 3600.0}
            self._quiet_ask = {k: v for k, v in self._quiet_ask.items() if k in self._shadow_last}
        markets = list(self.by_coin.get(coin, []))
        if not markets:
            return
        # BACKPRESSURE (live regression: shard closed with 1013 'slow consumer:
        # send buffer full'). A lead (Binance) tick reads the LAST CACHED oracle
        # price at :6939, so a second lead-tick fan-out for the same coin while
        # one is still in flight re-evaluates an IDENTICAL signal input - it is
        # duplicate work, not a missed fire. Binance aggTrade across 3 coins runs
        # tens of ticks/sec and each tick gathers over every market for that
        # coin; those tasks share the event loop with the market-WS shard reader
        # (:3231), which only reaches its next recv() between them. Left
        # unbounded the reader is starved, Polymarket's send buffer to us fills,
        # and it closes the shard - which then manufactures book_not_ready /
        # book_stale skips downstream. Coalescing bounds the fan-out to one per
        # coin without adding latency to the FIRST tick.
        # R46d CORRECTION: 2g-F REFUTED this as the cause of the closes. r46a cut
        # market evals 4.7x and the strict close rate did not move (7.13/hr over
        # 3.225h vs the 8.21/hr baseline, P(X<=23|lam=26.48)=0.29). The coalescing
        # below is kept because duplicate work is still duplicate work, but the
        # "reader is starved" story above is WRONG: see _WS_MAX_QUEUE for the
        # mechanism, which needs no starvation at all.
        # Oracle wakes (is_lead=False) carry a NEW price (:6942) and are never
        # coalesced - they are the signal.
        if is_lead:
            if coin in self._lead_eval_inflight:
                return self._skip_latarb('lead_eval_coalesced')
            # R46a: the in-flight guard bounds CONCURRENCY, not RATE. A fan-out
            # that completes in ~5ms readmits the very next aggTrade tick, so the
            # starvation this was meant to cure survived it: measured on the
            # 2026-08-22/23 soak, 165 `1013 slow consumer: send buffer full`
            # shard closes in 18h56m (one per 6.9 min), each logging "books not
            # ready until resnapshot", yielding 159,253 book_stale /
            # book_not_ready / no_book_not_ready skips (10.2% of non-coalesced
            # evals) and 13,832 forced book resyncs. Proof it is a BLACKOUT and
            # not jitter: among books that survive the age gate, age is p99=238ms
            # with 2 of 80,000 over the 250ms cap - a shard is either healthy or
            # dark. And it is adversely timed by construction: the send buffer
            # fills when message rate spikes, i.e. during volatility, which is
            # exactly when displacement fires. So we went blind precisely when
            # the alpha appeared.
            #
            # Nothing is given up by rate-limiting it, because post-R33/r45 the
            # lead tick is inert: it sets no direction and no magnitude (:7501),
            # its Phi(z) is overwritten by the empirical win rate (:7599), and
            # its volatility contribution is discarded - 0 of 1,669
            # CLOB-winner-graded harness rows reach GBM_SIGMA_FLOOR_PER_SEC
            # (median realized 1.065e-04, i.e. the floor is 2.35x ABOVE realized
            # vol), and live shadow rows sit at the floor in 98.71% of 40,000.
            # A lead tick's only surviving job is re-pricing a BOOK change inside
            # the 1.2s LATARB_ORACLE_MAX_AGE_S window, which 400ms still samples
            # ~3x. Oracle wakes (is_lead=False) carry the signal and are never
            # rate-limited, here or above.
            #
            # Reuses cfg.eval_debounce_ms rather than adding a knob: it is
            # already declared (:514), already validated >=0 with 0=off (:782),
            # and already exported as EVAL_DEBOUNCE_MS=400 - but had no read site
            # anywhere in the file, because the directional path that was to use
            # it is retired. If that path is ever revived, split the knob then.
            _lead_debounce_s = max(0.0, float(self.cfg.eval_debounce_ms)) / 1000.0
            if _lead_debounce_s > 0.0:
                _lead_mono = time.monotonic()
                if _lead_mono - self._lead_eval_ts.get(coin, 0.0) < _lead_debounce_s:
                    return self._skip_latarb('lead_eval_debounced')
                self._lead_eval_ts[coin] = _lead_mono
            self._lead_eval_inflight.add(coin)
        try:
            results = await asyncio.gather(*(self._eval_market(mkt, coin, _bankroll, oracle_px=oracle_px) for mkt in markets), return_exceptions=True)
        finally:
            if is_lead:
                self._lead_eval_inflight.discard(coin)
    ...
    async def _eval_market(self, mkt: Market, coin: str, _bankroll: float, oracle_px: Optional[float]=None) -> None:
        # R33: the Binance lead `price` argument is GONE. Displacement is now
        # measured on the settlement feed at both ends (see `oracle_now` below),
        # so nothing in this function may read a USDT-quoted price. Removing the
        # parameter rather than leaving it unused makes that structural: a future
        # edit cannot reintroduce the mixed-feed defect without re-plumbing it.
        # The Binance lead still WAKES this path (on_binance_tick) and still
        # feeds sigma via tracker.volatility(); it just no longer sets direction.
        now = time.time()
        ...
        # R33: open AND current are both settlement-oracle (Chainlink) now. The
        # old text here read "Lead price = Binance for displacement" - that WAS
        # the defect, so the comment is corrected rather than left to re-teach it.
        ...
        if not open_price or open_price <= 0:
            ...
            # R36 (claim 2): the shared cache can hold a Binance {coin}USDT open
            # from the directional REST fallback. `oracle_now` below is Chainlink
            # USD, so consuming a 'kline' anchor here rebuilds the exact
            # mixed-currency displacement R33 removed - with the sign INVERTED,
            # because a USDT open reads HIGH and biases displacement DOWN.
            # LatArb refuses it; the directional path is free to keep using it
            # (it compares Binance to Binance, so it has no basis exposure).
            # Whitelist, not blacklist: an anchor whose currency is not POSITIVELY
            # known to be USD is refused. 'restored' (from redeem_meta, which
            # persists the value but not the feed) lands here too and re-resolves
            # at the next interval roll.
            _anchor_src = strat_anchor._open_price_src.get(mkt.market_id, '') if strat_anchor is not None else ''
            if cached > 0 and _anchor_src == 'chainlink':
                # Resolved from Chainlink history earlier in this market's life
                # and the WINDOW has since pruned past interval_start. Still USD.
                open_price = cached
            elif self.tracker.history_starts_after(coin, interval_start):
                # R37: the interval opened before this process held any oracle
                # tick (cold boot, or a feed outage that outlived the interval).
                # No USD anchor can EVER be established for it: the RTDS feed has
                # no backfill endpoint and _on_price only ever appends at `now`,
                # so the set of ticks at-or-before interval_start is fixed at
                # discovery and can only shrink. Refusing HERE rather than one
                # branch lower is the whole point - otherwise the kline fallback
                # fills the cache with a USDT open, the whitelist below refuses it
                # on every subsequent tick, and the operator reads 10,951
                # 'anchor_currency' skips (2026-08-17 live run) that look like a
                # currency bug instead of a cold start. It also drops a pointless
                # 3s REST round-trip per market: nothing consumes a kline anchor
                # now that the directional path is retired (:4619 raises).
                # These skips end when the market expires - at most one interval
                # (15 min) after boot, which is 79% of a 21-minute run and 0.3%
                # of an 82-hour one. That asymmetry, not DRY_RUN, is why the long
                # dry run looked healthy and the short live run took no trades.
                return self._skip_latarb('pre_boot_interval', '%s %s interval opened %.0fs ago, before this process had oracle history - no USD anchor is possible; unanchorable for its remaining %.0fs', coin, mkt.market_id, max(0.0, now - interval_start), max(0.0, ttc))
            elif cached > 0:
                return self._skip_latarb('anchor_currency', '%s %s open anchor src=%r is not verified-USD - refusing mixed-currency displacement', coin, mkt.market_id, _anchor_src or 'unset')
            else:
        ...
        # R42 (root cause of the 2026-08-19 anchor_currency flood, 0 -> 4823 in six
        # minutes): this used to be guarded on `market_id not in _open_prices`, but
        # _warm_open_price(:7372) -> :4812 already inserted a Binance-kline
        # placeholder under that key. So once a kline landed first the 'chainlink'
        # stamp below could NEVER be written, which made the cached-anchor safety
        # net at :7346 (`cached > 0 and _anchor_src == 'chainlink'`) dead code for
        # that market. Consequence: the market evaluated fine while the live read
        # worked, then the first transient read failure - a feed gap, or the history
        # window pruning past interval_start - fell through to :7369 and refused it
        # as mixed-currency for the whole rest of its life. Gate the write on the
        # STAMP, not on key presence: reaching here means either :7319 returned a
        # direct Chainlink read (USD by construction, so it must overwrite a kline)
        # or :7349 already served a 'chainlink' value (condition False, no rewrite).
        if strat_op is not None and strat_op._open_price_src.get(mkt.market_id) != 'chainlink':
            _prev_src = strat_op._open_price_src.get(mkt.market_id)
            strat_op._open_prices[mkt.market_id] = float(open_price)
            strat_op._open_price_src[mkt.market_id] = 'chainlink'
            if _prev_src:
                self.log.info('open anchor upgraded %s %s src=%r -> chainlink @ %.4f (kline placeholder no longer poisons this market)', coin, mkt.market_id, _prev_src, float(open_price))
        # R33 FIX (root cause of the loss): displacement used to be
        #     (price - open_price) / open_price
        # where `price` is the Binance LEAD tick and `open_price` comes from
        # Chainlink RTDS history. Binance quotes in USDT, Chainlink in USD, so
        # the persistent USDT/USD basis entered every displacement as fake
        # upward momentum. Measured on the operator's 1,095-row shadow log:
        # basis median +10.01bp, 98.9% positive, and near-identical across
        # BTC/ETH/SOL (per-coin median spread only 0.94bp) - the signature of a
        # QUOTE-CURRENCY offset, not feed lag (lag would scale with asset vol).
        # The artifact was 3.39x LARGER than the real oracle move it was added
        # to (median |basis| 10.01bp vs median |oracle move| 2.95bp), so
        # `disp_too_small` cleared on flat markets: 784 of 784 gate-clearing
        # rows were UP, 0 were DN. The bot read UP 83.7% of the time on a feed
        # that settles UP 13.8% of the time -> 8W/17L, -$22.62 over 25 settles.
        # Chainlink is what SETTLES these markets, so measure displacement on
        # Chainlink at BOTH ends: one feed, one currency, no basis.
        oracle_now = float(oracle_px) if oracle_px is not None and oracle_px > 0 else 0.0
        if oracle_now <= 0:
            # N12 FIX, now load-bearing: the settlement feed IS the price input,
            # not just a sign confirmation, so a stale oracle means no signal.
            # r46-g: log the AGE. This skip fires 10,887 times in 4.58h but
            # kills only 6.7% of arrivals at this gate -- NOT the 71.2% of all
            # skips I first published. That figure was _log_latarb_skip's
            # 1-line-per-key-per-30s throttle (:7434) read as a frequency; a
            # key that fires continuously saturates the limiter no matter how
            # often it fires. The field is still worth having: the tolerance is
            # the hard-coded 1.2s at :252 against an RTDS push feed, so "1.2s
            # is too tight" stays a guess until the age is on record.
            # Instrument, then fix -- same order as the LEAK #5 FIVEMIN timer.
            _age = float('inf')
            try:
                if self.tracker is not None and self.tracker.feed is not None:
                    _age = float(self.tracker.feed.price_age_s(coin))
            except Exception:
                pass
            if math.isfinite(_age):
                self._oracle_age_s.append(_age)
            return self._skip_latarb('oracle_stale', '%s %s oracle feed stale/missing (oracle_px=None age=%.2fs tol=%.2fs)', coin, mkt.market_id, _age, LATARB_ORACLE_MAX_AGE_S)
        displacement = (oracle_now - open_price) / open_price
        up = displacement > 0
        # R44: displacement floor. Verified on 655 venue-graded candidates:
        # sub-5bps displacements win 32% vs ~40% breakeven. See Config fields.
        if abs(displacement) * 10000.0 < self.cfg.latarb_min_disp_bps:
            return self._skip_latarb('disp_floor')
        # R45: paired-quote instrumentation for the pre-registered fade test.
        # Logged BEFORE the side gate so DN-displaced intervals are captured
        # too; opus.log only - the shadow CSV schema stays byte-identical.
        _pair_key = str(getattr(mkt, 'market_id', ''))
        if _pair_key and now - self._pair_log_ts.get(_pair_key, 0.0) >= 60.0 \
                and mkt.book_yes is not None and mkt.book_no is not None \
                and mkt.book_yes.best_ask is not None and mkt.book_no.best_ask is not None:
            self._pair_log_ts[_pair_key] = now
            # R46b: q is the EFFECTIVE value from _emp_q (band-aware when
            # enabled), not the flat config field, and frac is logged so the
            # pre-registered R* test (2g-G) can be scored from opus.log alone
            # without needing a separate capture.
            _pe_frac = ((float(mkt.tf_secs) - ttc) / float(mkt.tf_secs)) if mkt.tf_secs else 0.0
            self.log.info('PAIREVAL coin=%s mid=%s side=%s yes_ask=%.4f no_ask=%.4f disp=%.6f q=%.4f frac=%.3f',
                          coin, _pair_key, 'UP' if up else 'DN',
                          float(mkt.book_yes.best_ask), float(mkt.book_no.best_ask),
                          displacement,
                          self._emp_q(up, float(mkt.book_yes.best_ask) if up else float(mkt.book_no.best_ask),
                                      abs(displacement) * 10000.0),
                          _pe_frac)
        # R46b (Critic 2d-2): elapsed-fraction floor for rule R*. Placed AFTER
        # PAIREVAL on purpose - instrumentation must keep capturing every row,
        # including the early-window rows this gate rejects, or the corpus that
        # would validate/refute the gate stops being collected the moment the
        # gate is enabled.
        if self.cfg.latarb_min_elapsed_frac > 0.0 and mkt.tf_secs:
            if ((float(mkt.tf_secs) - ttc) / float(mkt.tf_secs)) < self.cfg.latarb_min_elapsed_frac:
                return self._skip_latarb('early_window')
        # R44: side filter. DN underperformed its own ask by -8.4c/share
        # (t=-3.02) on every coin in both regimes.
        if self.cfg.latarb_side_filter != 'BOTH' and up != (self.cfg.latarb_side_filter == 'UP'):
            return self._skip_latarb('side_filter')
        # R33: the old oracle-sign veto compared the Binance lead against the
        # Chainlink oracle. Under a single feed it reduces to displacement**2 <
        # 0, which is never true, so it is deleted rather than left as dead
        # code. It was the largest skip bucket precisely because the two feeds
        # disagreed by the basis on every tick.
        book = mkt.book_yes if up else mkt.book_no
        token_chk = mkt.yes_token if up else mkt.no_token
        # R51: do not trade a market whose book feed is not being fed. The R50
        # watchdog already measures this and was left detect-only on the premise
        # that "book_stale blocked every eval ... zero trades on stale books".
        # That premise is FALSE when all four shards die at once: in the
        # 2026-08-31 06:32-10:40 stall book_stale took +270,313 rejections and
        # 103 fills still went through, booking +336.67 while the NIC received
        # 57 packets/s. A fixed 250 ms age threshold cannot see a rate collapse,
        # because age-at-eval is small by construction. Gate on rate instead.
        # Counter only, no log line - the stall itself is already logged by R50.
        if self.polyfeed is not None and self.polyfeed.shard_stalled(token_chk) is not None:
            return self._skip_latarb('shard_throughput_stalled')
        ...
        # Transport age (last WS touch). No hard 600ms floor ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â liquid books never clear it.
        ...
        # r46-e: was min(...)>T, which rejects only when BOTH books are stale - the opposite
        # of the documented intent. fresh_books() requires both present AND both fresh, and
        # routes through is_stale() so the venue-clock check applies here too.
        if not mkt.fresh_books(LATARB_DUAL_BOOK_STALE_MS):
        ...
        ask = book.best_ask
        if ask is None or ask > self.cfg.latarb_ask_cap:
            return self._skip_latarb('ask_cap')
        # R38: see Config.latarb_min_ask - a cheap ask clears the edge gate by
        # arithmetic, since model_prob is ~0.56 regardless of the book.
        if ask < self.cfg.latarb_min_ask:
            return self._skip_latarb('ask_floor')
        ...
        # R33: same single-feed rule as `displacement` above. Using the Binance
        # `price` here put the +10bp USDT/USD basis straight into z and p_up.
        log_disp = math.log(oracle_now / open_price) if open_price > 0 else 0.0
        if not math.isfinite(log_disp):
        ...
        # Critic 2k-1(3): bound the tail before Phi. Stated precisely, because
        # the directive that asked for it overstates what it can do: this does
        # NOT hold the 0.85 clip shut, since Phi(3)=0.9987 >> 0.85. It only stops
        # the ~0.9% of rows with |z|>3 from pinning the clip on their own. What
        # actually keeps the gate from going hyper-permissive is
        # GBM_SIGMA_FLOOR_PER_SEC staying at its default (Critic 2k-2).
        if _Z_ABS_CAP > 0.0:
            z = max(-_Z_ABS_CAP, min(_Z_ABS_CAP, z))
        p_up = 0.5 * (1.0 + math.erf(z / math.sqrt(2.0)))
        model_prob = p_up if up else 1.0 - p_up
        model_prob = max(0.3, min(0.85, model_prob))
        # R45: empirical-calibration mode. When configured, the floored-sigma
        # Phi(z) probability is REPLACED by the maintained graded win rate for
        # the traded side (root_cause_report.md: Phi(z) overpredicts +13.82pp,
        # z~=7; AUC 0.5418). Both-or-neither is enforced in Config.validate.
        # R46b: routed through _emp_q so the band-aware split (2g-C) applies
        # here and in PAIREVAL from one definition - a second copy of the
        # band arithmetic is exactly how the log and the gate drift apart.
        _emp_q_eff = self._emp_q(up, ask, abs(displacement) * 10000.0)
        if _emp_q_eff > 0.0:
            model_prob = max(0.3, min(0.85, _emp_q_eff))
        ...
        # Last gate before sizing, and deliberately after the cheap ones so the
        # skip counters stay comparable to the pre-filter run. Two distinct keys:
        # quiet_unproven = not enough shadow samples yet (the filter is starving,
        # e.g. the market is too new), not_quiet = the book demonstrably moves.
        _qr = _quiet_reason(self._quiet_ask.get(mkt.market_id), self.cfg.latarb_quiet_min_samples, self.cfg.latarb_quiet_ask_range)
        if _qr:
            return self._skip_latarb(_qr)
        ...
        # r52: LatArb hardcoded otype='FAK' and ignored cfg.entry_mode, so the
        # only path that actually trades crossed the spread as a taker even with
        # ENTRY_MODE=maker. The 2026-09-04 run paid $11.74 of fees on an $11.67
        # gross loss - 50.2% of the total. Maker fee on this venue is 0 (fd.to
        # verified live), so the maker leg re-prices to bid-side with fee=0 and no
        # crossing-delay buffer. Gate threshold is UNCHANGED: same latency_arb_edge,
        # measured against the price actually paid, exactly as the taker leg does.
        _maker_entry = str(getattr(self.cfg, 'entry_mode', 'taker')).lower() == 'maker'
        if _maker_entry:
            _post_px = maker_entry_price(book.best_bid if book else None, book.best_ask if book else None, tick, self.cfg.maker_join_ticks, model_prob)
            if _post_px is None:
                return self._skip_latarb('no_maker_price', '%s %s no valid maker price (bid=%s ask=%s tick=%.4f)', coin, mkt.market_id, book.best_bid if book else None, book.best_ask if book else None, tick)
            fill_px = float(_post_px)
            fee_fill = 0.0
            edge_fill = model_prob - fill_px
        else:
        ...
        async with self._place_lock:
            ...
            # CORRELATION CAP.  2026-08-19 dry run: BTC DN @0.460 $2.39 and
            # SOL DN @0.530 $2.74 opened 2.1s apart in the SAME 12:35-12:40Z
            # interval, both lost, -$5.12 of a -$4.27 session.  BTC/ETH/SOL
            # 5m returns are highly correlated, so N same-direction entries
            # in one window is ONE bet at Nx size.  Every pre-existing cap
            # missed it: max_position (:7506) is per-market, net/gross
            # (:7511) are bankroll-scaled DOLLAR caps that $5.11 never
            # approached, max_concurrent_evals (:4670) is a FiveMinStrategy
            # eval semaphore, and Risk (:8470) counts no positions.
            # Keyed on (tf_secs, end_time): discovery runs 5m AND 15m
            # markets concurrently (:3882), so end_time alone would merge a
            # 5m and a 15m window that close on the same second.  Direction
            # comes from latarb_hold_tokens, written a few lines below
            # INSIDE this same lock and discarded on miss - no new state to
            # leak.  Linear scan over by_coin (~12 markets: 3 coins x 5m/15m
            # x 2 offsets); no index needed at that size.
            _ikey = (int(mkt.tf_secs), int(round(float(mkt.end_time))))
            _same_dir = 0
            for _peers in self.by_coin.values():
                for _pm in _peers:
                    if not _pm.end_time or str(_pm.market_id) == str(mkt.market_id):
                        continue
                    if (int(_pm.tf_secs), int(round(float(_pm.end_time)))) != _ikey:
                        continue
                    _held = getattr(_pm, 'latarb_hold_tokens', None) or set()
                    if (_pm.yes_token in _held) if up else (_pm.no_token in _held):
                        _same_dir += 1
            if _same_dir >= self.cfg.max_latarb_per_interval_dir:
                return self._skip_latarb('interval_dir_cap', '%s %s already %d live %s latarb leg(s) in the interval ending %.0f (tf=%ds) - cap %d; correlated coins make this one bet at %dx size', coin, mkt.market_id, _same_dir, 'UP' if up else 'DN', float(mkt.end_time), int(mkt.tf_secs), self.cfg.max_latarb_per_interval_dir, _same_dir + 1)
        ...
        if token:
            # r52: under maker entry the expectation is the POSTED price, not the
            # depth-walked ask VWAP, or realized-slip is measured against a price
            # this order never tried to pay.
            self._expected_fill_px[token] = float(fill_px) if _maker_entry else float(entry_vwap2)
        ...
        try:
            # r52: om.place() converts a non-IOC otype into post-only GTD when
            # cfg.entry_mode == 'maker' (:2895), so 'GTC' here is the existing
            # maker plumbing, not a new one.
            oid = await self.om.place(token, Side.BUY, fill_px if _maker_entry else sweep, sz, Strategy.TEMPORAL, otype='GTC' if _maker_entry else 'FAK', neg_risk=mkt.neg_risk, tick_size=tick, quote_ts=book.ts if book else None, max_quote_age_ms=self.cfg.latarb_shadow_max_age_ms if self.cfg.latarb_shadow_max_age_ms > 0.0 else None)
        ...
        # P1: use actual tracked matched shares + avg fill only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never size/entry_vwap rewrite.
        matched_sz = 0.0
        actual_fill_px = 0.0
        if oid:
        ...
        # E3: the order asked for the QUANTIZED share count, not sz/sweep.
        req_shares = ioc_buy_request_shares(sz, float(sweep), max_usdc=self.cfg.max_order_size, mkt=mkt)
        if req_shares <= 0.0:
            req_shares = matched_sz
        ...
        if filled:
            ...
            # Live resting maker order (oid set, filled=False): do NOT unwind.
            # The 'latarb' tag + hold + _pending_entry set before place() stay
            # armed so the deferred dry-run fill lands settlement PnL. Only a
            # genuinely dead order (oid falsy: stale-quote/STP reject, FAK
            # zero-match) unwinds here.
            if strat is not None:
                strat._pending_entry[rkey] = max(0.0, strat._pending_entry.get(rkey, 0.0) - sz)
                if strat._pending_entry.get(rkey, 0.0) < 1e-09:
                    strat._pending_entry.pop(rkey, None)
    ...
    def _shadow_scan(self, coin: str, oracle_px: Optional[float]=None) -> None:
        # R33: the Binance `lead_px` argument this used to take is GONE, not
        # merely unused. The shadow path now has no access to a USDT-quoted
        # price at all, so the mixed-feed defect cannot be reintroduced here.
        try:
            ...
            for mkt in markets:
                ...
                if not open_price or open_price <= 0:
                    continue
                # R33 FIX: the shadow log has to measure what the live path now
                # measures, or the recorded evidence keeps the defect the live
                # path no longer has. `price` is the Binance LEAD (USDT-quoted),
                # `open_price` is Chainlink (USD); using both put the +10bp
                # USDT/USD basis into every logged disp / z / model_prob. Log
                # the settling feed at both ends, exactly like _eval_market.
                # NOTE: with Chainlink in the `spot_price` column the Binance
                # lead is no longer recorded, so the basis itself is no longer
                # observable from this CSV. That is deliberate - the basis can
                # no longer reach the signal. If the sub-30s lead-lag strategy
                # is ever scoped, add a `lead_price` column at that point.
                settle_px = float(oracle_px) if oracle_px is not None and oracle_px > 0 else 0.0
                if settle_px <= 0:
                    continue
                disp = (settle_px - open_price) / open_price
                ...
                if max_age > 0.0 and chosen_book.age_ms > max_age:
                    continue
                if not mkt.fresh_books(LATARB_DUAL_BOOK_STALE_MS):  # r46-e: see _eval_market
                    continue
                # R48 (P3): unbiased panel. The throttle now applies to EVERY row,
                # including near-close, and prior candidacy no longer suppresses
                # later rows. Previously `near_close = ttc <= 5.0` bypassed both
                # the candidate check and the throttle, which put 99.48% of the
                # corpus in the final 5s where nothing is purchasable.
                if now - self._shadow_last.get(mkt.market_id, 0.0) < throttle:
                    continue
                self._shadow_last[mkt.market_id] = now
                # Sample the quiet-book window HERE and nowhere else: this is the
                # exact point, cadence and freshness the rule was measured on, and
                # `by.best_ask` is the same value that lands in the tape's yes_ask
                # column the measurement read.
                _qw = _quiet_note(self._quiet_ask.get(mkt.market_id), by.best_ask)
                if _qw is not None:
                    self._quiet_ask[mkt.market_id] = _qw
                self._shadow_log(now, mkt, coin, ttc, disp, settle_px, open_price, by, bn, oracle_px=oracle_px)
        except Exception as e:
            self.log.warning('shadow scan error: %s', e)

    def _shadow_log(self, now, mkt, coin, ttc, disp, settle_px, open_price, by, bn, oracle_px: Optional[float]=None) -> None:
        # R33: this argument was named `price` and carried the Binance LEAD tick
        # while `open_price` carried a Chainlink value. That name is how the
        # USDT/USD basis stayed invisible in every logged disp / z / model_prob.
        # Renamed to `settle_px` so the feed is unmistakable at the call site.
        iso = datetime.fromtimestamp(now, tz=timezone.utc).isoformat(timespec='seconds').replace('+00:00', 'Z')
        up = disp > 0
        ...
        # R33: settlement feed at both ends, matching _eval_market exactly.
        log_disp = math.log(settle_px / open_price) if open_price > 0 else 0.0
        ...
        # R33: `disp` is now computed FROM the oracle, so this test reduces to
        # disp*disp >= 0 - true by construction. Kept as a written column (not
        # deleted) because the header must stay byte-identical or the schema
        # check in _write_shadow_row rotates every existing log.
        oracle_sign_ok: Optional[bool] = True if oracle_price is not None and open_price > 0 else None
        mos = getattr(mkt, 'min_order_size', None)
        mos_val = float(mos) if mos is not None else 0.0
        req_shares = 0.0
        fee_rate = _market_fee_rate(mkt, self.cfg)
        taker_delay_buffer = 0.0  # N9 FIX: vestigial (sigma~0.00025 ÃƒÂ¢Ã¢â‚¬ Ã¢â‚¬â„¢ buffer~0.0001 = 0.5% of edge threshold); removed to avoid false confidence
        ...
        # R48 (P3): record every scanned market, candidate or not. The old gate
        # dropped any mid-window row that was not `base_candidate`, i.e. it
        # conditioned the corpus on `edge >= latency_arb_edge` and `ask <= 0.65`
        # - the gate under study - so the counterfactual could never be observed.
        # is_candidate / base_candidate remain reconstructible offline from the
        # existing columns; the header is deliberately NOT changed, because
        # _write_shadow_row() rotates the log on any header drift.
        if is_candidate:
            ...
            if f is None:
                ...
                if os.path.exists(path) and os.path.getsize(path) > 0:
                    ...
                    if existing_hdr != header.strip() or corrupt_rows:
                        ...
                        need_header = True
                # R36 FIX: line-buffered, same reason as the calibration log.
                f = open(path, 'a', buffering=1, encoding='utf-8')
                self._shadow_fh = f
                if need_header:
                    f.write(header)
                # R35 FIX (L3): --latarb-analyze reads its gate thresholds from the
                # CURRENT env (:10294), never from the file, so replaying a CSV on
                # a box whose .env has drifted grades it against gates that never
                # produced it. Worked example: a run recorded with min_age=0/
                # max_age=250/min_prob=0.58 (the from_env fallbacks) replayed on
                # a box whose .env supplied 400/2000/0.50 made the harness report
                # "entry_window: 8 -> age_ge_min: 0 / No scorable signals" and
                # hid a real -0.54/share result. Write the gate config to a
                # SIDECAR file rather than into the CSV: three separate
                # csv.DictReader consumers (:4234, :10119, :10298) take the first
                # line as the header, so a '#' comment row there would be read as
                # the field names and break all of them. Best-effort - a failure
                # here must never cost a shadow row.
                try:
                    with open(path + '.gate.json', 'w', encoding='utf-8') as _gf:
                        json.dump({
                            'recorded_at': datetime.now(timezone.utc).isoformat(),
                            'build': _BUILD_FINGERPRINT,
                            'latarb_shadow_min_age_ms': float(self.cfg.latarb_shadow_min_age_ms),
                            'latarb_shadow_max_age_ms': float(self.cfg.latarb_shadow_max_age_ms),
                            'latency_arb_min_prob': float(self.cfg.latency_arb_min_prob),
                            'latency_arb_edge': float(self.cfg.latency_arb_edge),
                            'min_top_book_usdc': float(self.cfg.min_top_book_usdc),
                            'gbm_sigma_floor_per_sec': GBM_SIGMA_FLOOR_PER_SEC,
                            'dry_run': bool(self.cfg.dry_run),
                        }, _gf, indent=2, sort_keys=True)
                except Exception as _e:
                    self.log.warning('shadow gate-config sidecar write failed: %s', _e)
            f.write(line)
...
class RedemptionEngine:
    ...
    def enqueue(self, condition_id: str, neg_risk: bool, shares_yes: float, shares_no: float, on_settled: Callable[[Optional[float], Optional[List[float]]], None]) -> bool:
        if not condition_id:
            self.log.warning('Redeem: missing condition_id ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â cannot enqueue; settlement metadata retained')
            return False
    ...
    async def run(self) -> None:
        if not self.cfg.redeem_enabled:
            self.log.info('Redeem engine disabled (REDEEM_ENABLED=false) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â winning legs settle as soft estimates only')
            return
        loop = asyncio.get_running_loop()
        ...
        while True:
            ...
            try:
                ...
                if _is_rpc_transport_error(e):
                    self._drop_web3()
                # R35 FIX (R3): escalate severity once an item is clearly stuck.
                # Nothing here escalates on attempt count: only the
                # `elif self._is_terminal_phase(phase)` branch (:7814) ever calls
                # on_fatal, and that covers just nonce_conflict / receipt_error /
                # stuck. Every other failure - a persistent 'insufficient funds
                # for gas', a gas cap RuntimeError, an endpoint that estimates
                # badly - lands here, backs off to the 256s ceiling and re-queues
                # unconditionally in the finally block below, forever.
                # _status_loop prints no
                # pending-redemption count either, so unredeemed capital could sit
                # stranded indefinitely behind a WARNING that looks identical on
                # attempt 3 and attempt 300. Deliberately NOT wired to on_fatal:
                # that halts all trading (halt_type='redemption'), and a
                # recoverable RPC problem must not take the bot down. A CRITICAL
                # line is enough for the operator to see it and act.
                if item['attempts'] >= 10:
                    self.log.critical('REDEEM STRANDED: %s has failed %d consecutive attempts (retrying every %.0fs, capital remains unredeemed): %s', item.get('condition_id', '?')[:18], item['attempts'], delay, e)
                else:
    ...
    def _finalize_receipt(self, item: dict, receipt: dict, cond: bytes, target: str, holder: str) -> dict:
        ...
        if status != 1:
            ...
            # R35 FIX (R2): count the revert as an attempt and back off. This
            # branch returned phase='queued' with neither an `attempts` bump nor
            # a '_next_delay', so run() fell through to the flat
            # `delay = next_delay or _REDEEM_POLL_S` at :7825 - 5s - and, because
            # tx_hash/raw_tx/nonce are set to None here and therefore popped by
            # the :7792-7793 merge, the next pass re-prepared and re-submitted
            # with a fresh nonce. A deterministically reverting redemption
            # (already-redeemed condition, unresolved condition, wrong target)
            # therefore burned a signed transaction every ~15-20s forever, paying
            # real gas on each, with nothing escalating and nothing slowing down.
            # Only the exception path at :7829-7831 had a backoff; a revert is not
            # an exception, so it never reached it. Reuse that exact formula so
            # both failure routes decay identically (ceiling 2**8 = 256s), and
            # keep phase='queued' - a revert IS retryable, so this throttles the
            # retry rather than making it terminal.
            _att = int(item.get('attempts', 0)) + 1
            return {'phase': 'queued', 'attempts': _att, 'last_tx_hash': tx_hash, 'tx_hash': None, 'raw_tx': None, 'nonce': None, 'prepared_at': None, 'submitted_at': None, 'last_error': f'transaction reverted status={status}', '_next_delay': min(300.0, float(2 ** min(_att, 8)))}
        try:
        ...
        if tx_hash:
            ...
            if known:
                if age >= _REDEEM_STUCK_S:
                    # R35 NOTE (R1): the usual cause is fee underpricing. _gas_kwargs
                    # (:8042) is evaluated ONCE at prepare time and baked into the
                    # signed raw, and the rebroadcast at :7969 re-sends those exact
                    # bytes - there is no fee bump and no same-nonce replacement
                    # anywhere. maxFeePerGas = base*2 + priority, so a tx prepared
                    # at base 40 gwei carries ~110 gwei and simply will not mine
                    # while the base fee sits above that. Deliberately NOT fixed by
                    # re-signing with a bumped fee: that means rebuilding and
                    # re-signing a real-money transaction, and this failure neither
                    # loses capital (CTF redemption never expires) nor stays stuck
                    # by itself - the receipt check above runs BEFORE this terminal
                    # short-circuit, so if the tx ever mines the boot sweep flips
                    # the row to 'confirmed' and clears the halt automatically. Name
                    # the likely cause in the message so the operator can act
                    # instead of having to derive it.
                    return {'phase': 'stuck', 'terminal_error': f'redemption transaction pending for {age:.0f}s (nonce {item.get("nonce", "?")}, tx {tx_hash[:18]}); it is in the mempool but not mining - most likely the baked-in maxFeePerGas is below the current base fee. The signed tx is rebroadcast unchanged, so it cannot self-correct: either wait for the base fee to fall, or replace it externally at the same nonce with a higher fee. Operator reconciliation required.', 'last_checked_at': time.time(), '_next_delay': 60.0}
...
class Risk:
    ...
    def __init__(self, cfg: Config, om: OrderManager) -> None:
        ...
        # of the day.  See ok() Ã¢â‚¬â€ using the live balance made the cap shrink with
        # every loss, so the operator's configured allowance was never reachable.
        self._day_start_bankroll = 0.0
        # R35 FIX (R7): last daily cap announced by ok(), so the "which knob
        # binds" line logs on change instead of on every tick.
        self._daily_cap_logged = -1.0
        self._day_reset = time.monotonic()
        # AUDIT FIX (calendar-day anchor): variance halts (daily_loss /
        # consec_losses / drawdown) must clear at the UTC midnight boundary the
        # operator expects, not on a rolling 24h window anchored to bot start
        # (that left the bot dead for up to ~24h after a halt).  The anchor is
        # persisted in risk_row['day_anchor'] and restored on boot.
        self._day_anchor: Optional[str] = None  # ISO date (YYYY-MM-DD), UTC
    ...
    def ok(self) -> bool:
        if self.om.fill_apply_error:
            ...
            return False
        # AUDIT FIX (calendar-day anchor): roll the day at the UTC midnight
        # boundary (persisted _day_anchor), with the old rolling-24h window as
        # fallback only until an anchor is established (e.g. state written by
        # an older build).  Variance halts clear here, exactly as the refusal
        # message promises ("clear themselves at the next daily reset").
        _now_utc_day = datetime.now(timezone.utc).date().isoformat()
        if self._day_anchor is None:
            self._day_anchor = _now_utc_day
        if self._day_anchor != _now_utc_day or time.monotonic() - self._day_reset > 86400:
            self._day_start = self._pnl
            self._day_start_bankroll = self._current_bankroll()  # R10 FIX: re-anchor the day's budget
            self._daily_cap_logged = -1.0  # R35 FIX (R7): re-announce the new day's cap
            self._day_reset = time.monotonic()
            self._day_anchor = _now_utc_day
            ...
            if self._halted:
                ...
                if self._halt_type in ('daily_loss', 'consec_losses', 'drawdown'):
                    ...
                    self._halt_type = ''
                    self._consecutive_losses = 0
                    self.log.info('Daily reset ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â halt cleared (type was %s)', _cleared_type)
                else:
                    self.log.warning('Daily reset ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â halt NOT cleared (type=%s requires operator)', self._halt_type)
        if self._halted and self._halt_type == 'rejects' and (self.om.rejects < 5):
            self.log.info('Reject-halt auto-cleared ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â venue recovered (reject count back to %d)', self.om.rejects)
            self._halted = False
            self._reason = ''
            self._halt_type = ''
        if self._halted:
        ...
        # instead of B0*p Ã¢â‚¬â€ at MAX_DAILY_LOSS_PCT=0.35 that is 26% of bankroll,
        ...
        # R35 FIX (R7): announce WHICH cap binds, once per trading day. The
        # branch above is deliberate (see the comment at :8086) but it is
        # completely silent, and the two knobs can differ by an order of
        # magnitude: on a $9.33 paper bankroll the default pct of 0.05 yields a
        # $0.47 cap while MAX_DAILY_LOSS reads $50.00 in the same config, so an
        # operator who set the dollar figure sees a halt at 1% of it and no
        # indication that MAX_DAILY_LOSS was never consulted. Deliberately NOT
        # changed to min(): that would silently tighten a funded live account
        # from pct to the $50 default, which is a behaviour change nobody asked
        # for. Log the effective number instead and let the operator choose.
        if abs(daily_cap - self._daily_cap_logged) > 1e-09:
            self._daily_cap_logged = daily_cap
            if basis > 0.0 and self.cfg.max_daily_loss_pct > 0.0:
                self.log.info('Daily loss cap $%.2f = %.2f%% of day-start bankroll $%.2f (MAX_DAILY_LOSS_PCT binds; MAX_DAILY_LOSS=$%.2f is NOT consulted while pct>0).', daily_cap, self.cfg.max_daily_loss_pct * 100.0, basis, self.cfg.max_daily_loss)
            else:
                self.log.info('Daily loss cap $%.2f from MAX_DAILY_LOSS (no bankroll observed yet, or MAX_DAILY_LOSS_PCT=0).', daily_cap)
        if dp < -daily_cap:
...
class Bot:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        # A7 FIX: durable state writes (json.dump + fsync + os.replace) used to run
        # on the asyncio loop thread inside _pos_lock on EVERY fill, stalling WS
        # delta processing and order placement for the duration of the syscall.
        # max_workers=1 is load-bearing, not a tuning choice: snapshots are
        # FULL-STATE, so two concurrent writes could land out of order and
        # resurrect a stale position.  One worker serialises them by construction.
        self._state_io_pool = concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix='state-io')
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            self.log.warning('Interrupt received ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â running graceful shutdown')
            # R35 FIX (L5): record the interrupt in the durable boot record.
            # This handler SWALLOWS the interrupt rather than re-raising, so
            # _boot returns, run() returns, and main() reaches its success line
            # and writes 'EXIT rc=0 | run() returned normally' (:10651) - an
            # interrupted run was indistinguishable from a clean one in the one
            # file an operator reads to reconstruct why the process stopped.
            # `booted` matters because the SIGINT/SIGTERM handler is only
            # registered at :9033, three lines before `self.running = True`
            # (:9036): an interrupt arriving before that point kills the process
            # during recover_open_orders / _verify_live_position_state with no
            # handler installed, and _shutdown then no-ops on its
            # `if not self.running` guard (:10128). Nothing new leaks - the boot
            # places no orders - but the operator should be able to tell that the
            # graceful shutdown the line above claims did not actually run.
            boot_record('EXIT rc=0 INTERRUPTED', f'interrupt during {"run" if self.running else "boot"}; graceful shutdown {"ran" if self.running else "SKIPPED (handler not yet installed)"}')
            try:
            ...
            # R35 FIX (L3): run the graceful shutdown on EVERY exceptional exit,
            # not just KeyboardInterrupt. Both `except FatalBotError: raise`
            # (:8292) and `except Exception: ... raise` (:8301) re-raised straight
            # past this block, so a fatal exit skipped _shutdown entirely - and
            # with it om.cancel_all() (:10132) and _save_runtime_state() (:10138).
            # That is the damaging pair: a live fatal left resting BUY orders
            # working on the venue with no process left to manage them, and
            # discarded the runtime state the next boot reconstructs positions
            # from. FatalBotError is not an edge case either - it is the designed
            # fatal exit that every supervised loop deliberately re-raises into.
            # Safe to call unconditionally: _shutdown short-circuits on
            # `if not self.running` (:10128), so the KeyboardInterrupt path that
            # already ran it at :8297 and the normal path that ran it via
            # _shutdown_wait (:10104) both no-op here, and a crash before
            # `self.running = True` correctly does nothing. It MUST run before the
            # session/client teardown below, because cancel_all needs the network.
            # Its own exception is logged and swallowed so it can never mask the
            # original fault that is propagating.
            try:
                await self._shutdown()
            except Exception as se:
                self.log.error('Shutdown during exceptional exit failed: %s', se)
    ...
    async def _save_runtime_state_async(self) -> None:
        """Loop-friendly twin of _save_runtime_state for the per-fill hot path.

        A7 FIX: builds the snapshot ON the loop thread (it reads live Market /
        Risk objects - see _snapshot_runtime_state) and awaits the blocking write
        on the single-worker state-io pool.

        The `await` is deliberate and must NOT be turned into fire-and-forget.
        _on_fill raises on checkpoint failure so latch_fill_failure can block all
        subsequent orders; acknowledging a fill before it is durable would trade
        a latency spike for silent capital-state loss, which is strictly worse.
        """
        if self._state_identity is None or not self._state_path:
            return
        state = _snapshot_runtime_state(self.markets, self.fivemin, self.risk, self.redeemer, self.cfg.dry_run, self._state_identity, self._state_path, self._trade_pnl_in_flight, self.om, self._applied_trade_order, self._applied_ioc_order, defer_write=True)
        path = self._state_path
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(self._state_io_pool, _write_latarb_state_durable, state, path)
    ...
    async def _boot(self) -> None:
        ...
        self.log.info('Polymarket SDK: %s', 'py-clob-client-v2 (CLOB V2)' if _SDK_IS_V2 else 'py-clob-client V1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â DEAD, orders WILL be rejected' if _HAS_SDK else 'NOT INSTALLED')
        ...
        if not self.cfg.dry_run and self.cfg.entry_mode == 'maker':
            ...
            if not hasattr(OrderType, 'GTD'):
                raise FatalBotError('LIVE maker: OrderType.GTD missing from CLOB SDK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â install py-clob-client-v2; refuse silent GTC')
            try:
                ...
                raise FatalBotError('LIVE maker: OrderArgs lacks expiration= support ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â upgrade py-clob-client-v2') from e
        ...
        if balance_opt is None:
            if self.cfg.dry_run:
                self.log.warning('CLOB balance fetch failed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dry-run continues with simulated bankroll floor')
                ...
                await asyncio.sleep(FATAL_PREFLIGHT_HOLD_S)
                raise FatalBotError('LIVE trading refused: could not fetch USDC balance (API failure ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not the same as a $0 wallet)')
        else:
            balance = balance_opt
        ...
        # AUDIT FIX: warn when the effective daily loss cap is below ~2 minimum
        # clips - the first losing clip then always trips the halt.  Seen live
        # on 2026-08-06: 5% x $13.33 = $0.67 cap vs a $2.59 clip halted the bot
        # after its first paper trade, then the refusal loop ran for hours.
        # Mirrors Risk.ok(): pct-of-bankroll binds when pct > 0, else dollars.
        _daily_cap_eff = (balance * self.cfg.max_daily_loss_pct) if (self.cfg.max_daily_loss_pct > 0.0 and balance > 0.0) else self.cfg.max_daily_loss
        _min_clip_floor = _VENUE_MARKET_BUY_MIN_SHARES * 0.5
        if _daily_cap_eff < 2.0 * _min_clip_floor:
            _cap_src = ('MAX_DAILY_LOSS_PCT=%.2f%% x $%.2f' % (self.cfg.max_daily_loss_pct * 100.0, balance)) if self.cfg.max_daily_loss_pct > 0.0 else 'MAX_DAILY_LOSS'
            self.log.critical('DAILY-CAP WARNING: effective daily loss cap $%.2f (%s) is below 2x the minimum clip ($%.2f = %d shares x $0.50). The FIRST losing clip will halt the bot until the next daily reset. Set MAX_DAILY_LOSS_PCT=0 to let MAX_DAILY_LOSS=$%.2f bind.', _daily_cap_eff, _cap_src, _min_clip_floor, int(_VENUE_MARKET_BUY_MIN_SHARES), self.cfg.max_daily_loss)
        ...
        if self.cfg.dry_run:
            passed = True
            self.log.info('DRY_RUN ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â skipping signing test')
            ...
            self.log.info("Running signing test on '%s'ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦", test_mkt.question[:40])
            passed = await self.client.test_order(test_token, test_tick, test_neg)
        if not passed:
            if not self.cfg.dry_run:
                self.log.warning('signing test FAILED for sig_type=%d ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refusing to try alternatives in LIVE (each probe costs an order). Set POLYMARKET_SIGNATURE_TYPE=1 or 2 in .env and restart.', self.cfg.signature_type)
            else:
                self.log.warning('sig_type=%d failed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â trying alternatives in DRY_RUNÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦', self.cfg.signature_type)
        ...
        if self.cfg.dry_run and str(getattr(self.cfg, 'entry_mode', '')).lower() == 'maker':
            self.polyfeed.set_trade_sink(self.om.on_dry_trade_print)
            self.log.info('r53 dry maker: fills come from real CLOB prints (back-of-queue), not DRY_RUN_FILL_PROB')
        ...
        if self.risk.halted and self.risk.status()['halt_type'] == 'redemption' and (not self.cfg.dry_run):
            ...
            if self._redeem_reconcile_deferred:
                ...
                try:
                    ...
                    raise
        # R35 FIX: settle already-expired restored inventory BEFORE the halt
        # gate, not after it. This loop used to sit below the `raise
        # SystemExit(EXIT_HALTED)`, so a refused boot skipped it entirely: a
        # market that expired while the bot was down stayed unsettled in the
        # state file, its P&L never reached the ledger, and every subsequent
        # refused boot re-restored the same stale position. The refusal message
        # then reported a pnl/day figure that predated the resolution. This is
        # pure bookkeeping over _redeem_meta (:5669) - it places no orders and
        # touches no chain - so it is safe on a refusing boot. Guarded because a
        # settlement integrity failure must not change the refusal outcome or
        # mask the halt that is about to be reported.
        ...
        for _mid in _restored_mids:
            ...
            if _m is not None and _m.end_time is not None and _m.end_time <= time.time():
                try:
                    ...
                    self.log.critical('boot settlement of expired restored market %s failed: %s', _mid, _e)
        ...
        if self.risk.halted:
            ...
            _clear_at = 'within 24h of the day start (rolling window)'
            _anchor = getattr(self.risk, '_day_anchor', None)
            if _anchor:
                try:
                    from datetime import timedelta as _td
                    _clear_at = (datetime.strptime(str(_anchor), '%Y-%m-%d').replace(tzinfo=timezone.utc) + _td(days=1)).strftime('%Y-%m-%d %H:%M UTC')
                except Exception:
                    pass
            self.log.critical('  daily_loss / consec_losses / drawdown halts clear at the next daily reset (UTC midnight): %s.', _clear_at)
            if _st['halt_type'] == 'daily_loss' and self.cfg.max_daily_loss_pct > 0.0:
                _basis = max(0.0, float(getattr(self.risk, '_day_start_bankroll', 0.0) or 0.0))
                self.log.critical('  cap = MAX_DAILY_LOSS_PCT %.2f%% x day-start bankroll $%.2f = $%.2f (MAX_DAILY_LOSS=$%.2f is NOT consulted while pct>0). Set MAX_DAILY_LOSS_PCT=0 to use the dollar cap.', self.cfg.max_daily_loss_pct * 100.0, _basis, self.cfg.max_daily_loss_pct * _basis, self.cfg.max_daily_loss)
            ...
            # R35 FIX: this was the ONLY refusal in _boot without a hold, and it is
            # the one refusal guaranteed to repeat - the conditions it refuses on
            # (a persisted variance halt) can only clear on a wall-clock timer, so
            # an immediate exit re-refuses on the very next boot. The four sibling
            # fatal-preflight paths at :8353/:8402/:8420/:8434 all sleep
            # FATAL_PREFLIGHT_HOLD_S first for exactly this reason. EXIT_HALTED's
            # own contract (:177-178) says a supervisor must NOT restart on rc=3,
            # but the 2026-08-03 operator ran Restart=always: the bot booted,
            # refused and exited every ~8.3s for 11h05m - 4,683 full boot
            # sequences on a $0.96 paper loss. Each cycle paid CLOB auth, a
            # balance fetch, market discovery (~74 Gamma requests), 24/24 book
            # + tick REST seeds and a per-market fee fetch for 12 markets. The
            # hold does not shorten the halt (the day-age clock at :6413 spans
            # restarts, so it clears at the same absolute time either way) - it
            # only stops a misconfigured supervisor from melting the API quota.
            self.log.critical('Holding %.0fs before exit to avoid a supervisor crash loop.', FATAL_PREFLIGHT_HOLD_S)
            await asyncio.sleep(FATAL_PREFLIGHT_HOLD_S)
        ...
        if self.cfg.latency_arb_enabled and (not self.cfg.dry_run) and getattr(self.cfg, 'require_latarb_live_proof', False):
            ...
            if not ok_live:
                self.log.critical('LATARB LIVE-PROOF: NO-GO ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fill/settle evidence failed:')
                for rsn in live_reasons:
                    self.log.critical('  - %s', rsn)
                raise FatalBotError('LATARB LIVE-PROOF failed; fix the evidence (or set LATARB_BOOTSTRAP_LIVE=true to run a deliberate min-size canary). Do NOT disable REQUIRE_LATARB_LIVE_PROOF - that removes the check entirely instead of satisfying it.')
            mode = live_stats.get('mode', 'pass')
            if mode == 'bootstrap':
                self.log.warning('LATARB LIVE-PROOF: BOOTSTRAP ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â insufficient live samples; min-size only. fills=%s settle=%s', live_stats.get('fills'), live_stats.get('settle'))
            else:
                self.log.info('LATARB LIVE-PROOF: GO ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â fills+settle evidence passed %s', live_stats)
        ...
        elif self.cfg.latency_arb_enabled and (not self.cfg.dry_run):
            # A8: live, but the operator opted OUT of the proof gate.  Sizing still
            # fails safe (_live_proof_ok stays False => min_order_size clips), so
            # this is a warning, not a block - the operator may be running the
            # deliberate min-size canary that bootstrap mode exists for.
            # It is loud because the measured signal has NEGATIVE expectancy
            # (see _measure_latarb_edge.py: 18.8% win rate at avg entry 0.503,
            # EV -0.63/$, reproducing the realised -$22.62 over 25 trades), so
            # every unproven live clip is expected to lose money.
            self.log.critical('LATARB LIVE, UNPROVEN: REQUIRE_LATARB_LIVE_PROOF=false so no fill/settle evidence was checked. Orders are capped at min_order_size ($%.2f) because the live-proof sizing flag stays off. Set REQUIRE_LATARB_LIVE_PROOF=true to enforce the gate.', self.cfg.min_order_size)
        ...
        if _LOOP_LAG_WARN_MS > 0:
            self.tasks.append(asyncio.create_task(self._loop_lag_watch(), name='loop_lag'))
        if self._redeem_reconcile_deferred:
        ...
        if self.cfg.dry_run and str(getattr(self.cfg, 'entry_mode', '')).lower() == 'maker':
            self.tasks.append(asyncio.create_task(self.om.dry_maker_loop(), name='dry_maker'))
        try:
    ...
    async def _loop_lag_watch(self) -> None:
        """R46c: measure event-loop starvation directly instead of inferring it.

        2g-F refuted the lead-fan-out hypothesis - r46-a cut market evals 4.7x
        and the strict `1013 slow consumer` close rate did not move (7.13/hr over
        3.23h vs the 8.21/hr baseline, P(X<=23|lam=26.48)=0.29) - and the box is
        nowhere near saturated (bot 23% CPU, loadavg 0.30 on 2 cores). A reader
        that stalls on an idle box is being blocked in bursts, not outrun.

        `BOOK RESYNC (drift)` is enriched +37pp in the 3s before a first-in-
        cluster close (11/17 vs 28% expected, ~3.4 sigma), but the resync BATCH
        SIZE carries no signal at all: P(close within 3s | Y>=3) = 0.008 =
        P(close | Y<3), and the Semaphore(6) binds on only 3.1% of batches. So
        resync is not the CPU burst - drift is detected because deltas were
        already being dropped, which makes it a fellow symptom of reader lag.
        That leaves the actual stall unidentified, and one more guess is worth
        less than one measurement: a 0.5s sleep returning 300ms late means 300ms
        in which nothing drained the socket.
        """
        last = time.monotonic()
        worst = 0.0
        breaches = 0
        while self.running:
            await asyncio.sleep(0.5)
            now = time.monotonic()
            lag_ms = (now - last - 0.5) * 1000.0
            last = now
            if lag_ms >= _LOOP_LAG_WARN_MS:
                breaches += 1
                worst = max(worst, lag_ms)
                self.log.warning('LOOPLAG %.0fms (breaches=%d worst=%.0fms thresh=%.0fms)', lag_ms, breaches, worst, _LOOP_LAG_WARN_MS)
    ...
    async def _seed_books(self) -> None:
        self.log.info('Seeding order books and tick sizesÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦')
    ...
    async def _refresh_onchain_winners(self, markets: List[Market]) -> int:
        """R38: fill fivemin._onchain_winner from CLOB tokens[].winner.

        Dry-run settlement had no authoritative input: the 20.7h run logged zero
        'market_resolved push' lines and booked all 25 settles from the spot compare,
        which named the wrong winner on 8 of them.  Only markets with settlement
        metadata and a passed end_time are polled, and only once each - a resolved
        winner never changes.  A market that is simply not resolved yet stays quiet
        and retryable; a transport/rate-limit failure is counted and warned about
        (R39) because silently losing this poll re-arms the spot-estimate fallback.
        Neither case ever invents an outcome.
        """
        if not self.fivemin or not self.session:
            return 0
        now = time.time()
        want: Dict[str, str] = {}
        for m in markets:
            mid = str(m.market_id)
            if mid in self.fivemin._onchain_winner or not m.condition_id:
                continue
            if not m.end_time or float(m.end_time) > now:
                continue
            if not any(k[0] == mid for k in self.fivemin._redeem_meta):
                continue
            want[mid] = str(m.condition_id)
        if not want:
            return 0

        async def _one(mid: str, cond: str) -> Optional[Tuple[str, str]]:
            try:
                async with self.session.get(f'{self.cfg.clob_url}/markets/{cond}', timeout=aiohttp.ClientTimeout(total=6)) as r:
                    if not r.ok:
                        # R39: no HTTP error is evidence of "no winner yet" - 404/5xx
                        # are just as blind as 429. Count them all, cache none.
                        return (mid, '')
                    data = await r.json(content_type=None)
            except Exception:
                return (mid, '')
            for tok in (data or {}).get('tokens') or []:
                if tok.get('winner') and tok.get('token_id'):
                    return (mid, str(tok['token_id']))
            return None

        found = 0
        errors = 0
        for res in await asyncio.gather(*[_one(k, v) for k, v in want.items()], return_exceptions=True):
            if isinstance(res, tuple) and len(res) == 2:
                # R39: '' marks a transport/rate-limit failure, NOT "no winner yet".
                # Only a real token_id is cached; a failure must stay retryable.
                if res[1]:
                    self.fivemin._onchain_winner[res[0]] = res[1]
                    found += 1
                else:
                    errors += 1
                ...
                errors += 1
        if found:
            self.log.info('On-chain winners: resolved %d/%d closed markets awaiting settlement', found, len(want))
        # R39: a silent wipeout here re-arms the dry-spot-ESTIMATE fallback that
        # R38 was built to remove, so say so out loud. Only warn when transport
        # actually failed - an unresolved-but-reachable market is the normal case
        # and must stay quiet.
        if errors and errors >= max(2, (len(want) + 1) // 2):
            self.log.warning('On-chain winner poll degraded: %d/%d requests failed (rate limit or network). Settlement will fall back to dry-spot-ESTIMATE after DRY_ONCHAIN_WAIT_S=%.0fs unless this clears.', errors, len(want), DRY_ONCHAIN_WAIT_S)
        return found
    ...
    async def _enrich_market_fees(self, markets: List[Market]) -> None:
        ...
        if not markets:
            return
        # Phase 2.2: no SDK (e.g. dry run with no key) meant this returned in
        # silence and every market kept CATEGORY_FEE_RATE, with nothing in the
        # log to distinguish "venue says 0.07" from "we assumed 0.07".
        if not self.client or not getattr(self.client, 'sdk', None):
            self.log.warning('Per-market fees: NO SDK - every market keeps the assumed CATEGORY_FEE_RATE=%.4f (exponent 1.0). Fee-dependent P&L is an assumption, not a measurement, for this run.', self.cfg.category_fee_rate)
            return
        loop = asyncio.get_running_loop()
        sdk = self.client.sdk
        seen: Set[str] = set()
        ...
        def _fetch_one(cond: str, yes_tid: str) -> Tuple[Optional[float], bool, float]:
            ...
            try:
                ...
                if getter and cond:
                    ...
                    if r is not None:
                        rf = float(r)
                        # R38: only a already-fractional rate is the coefficient.
                        if rf <= 1.0:
                            rate = rf
                            enabled = rate > 0
            ...
            try:
                ...
                if bps_fn and yes_tid:
                    # R38: get_fee_rate_bps returns an integer scale that does not
                    # match charged fees (1000 -> 0.10 vs 0.07 observed) and the old
                    # bps==1 branch produced rate=1.0, a 100% coefficient. Neither is
                    # usable, so the rate is left None and CATEGORY_FEE_RATE stands.
                    # Still called: it is the liveness probe for fee_exponent below.
        ...
        for m, res in zip(mkts, results):
            ...
            fetched.append(float(rate))
            updated += 1
        # Phase 2.2: the old line read 'updated 24/24 markets (fallback
        # CATEGORY_FEE_RATE=0.0700)', which names the fallback next to a full
        # success and so cannot be read either way. Report the rates that came
        # back and how many markets actually fell back.
        if updated:
            self.log.info('Per-market fees: %d/%d markets returned a venue rate %s; %d fell back to CATEGORY_FEE_RATE=%.4f', updated, len(mkts), sorted({round(r, 6) for r in fetched}), len(mkts) - updated, self.cfg.category_fee_rate)
        else:
            self.log.warning('Per-market fees: 0/%d markets returned a venue rate - all of them use the assumed CATEGORY_FEE_RATE=%.4f', len(mkts), self.cfg.category_fee_rate)
    ...
    async def _on_market_resolved(self, event: dict) -> None:
        ...
        self.log.info('market_resolved push %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â settling (winner=%s redeem=%s)', mkt.coin, (winning or '?')[:18], getattr(self.cfg, 'redeem_enabled', False))
    ...
    async def _on_fill(self, mkt: Market, tid: str, side_str: str, shares: float, price: float, trade_id: str='', order_ids: Optional[Set[str]]=None, aggregate_ioc: bool=False) -> bool:
        if not math.isfinite(price) or not 0.0 < price < 1.0:
            self.log.warning('_on_fill: invalid price %r for %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dropping', price, tid[:12])
            return False
        if not math.isfinite(shares) or not 0.0 < shares < 100000000.0:
            self.log.warning('_on_fill: invalid shares %r for %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â dropping', shares, tid[:12])
            return False
        async with self._pos_lock:
            ...
            if already_applied:
                ...
                try:
                    await self._save_runtime_state_async()
                except Exception as e:
                    self.om.latch_fill_failure(f'duplicate fill checkpoint failed for {trade_id or tid}: {e}')
                    raise
                return False
            if side_str not in ('BUY', 'SELL'):
                self.log.warning('_on_fill: unknown side %r Ã¢â‚¬â€ dropping', side_str)
                return False
            ...
            if self.fivemin:
                ...
                if side == Side.BUY:
                    ...
                    self.fivemin.note_cash_mutation()
            # A2 FIX: write fill data back to TrackedOrder.  _on_fill updated the
            # Position objects but never the order book-keeping, so _orders showed
            # filled_size=0.0 for a resting order that had actually filled until
            # the next REST reconcile() - up to 30s later.  LatArb reads
            # tracked.filled_size/avg_fill_price post-fill and was seeing zeros.
            # sell_fill is 0.0 on the BUY branch and the actual reduced quantity on
            # SELL, so _fill_qty is correct for both.
            # NOTE: no filled_size-vs-size completion check here - `size` is USDC
            # notional while filled_size is SHARES.  Marking OrderState.FILLED is
            # left to reconcile(), which compares against authoritative venue data.
            _fill_qty = shares if side == Side.BUY else sell_fill
            if _fill_qty > 0 and event_order_ids and self.om is not None and not aggregate_ioc:
                for _oid in event_order_ids:
                    _tr = self.om._orders.get(_oid)
                    if _tr is None:
                        continue
                    _prev = _tr.filled_size
                    _new_filled = _prev + _fill_qty
                    _tr.avg_fill_price = ((_tr.avg_fill_price * _prev + price * _fill_qty) / _new_filled if _prev > 0 else price)
                    _tr.filled_size = _new_filled
                    # A fill proves the order reached the venue, so PENDING is stale.
                    if _tr.state == OrderState.PENDING:
                        _tr.state = OrderState.OPEN
    ...
    async def _replay_rest_fill(self, trade: dict) -> bool:
        ...
        if mkt is None:
            # R37: reconcile_fills' first pass walks back 3600s before boot
            # (:2487) but every market this bot trades lives 5 or 15 minutes, so
            # a fill from 16-60 min ago is GUARANTEED to name a market that has
            # already expired: discover_5min_markets returns only ACTIVE markets
            # and a flat expired one leaves no persisted placeholder (:6507).
            # There is no destination for such a fill and there never will be,
            # yet raising here latched fill_durability (:2523 -> :8395) and
            # persisted halted=true, so EVERY later boot refused with rc=3.
            # Live on 2026-08-17: one real ETH 15m fill (BUY 5.2173 @ 0.4599,
            # 10:56:09Z, market ended 11:00:00Z) bricked the 11:52 restart and
            # every restart after it. Nothing is lost by skipping - that token's
            # venue position was already read and classified at boot by
            # _verify_live_position_state (:8703-8719).
            # A fill that lands while we are LIVE is a different fault (our
            # market map disagreeing with the venue) and still latches.
            ts = self.om._extract_trade_ts(trade)
            if 0.0 < ts < self.om._boot_ts:
                self.log.warning('_replay_rest_fill: SKIPPING pre-boot fill %s on token %s - it matched %.0fs before this process started and no live or persisted market owns it (expired); position recovery already accounts for it', trade_id[:16], asset_id[:16], self.om._boot_ts - ts)
                return False
        ...
        for (mkt, tid, local_shares, side_label), chain_shares in zip(tasks_meta, results):
            ...
            if diff >= threshold:
                ...
                if not first_msg:
                    first_msg = msg
        if nan_count:
            self.log.warning('drift check: %d/%d fetches returned NaN ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â coverage may be incomplete; if this persists, on-chain RPC is down', nan_count, len(results))
        if nan_count == len(results) and len(results) > 0:
            self.risk._halt(f'drift check: total RPC failure ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all {len(results)} fetches returned NaN', halt_type='rpc')
    ...
    async def recover_open_orders(self) -> None:
        if self.cfg.dry_run:
            ...
            return
        if not self.client.sdk:
            raise FatalBotError('recover_open_orders: no SDK ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â cannot verify open orders')
        try:
        ...
        for order in owned:
            ...
            ok = await self.client.cancel(oid)
            if not ok:
                self.log.warning('recover_open_orders: cancel returned false for %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â will verify remaining list', oid[:16])
        try:
            await self.client.cancel_all()
        except Exception as e:
        ...
        if still:
            ...
            raise FatalBotError(f'Open-order recovery did not converge ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â {len(still)} still open (sample={ids}). Refuse to trade with unmanaged resting risk.')
        async with self.om._lock:
            self.om._orders.clear()
            self.om._by_token.clear()
        self.log.info('Open-order recovery: converged ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â book clean')
    ...
    async def _apply_balance_snapshot(self, new_bal: float, expected_gen: Optional[int]=None) -> None:
        ...
        # R36 FIX: dropped the 'free * max_bankroll_fraction < min_order_size'
        # term. It made a small wallet self-halt: the shock detector cannot tell
        # our own fills from an external withdrawal, so >= $6 of fills inside one
        # BALANCE_REFRESH_S window trips the shock, and on a $6.39 wallet the
        # residual free cash then failed this test -> halt_type='capital_shock',
        # which :8214 does NOT auto-clear at UTC midnight (operator must re-arm).
        # 'cannot size a minimum order' is already handled as a soft skip at
        # :6976, which stops trading without paging anyone. Inventory over the
        # gross cap is a real problem and still halts.
        if self.cfg.halt_on_capital_shock and (not self.cfg.dry_run) and gross > _gross_cap + 1e-09:
            self.risk._halt(f'capital_shock: free=${free:.2f} gross=${gross:.2f} gross_cap=${_gross_cap:.2f} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â operator must re-arm after inventory is reduced or capital restored', halt_type='capital_shock')
        else:
            self.log.warning('capital_shock handled: free=$%.2f gross=$%.2f caps net/gross=$%.2f/$%.2f ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â new entries sized to residual (cooldown %.0fs)', free, gross, _net_cap, _gross_cap, self.fivemin._capital_shock_cancel_cooldown_s)
        ...
        while self.running:
            ...
            try:
                ...
                if self.fivemin:
                    ...
                    # R35 FIX (F2): same as the timer loop - a settlement integrity
                    # violation must halt, not vanish into a DEBUG line. Must
                    # precede `except Exception` (RuntimeStateError is a
                    # RuntimeError subclass at :5973).
                    except RuntimeStateError as se:
                        self.log.critical('Status loop settlement safety failure: %s', se)
                        self.risk._halt(f'status loop settlement safety failure: {se}', halt_type='state')
                    except Exception as se:
                    ...
                    # Phase 0.5: only the on-chain-graded slice is quotable P&L.
                    _cen = self.fivemin._settle_src_census
                    if _cen:
                        _oc = 'dry-onchain-winner'
                        _n_oc = _cen.get(_oc, 0)
                        self.log.info('  GRADED  onchain n=%d net=$%+.2f wr=%.4f | other %s | quote the onchain slice only', _n_oc, self.fivemin._settle_pnl_by_src.get(_oc, 0.0), self.fivemin._settle_wins_by_src.get(_oc, 0) / _n_oc if _n_oc else 0.0, ' '.join(f'{k}={v}(${self.fivemin._settle_pnl_by_src.get(k, 0.0):+.2f})' for k, v in sorted(_cen.items()) if k != _oc) or 'none')
                    # Phase 0.4: R46c null-slip probe. Pass: med_bps <= 0 and
                    # frozen0 < 5% of sim fills; otherwise dry P&L is optimistic
                    # by med_bps and must be regraded on the frozen ladder.
                    if self.cfg.dry_run and self.om is not None and self.om._dryslip_n > 0:
                        _ds_f = self.om._dryslip_fills
                        _ds_z = self.om._dryslip_frozen_zero
                        _ds_med = statistics.median(self.om._dryslip_bps) if self.om._dryslip_bps else 0.0
                        self.log.info('  DRYSLIP  probes=%d sim_fills=%d med_lookahead_bps=%+.1f (n=%d) frozen_sh0=%d (%.1f%% of fills) verdict=%s', self.om._dryslip_n, _ds_f, _ds_med, len(self.om._dryslip_bps), _ds_z, 100.0 * _ds_z / _ds_f if _ds_f else 0.0, 'n/a:no-sim-fills' if _ds_f == 0 else 'PASS' if _ds_med <= 0.0 and _ds_z / _ds_f < 0.05 else 'FAIL:dry-pnl-optimistic')
                if self.latency_arb is not None and (self.cfg.latency_arb_enabled or self.latency_arb.attempts > 0):
                    ...
                    # Phase 1.2: the distribution the 1.2s tolerance should be
                    # tuned against. Only ages of REJECTED reads are here, so a
                    # p50 far above the tolerance means the feed goes quiet in
                    # bursts, not that the tolerance is marginally too tight.
                    _oa = sorted(self.latency_arb._oracle_age_s)
                    if _oa:
                        self.log.info('  LATARB oracle_stale ages n=%d p50=%.2fs p90=%.2fs max=%.2fs tol=%.2fs', len(_oa), _oa[len(_oa) // 2], _oa[min(len(_oa) - 1, int(0.9 * len(_oa)))], _oa[-1], LATARB_ORACLE_MAX_AGE_S)
        ...
        while self.running:
            ...
            try:
                ...
                for sid, age in shard_ages.items():
                    if age > 90:
                        self.log.warning('Shard %d stale (%.0fs) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â restarting', sid, age)
                        await self.polyfeed.restart_shard(sid)
                # R50: liveness != throughput. The age>90 check above never fired
                # once in 66.43h, including through a 3.9h dual-shard stall at
                # ~1-4% of normal rate, because any single message refreshes
                # _last_msgs. This closes that blind spot.
                #
                # DETECT-ONLY ON PURPOSE - do not "improve" this into a restart.
                # The venue reconnected shard 1 at 04:10:49 and shard 0 at
                # 04:22:38 inside that stall ('no close frame received or sent',
                # which forces a reconnect + resubscribe). Throughput in the 10min
                # after each was LOWER than the 5min before (max 13.1 and 15.6
                # msg/s vs a 540.5 median). A reconnect demonstrably did not fix
                # it, twice, so restarting here would only fire ~310 futile
                # resnapshots over 3.9h and each one costs ~10% of evals via
                # mark_shard_not_ready(). The cause is off-box.
                #
                # Trading was never at risk: book_stale blocked every eval
                # (+33829 during the stall) and attempts/fills was identical at
                # both ends (43/39), so zero trades on stale books. The real cost
                # is 3.9h of degraded tape entering the shadow corpus unmarked -
                # which is what P4 gets graded on. This WARNING is the fix: it
                # makes those windows greppable and excludable. sol_ver.md R50.15.
                # R51: ONE call - shard_msg_rates() resets its own baseline, so a
                # second call anywhere would halve the window. Publish for the
                # entry gate; keep a non-empty previous sample if this one is {}.
                _rates = self.polyfeed.shard_msg_rates()
                if _rates:
                    self.polyfeed.last_shard_rates = _rates
                for sid, rate in _rates.items():
                    if _WS_MIN_MSG_RATE > 0 and rate < _WS_MIN_MSG_RATE:
                        self.log.warning('Shard %d throughput stalled: %.2f msg/s < floor %.0f - tape degraded, cause is off-box, NOT restarting (sol_ver R50.15)', sid, rate, _WS_MIN_MSG_RATE)
                overall_age = self.polyfeed.last_msg_age_s
                if overall_age > 120:
                    self.log.warning('ALL shards stale ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â full reconnect')
                ...
                if stale_binance:
                    self.log.warning('Binance feed stale for %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reconnecting', ', '.join(stale_binance))
                    await self.binance.restart()
                stale_cl = [c for c in self.cfg.coins if self.chainlink.price_age_s(c) > 20.0]
                if stale_cl:
                    self.log.warning('Chainlink RTDS stale for %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â reconnecting', ', '.join(stale_cl))
        ...
        while self.running:
            ...
            if self.fivemin and self.fivemin_markets:
                try:
                    ...
                    # R35 FIX (F9): pass the FULL market list, not one filtered on
                    # _recovery_placeholder. The filter was correct when this call
                    # still drove the directional entry fan-out - you cannot
                    # evaluate an entry on a placeholder that has no book. But
                    # evaluate_all is now nothing but the periodic drift check for
                    # _net_exposure / _gross_exposure (:4476-4491), and placeholders
                    # carry REAL cost basis: _market_from_runtime_row (:6455) builds
                    # them and the restore loop at :6440-6442 writes pos.shares /
                    # pos.cost straight onto them. Excluding them made this resync
                    # OVERWRITE the correct incremental totals with a number that
                    # silently omitted every restored position - so the LatArb caps
                    # read low and admitted size the account did not have. The flag
                    # is cleared on rediscovery (:9987), which hides the leak in the
                    # common case; a market that expired during downtime is never
                    # rediscovered, so its placeholder under-reports permanently.
                    await self.fivemin.evaluate_all(self.fivemin_markets)
                # R35 FIX (F2): RuntimeStateError must HALT, not log at DEBUG.
                # _apply_settlement raises it on five settlement-integrity
                # violations, and settle_expired_latarb (called just above) has no
                # guard of its own. The discovery loop already treats it as
                # halt-worthy (:9981-9983 -> halt_type='state'), but this timer
                # loop swallowed it into a DEBUG line that is invisible at the
                # default INFO level. Because this loop runs on STRATEGY_INTERVAL_S
                # (1.5s) and discovery on DISCOVERY_INTERVAL_S (10s), the silent
                # handler usually wins the race - so a corrupt settlement could be
                # dropped repeatedly with no operator-visible trace while the bot
                # kept trading on the bad book. Ordering matters: RuntimeStateError
                # subclasses RuntimeError (:5973), so this clause MUST precede the
                # broad `except Exception` below or it would never be reached.
                except RuntimeStateError as e:
                    self.log.critical('Timer loop settlement safety failure: %s', e)
                    self.risk._halt(f'timer loop settlement safety failure: {e}', halt_type='state')
                except Exception as e:
    ...
    async def _fivemin_refresh(self) -> None:
        ...
        while self.running:
            ...
            try:
                ...
                now = time.time()
                if self.fivemin:
                    # R38: authoritative winner first - settle_expired_latarb is sync and
                    # would otherwise fall through to the spot estimate.
                    _t0 = time.monotonic()
                    try:
                        await self._refresh_onchain_winners(self.fivemin_markets)
                    except Exception as we:
                        self.log.debug('on-chain winner refresh: %s', we)
                    _t1 = time.monotonic()
                    self.fivemin.settle_expired_latarb(self.fivemin_markets, now)
                    _t2 = time.monotonic()
                    # LEAK #5: name the sub-step instead of refactoring on a
                    # guess. Only the settle half is SYNCHRONOUS, so only it can
                    # stall the loop; the winner poll is awaited and yields.
                    # mod300 places each sample against the 300s boundary the
                    # LOOPLAG breaches cluster 1.9-3.1s past.
                    if (_t2 - _t0) * 1000.0 >= _FIVEMIN_STEP_WARN_MS:
                        self.log.info('FIVEMINSTEP mod300=%.1f winners_ms=%.1f settle_sync_ms=%.1f n_mkts=%d',
                                      now % 300.0, (_t1 - _t0) * 1000.0,
                                      (_t2 - _t1) * 1000.0, len(self.fivemin_markets))
                keep: List[Market] = []
                ...
                # R35 FIX (L6): WARNING, not DEBUG. LOG_LEVEL's effective default
                # is INFO (:506), so a DEBUG line here is invisible on a normal
                # deployment. Discovery itself is not the exposure - it absorbs
                # its own HTTP/parse failures per-request and always logs a
                # visible 'Found %d 5-min markets' line, so a dead Gamma endpoint
                # already shows up as 'Found 0'. The uncovered part is the
                # post-discovery body of this handler: polyfeed.subscribe_live,
                # _enrich_market_fees, polyfeed.unsubscribe and
                # _save_runtime_state. A persistent subscribe_live failure leaves
                # newly discovered markets with no book feed - the bot looks
                # healthy and simply never trades them - and a failing
                # _save_runtime_state silently stops checkpointing. Neither is
                # halt-worthy on its own (the next refresh retries in
                # DISCOVERY_INTERVAL_S), but both must be visible.
                self.log.warning('Discovery refresh failed: %s', e)
    ...
    def _prune_expired_bot_state(self, active_mids: Set[str]) -> None:
        ...
        # R39: _onchain_winner is filled by _refresh_onchain_winners and was never
        # pruned, so it grew for the life of the process (~864 markets/day). Drop
        # entries only once the market is gone from fivemin_markets AND has no
        # settlement metadata left - a market awaiting the DRY_ONCHAIN_WAIT_S
        # window is retained in fivemin_markets precisely so its winner survives,
        # and pruning it early would silently re-arm the spot-ESTIMATE fallback
        # that R38 exists to prevent.
        if self.fivemin is not None:
            _pending_settle = {k[0] for k in self.fivemin._redeem_meta}
            for _mid in [m for m in self.fivemin._onchain_winner if m not in active_mids and m not in _pending_settle]:
                self.fivemin._onchain_winner.pop(_mid, None)
    ...
    async def _shutdown(self) -> None:
        if not self.running:
            return
        self.running = False
        self.log.info('Shutting downÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦')
        await self.om.cancel_all()
        await self.polyfeed.stop()
        ...
        for t in self.tasks:
            if t is not asyncio.current_task():
                t.cancel()
        # R35 FIX (L4): cancel the fire-and-forget tasks too, and WAIT for them,
        # before the executor below closes. self._bg_tasks (:8264, fed at :9343,
        # :9354, :9903) was never cancelled and never awaited - its done-callback
        # is a bare `discard`, so results are dropped on the floor. A bg task
        # still inside _on_fill when _state_io_pool.shutdown(wait=True) completed
        # would reach the run_in_executor at :8360 against a dead pool and get
        # RuntimeError('cannot schedule new futures after shutdown'). _on_fill
        # does catch that and route it into om.latch_fill_failure, so the failure
        # is latched rather than silent - but the durable checkpoint for that fill
        # is still lost, which is precisely the state the next boot rebuilds
        # positions from. Awaiting here closes the window: by the time the pool
        # shuts down, nothing is left that could enqueue another write.
        # return_exceptions=True because these tasks are being cancelled by
        # design and a CancelledError here must not abort the rest of shutdown.
        _bg = [t for t in self._bg_tasks if t is not asyncio.current_task()]
        for t in _bg:
            t.cancel()
        if _bg:
            try:
                await asyncio.wait_for(asyncio.gather(*_bg, return_exceptions=True), timeout=5.0)
            except Exception as e:
                self.log.warning('Background task drain incomplete at shutdown: %s', e)
        if self.metrics:
            self.log.info('Final metrics: %s', self.metrics.summary())
        self._drift_io_pool.shutdown(wait=True, cancel_futures=True)
        # A7: drain queued state writes before exit.  cancel_futures would discard
        # a pending durable checkpoint, so this one waits without cancelling.
        self._state_io_pool.shutdown(wait=True)
        self.log.info('Final status: %s', self.risk.status())

    def _banner(self) -> None:
        self.log.info('=' * 64)
        self.log.info('  POLYMARKET CRYPTO BOT %s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Antigravity Opus 4.6', _BOT_VERSION)
        self.log.info('=' * 64)
        if self.cfg.private_key:
            key_hash = hashlib.sha256(self.cfg.private_key.encode()).hexdigest()[:8]
            self.log.info('  KeyHash  : %sÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦  (sha256[:8] of POLYMARKET_PRIVATE_KEY)', key_hash)
        else:
            self.log.info('  KeyHash  : <not set>')
        self.log.info('  Proxy    : %s', self.cfg.proxy_address[:20] if self.cfg.proxy_address else '(none ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â EOA mode)')
        ...
        self.log.info('  LatArb   : enabled=%s  shadow=%s  age=%.0f..%.0fms  edge=%.3f  min_prob=%.2f  ask_cap=%.2f  live_proof=%s', self.cfg.latency_arb_enabled, self.cfg.latarb_shadow, self.cfg.latarb_shadow_min_age_ms, self.cfg.latarb_shadow_max_age_ms, self.cfg.latency_arb_edge, self.cfg.latency_arb_min_prob, self.cfg.latarb_ask_cap, self.cfg.require_latarb_live_proof)
        self.log.info('  QuietBk  : %s  min_samples=%d  ask_range=%.4f  (sampled on the %.0fms shadow throttle; verdict FROZEN at first candidacy)', 'ON' if self.cfg.latarb_quiet_min_samples > 0 else 'OFF', self.cfg.latarb_quiet_min_samples, self.cfg.latarb_quiet_ask_range, self.cfg.latarb_shadow_throttle_ms)
        self.log.info('  LatArb gates: max_spread=%.2f min_top_depth=$%.2f oracle_max_age=%.1fs cooldown=%.1fs ttc_exit=%.0fs', self.cfg.max_spread_pct, self.cfg.min_top_book_usdc, LATARB_ORACLE_MAX_AGE_S, self.cfg.latency_arb_cooldown, self.cfg.forced_exit_ttc_s)
        # R37: state the warm-up explicitly. The oracle feed has no backfill, so
        # every market whose interval opened before this line is unanchorable in
        # USD for the rest of its life and skips as 'pre_boot_interval'. 15m
        # markets are discovered (:3677), so that is up to 15 min of structural
        # blackout after every restart - 79% of a 21-minute run, 0.3% of an
        # 82-hour one. Judge a run only on the time AFTER this window closes.
        self.log.info('  LatArb warmup: no LatArb entry on any interval that opened before this line (no oracle backfill) - expect pre_boot_interval skips for up to 15 min. A short run can be mostly warmup.')
...
def evaluate_latarb_go_no_go(cfg: 'Config') -> Tuple[bool, List[str]]:
    ...
    # P1: live LatArb is one position per market ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â score first executable signal only
    ...
    for row in sorted(rows, key=lambda x: x['ts']):
        ...
        if cfg.latarb_shadow_max_age_ms > 0 and age > cfg.latarb_shadow_max_age_ms:
            continue
        # F7: live dual-book stale gate. r46-e: was min(...)>T, which rejected only when
        # BOTH books were stale; the live gate now requires BOTH FRESH, so replay uses max().
        # LIMIT: the shadow CSV logs only local apply-age (yes_age_ms/no_age_ms) and has no
        # venue-age column, so replay cannot reproduce is_stale()'s exchange-clock check.
        if max(row['ya'], row['na']) > LATARB_DUAL_BOOK_STALE_MS:
            continue
        ...
        if not ask or ask <= 0 or ask > cfg.latarb_ask_cap or ask < cfg.latarb_min_ask:
            continue
        ...
        # Mid-window close is not settlement ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â do not score as W/L (prevents false GO).
        if _res_ttc > 5.0:
            continue
    ...
    for coin in sorted({c.upper() for c in cfg.coins}):
        ...
        if not st:
            continue  # no samples yet ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â aggregate gate still protects live
        cn = int(st['n'])
        if cn < cfg.latarb_min_proven_signals:
            continue  # undersampled ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â do not fail global GO
...
def evaluate_latarb_live_proof(cfg: 'Config') -> Tuple[bool, List[str], dict]:
    ...
    # Live authorization uses LIVE samples only ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â never dry-run contamination.
    ...
    if insufficient:
        if bootstrap:
            ...
            return (True, [f'bootstrap: live attempts {att}/{min_att}, live settle {sn}/{min_sn} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â min-size canary only (dry excluded)'], stats)
        if att < min_att:
...
def _run_latarb_analyze(path: Optional[str]) -> None:
    ...
    live_min_age = max(0.0, float(cfg.latarb_shadow_min_age_ms))
    print(f'gate config: min_age={live_min_age:.0f}ms max_age={cfg.latarb_shadow_max_age_ms:.0f}ms ask_cap={cfg.latarb_ask_cap:.2f} min_prob={cfg.latency_arb_min_prob:.2f} edge>={cfg.latency_arb_edge:.3f} min_top_depth=${cfg.min_top_book_usdc:.2f} scoring=first-signal-per-market\n')
    ...
    if not raw:
        print('No parseable rows ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â nothing to analyze.')
        return
    ...
    for r in sorted(raw, key=lambda x: x['ts']):
        ...
        if max(r['ya'], r['na']) > LATARB_DUAL_BOOK_STALE_MS:  # r46-e: both-fresh, see F7
            continue
        ...
        # Mid-window close is not settlement ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â exclude from W/L (prevents false GO).
        if _res_ttc > 5.0:
            continue
    ...
    if n_sig:
        ...
        print('\nNo scorable signals (gates empty or all closes residual_ttc>5s ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not a POSITIVE).')
...
def main() -> None:
    ...
    try:
        ...
        log.info('uvloop not installed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â using stdlib asyncio loop')
    ...
    if errs:
        ...
        sys.exit(1)
    try:
        _rc_path = write_run_config(cfg)
        log.info('RUN CONFIG: resolved settings written to %s (commit this with the report)', _rc_path)
    except Exception as _rc_e:
        log.error('RUN CONFIG: could not write resolved config: %s', _rc_e)
    # R35: warn (do NOT fail) when the drawdown cap is small relative to one
    # clip. The halt at :8001 fires on give-back from the peak, so a cap below
    # ~2x max_order_size can be reached by two losing trades - and because a
    # drawdown halt only clears on the day timer (:6414), that buys a multi-hour
    # outage from ordinary variance. The shipped defaults are 50.0 vs 25.0 (2x)
    # and are fine; the 2026-08-03 operator ran 6.00 vs 4.00 (1.5x) and halted
    # after four trades totalling -$0.96. Warning, not an error: the operator may
    # deliberately want a tight cap, and failing the boot would brick a live
    # config that was merely aggressive.
    # R36 FIX: compare against MAX_CONSECUTIVE_LOSSES x MAX_ORDER_USDC, not a
    # hardcoded 2x. At 2x the guard passed $10 against $4 clips and stayed
    # silent while the drawdown halt fired on 2026-08-07 at +$16.59 net for the
    # day (peak +$28.16, give-back $11.57 > $10 cap). If the config tolerates
    # N consecutive losses, the drawdown cap must clear N max-size losses or the
    # drawdown halt pre-empts it and MAX_CONSECUTIVE_LOSSES never binds.
    _dd_floor = max(1, cfg.max_consecutive_losses) * cfg.max_order_size
    if cfg.max_drawdown_from_peak > 0.0 and cfg.max_drawdown_from_peak < _dd_floor:
        log.warning('MAX_DRAWDOWN_FROM_PEAK=$%.2f is below MAX_CONSECUTIVE_LOSSES=%d x MAX_ORDER_USDC=$%.2f = $%.2f - the drawdown halt will pre-empt the consecutive-loss halt, and a session that is net profitable can still halt on give-back from its peak. Consider >= $%.2f.', cfg.max_drawdown_from_peak, cfg.max_consecutive_losses, cfg.max_order_size, _dd_floor, _dd_floor)
    ...
    try:
        ...
        # R37: exit EXIT_HALTED, not 1. Another live instance owning the wallet is
        # not retryable - it clears only when that process dies - but rc=1 is also
        # what a crash (:10892) and a bad config (:10839) return, so a supervisor
        # cannot tell "do not restart" from "restart me". On 2026-08-17 a second
        # launcher raced pid=12083 by 3s and then burned the whole session in a
        # 25s loop (20s INSTANCE_LOCK_WAIT_S + 5s backoff), hiding the log of the
        # instance that DID hold the lock. rc=3 already means "refused to start"
        # (:9163); supervisors must break the loop on it.
        boot_record(f'EXIT rc={EXIT_HALTED} REFUSING TO START', f'instance lock: {e}')
        sys.exit(EXIT_HALTED)
    if cfg.proxy_address:
        log.info('Proxy  : %s', cfg.proxy_address)
    else:
        log.info('Proxy  : (none ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â EOA mode)')
